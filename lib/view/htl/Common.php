...
//        $fareTaxTotalInbound = 0;
    ...
    //                                    $boxInfo = $this->_BoxService->fetchBoxById($db, $key);
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costAmountTaxOutboundTotal / Sgmov_View_Eve_Common::CURRENT_TAX));
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax'] = floor((string)($fareAmountOutboundTotal * Sgmov_View_Eve_Common::CURRENT_TAX));
...
//                    }
    ...
    //Sgmov_Component_Log::debug($inForm);
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax'] = floor((string)($fareAmountInboundTotal * Sgmov_View_Eve_Common::CURRENT_TAX));
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/htl/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/htl/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/htl/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/htl/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
...
//                                $charterInfo = $this->_CharterService->fetchCharterById($db, $comiket_charter['charter_id']);
            ...
            $data['edit_url'] = $urlPublicSsl . "/htl/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            // 説明書URL
//            $data['manual_url'] = $urlPublicSsl . "/htl/pdf/manual/manual_{$comiket["eventsub_id"]}.pdf";
            $data['manual_url'] = '';
...
//            if($eventData['id'] != '2' && $eventData['id'] !== '4') { // コミケとGoOutCampは記載しない
            if($eventsubData['manual_display'] == '1') { // コミケとGoOutCampは記載しない
                $data['manual_url'] = "【説明書URL】" . PHP_EOL . $urlPublicSsl . "/htl/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            }
//            $data['manual_url'] = $urlPublicSsl . "/htl/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
...
//                    && $eventData['id'] !== '4')  {
                ...
                $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/htl/paste_tag/{$pasteTagId}/";
            }
