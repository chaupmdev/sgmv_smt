 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Zzy_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'ZZY';

    /**
     * ZZY001の画面ID
     */
    const GAMEN_ID_ZZY001 = 'ZZY001';

    /**
     * ZZY002の画面ID
     */
    const GAMEN_ID_ZZY002 = 'ZZY002';

    /**
     * ZZY003の画面ID
     */
    const GAMEN_ID_ZZY003 = 'ZZY003';
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //2022/08/19 沖縄外す対応
        unset($prefectureAry["ids"][47]);
        unset($prefectureAry["names"][47]);
        ...
        if ($inForm['comiket_detail_type_sel'] == "2") {
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Zzy_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Zzy_Common::CURRENT_TAX));
                        // // 税込
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "2") { // 復路
            if(!$this->isCurrentDateWithInTerm("arrival", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/zzy/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
        } else { // 往復
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/zzy/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/zzy/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/zzy/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
...
//        $departureTo = $eventsubData["departure_to"];
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            $payMethodList = array('2' => 'クレジット', '3' => '現金');//電子マネー⇒現金対応
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/zzy_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/zzy_complete_business_sgmv{$tmplateType}.txt";
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/zzy_complete_individual{$tmplateType}.txt";

                $mailTemplateSgmv[] = "/zzy_complete_individual_sgmv{$tmplateType}.txt";
                ...
                if($comiket['payment_method_cd'] == '3') { // 電子マネー
                    ...
                    $data['digital_money_attention_note'] = "※ イベント当日、受付にて現金でお支払いお願いします。";
                }
            }
            ...
            foreach ($comiket_detail_list as $comiket_detail) {
                ...
                //搬出用メールテンプレート
                $mailTemplate[] = '/zzy_parts_complete_choice_2.txt';

                $mailTemplateSgmv[] = '/zzy_parts_complete_choice_2_sgmv.txt';
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) { // 法人
                $mailTemplate[] = "/zzy_parts{$footerTmpName}_footer_type_1.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/zzy_parts{$footerTmpName}_footer_type_1.txt";
            } else { // 個人
                $mailTemplate[] = "/zzy_parts{$footerTmpName}_footer_type_2.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/zzy_parts{$footerTmpName}_footer_type_2.txt";
            }
            ...
            $data['edit_url'] = $urlPublicSsl . "/zzy/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            // 説明書URL
            $data['manual_url'] = '';

            if($eventsubData['manual_display'] == '1') { // 説明書表示有無 0：無、1：有
                $data['manual_url'] = "【説明書URL】" . PHP_EOL . $urlPublicSsl . "/zzy/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            }
...
//             }
            ...
            // サイズ変更/キャンセルURL
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            $data['cancel_url'] = $urlPublicSsl . "/zzy/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['size_change_url'] = $urlPublicSsl . "/zzy/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        $sendFileName = 'ZZY_' . date ( 'YmdHis' ) . '.csv';
        $boundary = "-----" . md5 ( uniqid () );
        ...
        try {
            ...
            $mailTemplateList = array(
                "/zzy_cancel_{$divType}_error.txt",
//                "/zzy_parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
            );
    ...
    protected function checkReqDate($comiketId, $titleParts = "", $MessageParts = "") {
        ...
        foreach ($comiketDetailList as $comiketDetailInfo) {
            ...
            if ($eventTermEndDate <= $toDate) {
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/zzy/error?t={$title}&m={$message}");
            }
