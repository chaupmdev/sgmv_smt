...
$exceptionFlg = FALSE;
// 処理を実行
$view = new Sgmov_View_Dsn_SizeChangeComplete();
$eveOutForm = array();
$eventData = array();
$eventsubData = array();
...
<head>
    ...
    <?php if (@empty($isCancelOnly)) : ?>
        ...
        <!-- 個数・サイズ変更 の場合表示 -->
        <!-- ********************************************************************************************** -->
        <title>宿泊ホテルより国内配送受付サービスの個数・サイズ変更お申し込みの完了│ＳＧムービング株式会社＜ＳＧホールディングスグループ＞</title>
    <?php else: ?>
        <!-- ********************************************************************************************** -->
        <!-- キャンセルのみの場合表示 -->
        <!-- ********************************************************************************************** -->
        <title>宿泊ホテルより国内配送受付サービスのキャンセルお申し込みの完了│ＳＧムービング株式会社＜ＳＧホールディングスグループ＞</title>
    <?php endif; ?>
    ...
    <div id="breadcrumb">
        <ul class="wrap">
            ...
            <?php if (@empty($isCancelOnly)) : ?>
                ...
                <!-- 個数・サイズ変更 の場合表示 -->
                <!-- ********************************************************************************************** -->
                <li class="current">宿泊ホテルより国内配送受付サービスの個数・サイズ変更お申し込みの完了</li>
            <?php else: ?>
                <!-- ********************************************************************************************** -->
                <!-- キャンセルのみの場合表示 -->
                <!-- ********************************************************************************************** -->
                <li class="current">宿泊ホテルより国内配送受付サービスのキャンセル処理完了</li>
            <?php endif; ?>
        </ul>
    </div>
    ...
    <div id="main">
        <div class="wrap clearfix">
            <?php if (@empty($isCancelOnly)) : ?>
                ...
                <!-- 個数・サイズ変更 の場合表示 -->
                <!-- ********************************************************************************************** -->
                <h1 class="page_title">宿泊ホテルより国内配送受付サービスの個数・サイズ変更お申し込みの完了</h1>
            <?php else: ?>
                <!-- ********************************************************************************************** -->
                <!-- キャンセルのみの場合表示 -->
                <!-- ********************************************************************************************** -->
                <h1 class="page_title">宿泊ホテルより国内配送受付サービスのキャンセル処理完了</h1>
            <?php endif; ?>
...
?>
        ...
        <?php if ($eventsubData['manual_display'] == '1') { ?>
            ...
            <div style="font-size:20px;">
                <a class="button_link2" href="<?=$forms['dispItemInfo']['dispEvent']['manual']?>" target="_blank" style="color:blue;">
