...
Sgmov_Lib::useView('mlk/Common', 'CommonConst');
Sgmov_Lib::useView('mlk/Bcm');
...
Sgmov_Lib::useServices(array('Comiket', 'ComiketDetail', 'ComiketBox', 'ComiketCargo', 'ComiketCharter'
    , 'CenterMail', 'SgFinancial', 'HttpsZipCodeDll', 'CargoFare', 'BoxFare', 'Charter', 'GyomuApi'));
...
class Sgmov_View_Eve_Complete extends Sgmov_View_Eve_Common {
    ...
    public function executeInner($argBackInputPaht="") {
        ...
        $inForm['comiket_toiawase_no'] = "";//業務側に連携しない為、空白で設定
        $inForm['comiket_toiawase_no_niugoki'] =  $toiawaseNo;
        
        $inForm['comiket_toiawase_no2'] = "";//業務側に連携しない為、空白で設定
        $inForm['comiket_toiawase_no_niugoki2'] =  $this->getToiawaseNo();
         // 搬入出の申込期間チェック
        ...
        // エラーが発生した時、戻る画面
        //$backInputPath = "input";
        $backInputPath = "input?tagId=".$inForm['comiket_id'];
// コミケ申込情報登録
        ...
        $outForm->raw_qr_code_string = $inForm['comiket_id'];//TagID
    ...
    public function _validate($inForm, $db) {
        ...
        if ($inForm['delivery_date_store'] !== $deliveryDateStore && $currentTime >= $dataHachakuten['confirm_end_time']."00") {
...
Sgmov_Component_Log::debug($checkForm);Sgmov_Component_Log::debug($inForm);
        ...
        switch ($inForm['comiket_payment_method_cd_sel']) {
            case '2': // クレジットカード決済
                $props = array(
                    'merchant_ccid'       => Sgmov_Component_Config::getMlkMdkCreditCardMerchantCcId(),
                    'merchant_secret_key' => Sgmov_Component_Config::getMlkMdkCreditCardMerchantSecretKey(),
                );
                break;
            case '1': // コンビニ決済
                $props = array(
...
Sgmov_Component_Log::debug($inForm['payment_url']);
    ...
    protected function registerComiket($db, $inForm, $comiketId, $backInputPath){
        ...
        try {
            ...
            //comiket_booth_name
            //$comiketDataInfoFlat["comiketData"]['booth_name'] = '';
            ...
            foreach ($comiketDataInfoFlat["comiketDetailDataList"] as $key => $val) {
                //Hard code for insert 
                /* 
                 * $val['hatsu_jis5code'] = '';
                $val['hatsu_shop_check_code'] = '';
                $val['hatsu_shop_check_code_eda'] = '';
                $val['hatsu_shop_code'] = '';
                $val['hatsu_shop_local_code'] = '';
                $val['chaku_jis5code'] = '';
                $val['chaku_shop_check_code'] = '';
                $val['chaku_shop_check_code_eda'] = '';
                $val['chaku_shop_code'] = '';
                $val['chaku_shop_local_code'] = '';
                $val['chaku_shop_local_code'] = '';
                $val['pref_id'] = '27';
                $val['address'] = '';
                 */
