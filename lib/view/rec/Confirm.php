...
Sgmov_Lib::useForms(array('Error', 'RecSession', 'Rec001Out'));
/**#@-*/
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Rec_Confirm extends Sgmov_View_Rec_Common {
    ...
    public function executeInner() {
        ...
        if(@empty($_SERVER["HTTP_REFERER"]) || strpos($_SERVER["HTTP_REFERER"], "/rec/") == false) {
            // セッション情報を破棄
        ...
        $dispItemInfo["wage"] = $this->_OccupationService->fetchWage($db, $inForm["employ_cd"], $inForm["center_id_hidden"], $inForm["occupation_cd_hidden"]);

        //  チケットを発行
        $ticket = $session->publishTicket(self::FEATURE_ID, self::GAMEN_ID_REC003);
        return array(
            'ticket'  => $ticket,
            'outForm' => $outForm,
    ...
    public function _createOutFormByInForm($inForm, $db) {
        ...
        return $this->createOutFormByInForm($inForm, new Sgmov_Form_Rec001Out());
    }
}