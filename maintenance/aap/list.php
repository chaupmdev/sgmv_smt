...
Sgmov_Lib::useView('aap/List');
/**#@-*/

// 処理を実行
$view = new Sgmov_View_Aap_List();
$forms = $view->execute();

/**
 * フォーム
 * @var Sgmov_Form_Aap001Out
 */
$aap001Out = $forms['outForm'];
?>
<!DOCTYPE html>
            ...
            <dl id="globalNav">
                ...
                <dd>
                    <ul>
                        ...
                        <li class="nav02">
<?php if($aap001Out->honsha_user_flag() === '1'){ ?>
...
<?php } ?>
    ...
    <div id="topWrap">
        ...
        <h3 class="list_ttl">マンション一覧</h3>
        <form action="/aap/input" method="post">
            <p class="add_btn">
                <a data-id="" href="#"><img alt="新規追加" src="/common/img/cruise/btn_new.png" /></a>
            </p>

            <div class="long_table">
                <table class="list_table" id="apartment_table">
                    <colgroup>
                        <col id="code" />
                        <col id="name" />
                        <col id="zip" />
                    ...
                    <thead>
                        <tr>
                            <th scope="col">マンションコード</th>
                            <th scope="col">マンション名</th>
                            <th scope="col">郵便番号</th>
                            <th scope="col">住所</th>
                            <th scope="col">取引先コード</th>
...
<?php
    ...
    $ap_ids        = $aap001Out->apartment_ids();
    $ap_cds        = $aap001Out->apartment_cds();
    $ap_names      = $aap001Out->apartment_names();
    $ap_zip_codes  = $aap001Out->apartment_zip_codes();
    $ap_address    = $aap001Out->apartment_address();
    $ap_agency_cds = $aap001Out->apartment_agency_cds();

    $html = '';
    $count = count($ap_names);
    for ($i = 0; $i < $count;  ++$i) {
        $html .= '
                        <tr data-id="' . $ap_ids[$i] . '">
                            <td class="number">' . $ap_cds[$i] . '</td>
                            <td>' . $ap_names[$i] . '</td>
                            <td>' . $ap_zip_codes[$i] . '</td>
                            <td>' . $ap_address[$i] . '</td>
                            <td>' . $ap_agency_cds[$i] . '</td>
                            <td><a href="#">▼編集</a></td>
