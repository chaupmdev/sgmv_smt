...
// イベント名
$eventName = 'コミックマーケット105　スマートコンテナ';

// 公開開始日時
$openDateStr = '2024-12-06 10:00:00';
// 公開終了日
$closeDateStr = '2024-12-20 17:00:00';
...
$endDate = new dateTime($closeDateStr);
$prm = array(
    'open' => $openDate,     'event' => $eventName, 'dir'  => 'eve' // evp固定で問題ないです
);

$prm_end = array(
    'open' => $endDate,     'event' => $eventName, 'dir'  => 'eve' // evp固定で問題ないです
);
...
function redirectKohkaiato($prm){
    ...
    $message = urlencode("「" . $prm['event'] . "」のお申込は 「" . $day . " " . $yobi . " " . $time . "」 をもって終了しました。");
