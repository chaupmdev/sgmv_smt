 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Bpn_ActiveShohin extends Sgmov_View_Bpn_Common {
    ...
    public function executeInner() {
       // セッションに情報があるかどうかを確認
        ...
        if (empty($param)) {
            ...
            $eventsubInfo = array();
            if (@empty($eventInfo)) {
                $eventsubInfo = $this->_EventsubService->getEventsubInfoByShikibetsushi($db, $shikibetsushi);
            } else {
                $eventsubInfo = $this->_EventsubService->fetchEventsubByEventsubId($db, $eventInfo["eventsubid"]);
            }
            ...
            if (empty($eventsubInfo)) {
                ...
                // イベント
                $inForm->event_sel = $eventsubInfo["event_id"];

                // inputmode
                $inForm->input_mode = $eventsubInfo["id"];
            ...
            // 数値ではない
            if (!is_numeric($bpnType)) {                
                Sgmov_Component_Log::debug ( '数値ではない' );
                Sgmov_Component_ErrorExit::errorExit(Sgmov_Component_ErrorCode::ERROR_VIEW_INVALID_PAGE_ACCESS);
            }
            
            // 数値ではない
            if (!is_numeric($shohinPattern)) {                
                Sgmov_Component_Log::debug ( '数値ではない' );
                Sgmov_Component_ErrorExit::errorExit(Sgmov_Component_ErrorCode::ERROR_VIEW_INVALID_PAGE_ACCESS);
            }
        }
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////
        // サイトの表示を shohin.term_fr(申込開始) ～ eventsub.arrival_to_time復路申込期間終了 で制御する
        ///////////////////////////////////////////////////////////////////////////////////////////////////
