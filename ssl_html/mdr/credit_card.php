...
Sgmov_Lib::useView('mdr/CreditCard');
/**#@-*/

// 処理を実行
$view = new Sgmov_View_Mdr_CreditCard();
$forms = $view->execute();
...
<head>
    ...
    <title>催事・イベント配送受付サービスのお申し込み│ＳＧムービング株式会社＜ＳＧホールディングスグループ＞</title>
...
?>
    ...
    <div id="main">
        <div class="wrap clearfix">
            ...
            <form action="/mdr/check_credit_card" method="post">
                <input name="ticket" type="hidden" value="<?php echo $ticket ?>" />
                <div class="section">
                    <p class="sentence br">
                        確定送料をご確認の上、クレジットカード情報をご入力ください。
                        <br />インターネットお申し込みについてのご質問については、<a data-inquiry-type="11" href="/pin/input" target = "_brank">こちら</a>からのみになります。
                    </p>
                    <dl class="plain_dl">
...
<?php } ?>
                    ...
                    <div class="result02 tac fs134per">
                        <span class = "wsp-nowrap">合計金額：</span>
                        &nbsp;<span class ="wsp-nowrap"><?php echo number_format((int)$eveOutForm->delivery_charge()); ?>円（税込）</span>
                    </div>
...
<?php
        echo Sgmov_View_Mdr_CreditCard::_createPulldown($eveOutForm->card_expire_month_cds(), $eveOutForm->card_expire_month_lbls(), $eveOutForm->card_expire_month_cd_sel());
?>
                                </select>
                                月
                            </label>
...
<?php
        echo Sgmov_View_Mdr_CreditCard::_createPulldown($eveOutForm->card_expire_year_cds(), $eveOutForm->card_expire_year_lbls(), $eveOutForm->card_expire_year_cd_sel());
?>
                                    </select>
                                    年
                                </label>
                        ...
                        <dl>
                            ...
                            <dd<?php if (isset($e) && $e->hasErrorForId('top_card_number')) { echo ' class="form_error"'; } ?>>
                                <input autocapitalize="off" class="w_220" id="card_number" inputmode="numeric" maxlength="16" name="card_number" data-pattern="^\d+$" placeholder="例）9999999999999999" type="<?php echo $inputTypeNumber; ?>" value="<?php echo $eveOutForm->card_number(); ?>" />
                            </dd>
                        </dl>
                        <dl>
                            <dt class="condition">
                            ...
                            <dd<?php if (isset($e) && $e->hasErrorForId('top_security_cd')) { echo ' class="form_error"'; } ?>>
                                <input autocapitalize="off" class="w_70" id="security_cd" inputmode="numeric" maxlength="4" name="security_cd" data-pattern="^\d+$" placeholder="例）9999" type="<?php echo $inputTypeNumber; ?>" value="<?php echo $eveOutForm->security_cd(); ?>" />
                    ...
                    <div class="btn_area">
                        ...
                        if(empty($comiketId)): ?>
                            <a class="back" href="/mdr/input/?clr=1">修正する</a>
                        <?php else: ?>
                            <a class="back" href="/mdr/input2/?clr=1">修正する</a>
                        <?php endif; ?>
...
?>
    ...
    <script charset="UTF-8" type="text/javascript" src="/mdr/js/credit_card.js"></script>
    <script>
        $(function() {
