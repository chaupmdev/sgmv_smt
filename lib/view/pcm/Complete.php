...
Sgmov_Lib::useServices(array('CorporativeMoving', 'CenterMail'));
Sgmov_Lib::useForms(array('Error', 'PcmSession', 'Pcm001In', 'Pcm003Out'));
/**#@-*/

 /**
 * 法人引越輸送情報を登録し、完了画面を表示します。
 * @package    View
 * @subpackage PCM
 * @author     K.Hamada(SCS)
 * @copyright  2009-2010 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Pcm_Complete extends Sgmov_View_Pcm_Common
{
    ...
    /**
     * 法人引越輸送サービス
     * @var Sgmov_Service_Corporativemoving
     */
    public $_corporativeMovingService;
    ...
    public function __construct()
    {
        ...
        $this->_corporativeMovingService = new Sgmov_Service_CorporativeMoving();
    ...
    public function executeInner()
    {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_PCM002, $this->_getTicket());
        ...
        // 情報をDBへ格納
        $this->_corporativeMovingService->insert($db, $data);

        // メール送信用データを作成
        $mailData = $this->_createMailDataFromInForm($db, $sessionForm->in);

        // 担当者通知メール送信
        $this->_centerMailService->_sendAdminMail($db, Sgmov_Service_CenterMail::FORM_KBN_PCM, $sessionForm->in->pref_cd_sel, $mailData, '/pcm_admin.txt');

        // サンキューメール送信
        if (!empty($sessionForm->in->mail)) {
        	$this->_centerMailService->_sendThankYouMail('/pcm_user.txt', $sessionForm->in->mail, $mailData);
        }

        // 出力情報を設定
        $outForm = new Sgmov_Form_Pcm003Out();
