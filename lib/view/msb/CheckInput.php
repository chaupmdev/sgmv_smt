...
Sgmov_Lib::useView('msb/Common');
Sgmov_Lib::useForms(array('Error', 'MsbSession', 'Msb001In', 'Msb002In'));
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Msb_CheckInput extends Sgmov_View_Msb_Common {
    ...
    public function executeInner() {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_MSB001, $this->_getTicket());

        // 入力チェック
        $sessionForm = $session->loadForm(self::FEATURE_ID);

        if (empty($sessionForm)) {
            $sessionForm = new Sgmov_Form_MsbSession();
            $sessionForm->in = null;
        }
    ...
    public function _redirectProc($inForm, $errorForm) {
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/msb/input');
        }
        
        // 個人の場合は、クレジット・コンビニ支払で表示画面切り替え
        switch ($inForm->comiket_payment_method_cd_sel) {
            case '1': // コンビニ
                Sgmov_Component_Redirect::redirectPublicSsl('/msb/confirm');
                break;
            case '2': // クレジット
                Sgmov_Component_Redirect::redirectPublicSsl('/msb/credit_card');
                break;
            case '3': // 電子マネー
                Sgmov_Component_Redirect::redirectPublicSsl('/msb/confirm');
                break;
    ...
    public function _createInFormFromPost($post, $creditCardForm) {
        ...
        $inForm = new Sgmov_Form_Msb002In();
        ...
        /** コミケットボックス **/
        $inForm->comiket_box_num_ary = filter_input(INPUT_POST, 'comiket_box_num_ary');
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $this->checkAzuke($inForm, $errorForm);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function checkAzuke($inForm, &$errorForm) {
        $db = Sgmov_Component_DB::getPublic();
        ...
        // 往復選択
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_azukari_kaisu_type_sel)->isSelected()->isIn(array_keys($this->comiket_detail_azukari_kaisu_type_lbls));
        if (!$v->isValid()) {
            $errorForm->addError('comiket_detail_azukari_kaisu_type_sel', '取出回数' . $v->getResultMessageTop());
        }
        ...
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_name)->isNotEmpty()->isLengthLessThanOrEqualTo(32)->
                isNotHalfWidthKana()->isWebSystemNg();
        if (!$v->isValid()) {
            $errorForm->addError('comiket_detail_name', 'お届け先名' . $v->getResultMessageTop());
        }

        // 集荷先郵便番号
        // 郵便番号(最後に存在確認をするので別名でバリデータを作成) 必須チェック
        $zipV = Sgmov_Component_Validator::createZipValidator($inForm->comiket_detail_zip1, $inForm->comiket_detail_zip2)->isNotEmpty()->isZipCode();
        if (!$zipV->isValid()) {
            $errorForm->addError('comiket_detail_zip', 'お届け先郵便番号' . $zipV->getResultMessageTop());
        }

        // 集荷先都道府県
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_pref_cd_sel)->isSelected()->isIn($prefectures['ids']);
        if (!$v->isValid()) {
            $errorForm->addError('comiket_detail_pref', 'お届け先都道府県' . $v->getResultMessageTop());
        }

        // 集荷先市区町村 必須チェック 40文字チェック WEBシステムNG文字チェック
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_address)->isNotEmpty()->isLengthLessThanOrEqualTo(14)->
                isNotHalfWidthKana()->isWebSystemNg();
        if (!$v->isValid()) {
            $errorForm->addError('comiket_detail_address', 'お届け先市区町村' . $v->getResultMessageTop());
        } else {
            $prefName = @$prefectures["names"][$prefectures["ids"][$inForm->comiket_detail_pref_cd_sel]];
            if (strpos($inForm->comiket_detail_address, $prefName) !== false) {
                $errorForm->addError('comiket_detail_address', 'お届け先市区町村には都道府県名は入力しないで下さい。');
            }
        }

        // 集荷先番地・建物名・部屋番号 必須チェック 40文字チェック WEBシステムNG文字チェック
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_building)->isNotEmpty()->isLengthLessThanOrEqualTo(30)->
                isNotHalfWidthKana()->isWebSystemNg();
        if (!$v->isValid()) {
            $errorForm->addError('comiket_detail_building', 'お届け先番地・建物名・部屋番号' . $v->getResultMessageTop());
        } else {
            $prefName = @$prefectures["names"][$prefectures["ids"][$inForm->comiket_detail_pref_cd_sel]];
            if (strpos($inForm->comiket_detail_building, $prefName) !== false) {
                $errorForm->addError('comiket_detail_building', 'お届け先番地・建物名・部屋番号には都道府県名は入力しないで下さい。');
            }
        }
        ...
        //GiapLN imp ticket #SMT6-381 2022/12/29
        $v = Sgmov_Component_Validator::createSingleValueValidator($comiektDetailTel)->isNotEmpty()->isPhoneHyphen()->isLengthLessThanOrEqualToForPhone();
        if (!$v->isValid()) {
            $errorForm->addError('comiket_detail_tel', 'お届け先TEL' . $v->getResultMessageTop());
        } else {
            ...
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_tel', 'お届け先TELの数値部分' . $v->getResultMessageTop());
            }
        }
