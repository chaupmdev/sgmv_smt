...
Sgmov_Lib::useView('csc/Edit');
Sgmov_Lib::useForms(array('Error', 'EveSession'));

// // 処理を実行
$view = new Sgmov_View_Csc_Edit();
...
// $startDt = date('Y-m-d', strtotime($shohin['start_date']));
// $endDt = date('Y-m-d', strtotime($shohin['end_date']));
$startDt = '';
$endDt = '';

if (!empty($shohin['start_date'])) {
    $startDtArr = explode(' ', $shohin['start_date']);
    $startDt = $startDtArr[0];
}
if (!empty($shohin['end_date'])) {
    $endDtArr = explode(' ', $shohin['end_date']);
    $endDt = $endDtArr[0];
}
    ...
    <div id="main">
        <div class="wrap clearfix">
            ...
            <div class="section other">
                <form id="form_edit" action="/csc/edit?id=<?= $id ?>" data-feature-id="CSC" data-id="CSC001" method="post">
                    <div class="dl_block comiket_block">
                        ...
                        <dl>
                            ...
                            <dd class="shohin_cd_err_apply">
                                <input 
                                    id="shohin_cd" class="shohin_cd" maxlength='30' inputmode="" name="shohin_cd" data-pattern="" type="text" 
                                    style="width: 40%;" 
                                    value="<?= @_h($shohin['shohin_cd']) ?>">
                            </dd>
                        </dl>
                        <!-- END 商品コード -->
                        ...
                        <dl>
                            ...
                            <dd class="shohin_name_err_apply">
                                <input id="shohin_name" class="shohin_name" maxlength='100' inputmode="" name="shohin_name" data-pattern="" type="text" style="width: 80%;" value="<?= @_h($shohin['shohin_name']) ?>">
                            </dd>
                        </dl>
                        <!-- END 商品名 -->
                        ...
                        <dl>
                            ...
                            <dd class="size_err_apply">
                                <input id="size" class="size" inputmode="" name="size" data-pattern="" type="text" style="width: 15%;" value="<?= @_h($shohin['size']) ?>">
                            </dd>
                        </dl>
                        <!-- END サイズ -->
                            ...
                            <dd class="option_id_err_apply">
                                <select class='option_id' name="option_id">
                                    <?php foreach ($costcoOptions as $key => $val) : ?>
                                        <option value="<?= @$key ?>" <?= @$key == @$shohin['option_id'] ? ' selected' : ''; ?>>
                                            <?= @$val ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <dd class="data_type_err_apply">
                                <select class='data_type' name="data_type">
                                    <?php foreach ($dataTypeOptions as $key => $val) : ?>
                                        <option value="<?= @$key ?>" <?= @$key == @$shohin['data_type'] ? ' selected' : ''; ?>>
                                            <?= @$val ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <dl>
                            ...
                            <dd class="juryo_err_apply">
                                <input id="juryo" class="juryo" inputmode="" name="juryo" data-pattern="" type="number" style="width: 15%;" value="<?= @_h($shohin['juryo']) ?>">
                            </dd>
                        </dl>
                        <!-- END 重量 -->
                        ...
                        <dl>
                            ...
                            <dd class="konposu_err_apply">
                                <input id="konposu" class="konposu" inputmode="" name="konposu" data-pattern="" type="number" style="width: 15%;" value="<?= @_h($shohin['konposu']) ?>">
                            </dd>
                        </dl>
                        <!-- END 梱包数 -->
                    </div>
    ...
    <script>
        ...
        $(function() {
            ...
            $('#back_button').on('click', function() {
                console.log(baseUrl + '/csc/mst_shohin_list');
