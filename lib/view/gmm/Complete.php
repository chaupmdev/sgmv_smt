...
Sgmov_Lib::useView('gmm/Common', 'CommonConst');
Sgmov_Lib::useView('gmm/Bcm');
 ...
 */
class Sgmov_View_Gmm_Complete extends Sgmov_View_Gmm_Common {
    ...
    public function __construct() {
        ...
        $this->_BcmView             = new Sgmov_View_Gmm_Bcm();
    ...
    public function executeInner($argBackInputPaht="") {
        ...
        //チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_GMM003, $this->_getTicket());
     ...
     */
    protected function checkHannyuArrivalDate($db, $inForm, $eveSubData, $session){
        // 各地域ごとの締切日チェック
        ...
        if ($dateChArrival->format('Y-m-d H:i:s') <= $dateChNow->format('Y-m-d H:i:s')) {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error/?t=受付時間が終了しました&m=お預かり日：{$dispDate}の受付時間が終了しました");
            exit;
        }
        ...
        if ($dateColl <= $dateNow) {
            // セッション情報を破棄
            $session->deleteForm(self::FEATURE_ID);
            Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error/?t=受付時間が終了しました&m=お預かり日：{$yearColl}/{$monthColl}/{$dayColl}の受付時間が終了しました");
            exit;
        }
    }
    ...
    protected function registerComiket($db, $inForm, $comiketId, $backInputPath){
        ...
        try {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/gmm/{$backInputPath}/");
            exit;
        }
        $db->commit();
    ...
    protected function sendDataToGyomu($db, $inForm, $comiketId, $calcDataInfoData, $backInputPath, $sessionForm, $session, $toiawaseNo){
        ...
        if (@$liTreeData["treeData"]['payment_method_cd'] == self::PAYMENT_METHOD_CONVINI_AFTER) {
            ...
            if ($resultData['result'] == 'OK' && @$bcmResult['sendSts'] == '0') {
                ...
                $param = array('res_sgf_transactionId' => @$resultData["transactionInfo"]["transactionId"]);
                $mail_to = Sgmov_Component_Config::getLogMailTo();
                Sgmov_Component_Mail::sendTemplateMail($param, dirname(__FILE__) . '/../../mail_template/gmm_error_for_sgfinancial_shipment_report.txt', $mail_to);
                Sgmov_Component_Log::info('SgFinancial-ShipmentReport-NG');
            }
        }
...
Sgmov_Component_Log::debug($pay_limit);
     ...
     * @param Sgmov_Form_Eve001-003In $inForm 入力フォーム
     * @param $type 往復
     * @return array 決済用データ
     */
