 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Gmm_Input extends Sgmov_View_Gmm_Common {
    ...
    // 2022-03-24 ToanDD3 implementn SMT6-84
    ...
    public function executeInner() {
        ...
        if (!isset($_SESSION[self::LOGIN_ID]['user_type'])) {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/event/userSelect?event_nm=gmm");
        }
        
        if(@empty($_SERVER["HTTP_REFERER"]) || strpos($_SERVER["HTTP_REFERER"], "/gmm/") == false) {
            // セッション情報を破棄
...
///////////////////////////////////////////////////////////////////////////////////////////////////
        if(strlen($param) == 10) {
            Sgmov_Component_Redirect::redirectPublicSsl("/gmm/input2_dialog/{$param}");
        }
        ...
        if($input2DialogFlg == "1") {
            ...
            if(!is_numeric($input2DialogComiketId)){
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/temp_error");
                exit;
            }
            ...
            if($comiketData['tel'] == $input2DialogData1Tel
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/input/{$param}");
            } else {
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/temp_error/{$comiketData['event_id']}");
            }
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(@!empty($_SERVER["REQUEST_URI"]) && strpos($_SERVER["REQUEST_URI"], "/gmm/input2") !== false && empty($param)) {
        ...
        if (isset($eventsubInfo)) {
            if (isset($eventsubInfo['departure_fr_time'])) {
                ...
                if ($now<$departureFrTo) {
                    ...
                    $departureFrDate = date('Y/m/d', strtotime($departureFrTime));
                    $week = array("日", "月", "火", "水", "木", "金", "土");
                    $departureFrWeek = $week[date('w', strtotime($departureFrDate))];
                    $message = urlencode("「{$eventName}{$eventsubInfo['name']}」のお申込は 「{$departureFrDate}（{$departureFrWeek}）」 から受付を開始致します。");
                    Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error?t={$title}&m={$message}");
                    exit;
                }
...
//        }
        ...
        // チケット発行
        $ticket = $session->publishTicket(self::FEATURE_ID, self::GAMEN_ID_GMM001);
