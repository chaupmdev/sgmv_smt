...
Sgmov_Lib::useView('sso/CheckInput');
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Sso_SizeChangeCheckInput extends Sgmov_View_Sso_CheckInput {
    ...
    public function _redirectProc($inForm, $errorForm) {
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/sso/size_change');
        }
        ...
        if (@empty($boxNumTotal)) {
            Sgmov_Component_Redirect::redirectPublicSsl('/sso/size_change_confirm');
        }
        ...
        switch ($inForm->comiket_payment_method_cd_sel) {
            case '1': // コンビニ
                ...
                break;
            case '2': // クレジット
                Sgmov_Component_Redirect::redirectPublicSsl('/sso/size_change_credit_card');
                break;
            default:
                Sgmov_Component_Redirect::redirectPublicSsl('/sso/size_change_confirm');
                break;
...
Sgmov_Component_Log::debug("###################################### 501-1");
            ...
            if (@!empty($boxNumTotal)) {
                ...
                if(!$errorForm->hasError()) {
                    ...
                    if (@!empty($comiketInfo) && $comiketInfo['amount_tax'] < $inForm->delivery_charge) { // 金額が前回の時より多かった場合はエラー
                        ...
                        $errorForm->addError("comiket_box_outbound_num_ary", "搬入-合計金額が元の金額より大きくなっています。（元の金額：￥{$amountTaxOld}／今回の金額￥{$amountTaxNew}）");
                    }
                }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if($inForm->comiket_detail_type_sel == "2") { // 復路
            ...
            if (@!empty($boxNumTotal)) {
                ...
                if(!$errorForm->hasError()) {
                    ...
                    if ($totalNumDb < $totalNumDisp) {
                        $errorForm->addError("comiket_box_inbound_num_ary", "宅配数量合計が元の数量合計より大きくなっています。（元の数量合計：{$totalNumDb}個）");
                    }
                }
                ...
                if(!$errorForm->hasError()) {
                    ...
                    if (@!empty($comiketInfo) && $comiketInfo['amount_tax'] < $inForm->delivery_charge) { // 金額が前回の時より多かった場合はエラー
                        $amountTaxOld = number_format($comiketInfo['amount_tax']);
                        $amountTaxNew = number_format($inForm->delivery_charge);
                        $errorForm->addError("comiket_box_inbound_num_ary", "合計金額が元の金額より大きくなっています。（元の金額：￥{$amountTaxOld}／今回の金額￥{$amountTaxNew}）");
                    }
                }
    ...
    public function _createInFormFromPost($post, $creditCardForm) {
        ...
        $inForm->comiket_detail_inbound_binshu_kbn_sel = @$_POST['comiket_detail_inbound_binshu_kbn_sel'];
        $inForm->comiket_detail_outbound_binshu_kbn_sel = @$_POST['comiket_detail_outbound_binshu_kbn_sel'];
