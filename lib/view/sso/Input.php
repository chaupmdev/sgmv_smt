 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Sso_Input extends Sgmov_View_Sso_Common {
    ...
    public function executeInner() {
        ...
        if (!isset($_SESSION[self::LOGIN_ID]['user_type'])) {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl('/event/userSelect?event_nm=sso');
        }
//        
        if(@empty($_SERVER["HTTP_REFERER"]) || strpos($_SERVER["HTTP_REFERER"], "/sso/") == false) {
            // セッション情報を破棄
...
Sgmov_Component_Log::debug($sessionForm);
...
//Sgmov_Component_Log::debug($input2DialogComiketId);
            if(!is_numeric($input2DialogComiketId)){
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/temp_error");
                exit;
            }
...
//Sgmov_Component_Log::debug($input2DialogData1Tel);
            ...
            if($comiketData['tel'] == $input2DialogData1Tel
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/input/{$param}");
            } else {
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/temp_error/{$comiketData['event_id']}");
            }
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(@!empty($_SERVER["REQUEST_URI"]) && strpos($_SERVER["REQUEST_URI"], "/sso/input2") !== false && empty($param)) {
...
Sgmov_Component_Log::debug($_SERVER);
        if(@!empty($_SERVER["HTTP_REFERER"]) && strpos($_SERVER["HTTP_REFERER"], "/sso/") !== false ){
Sgmov_Component_Log::debug("########################### 649-5-1");
            // 初期表示時以外(入力エラーなどで戻ってきた場合など)
            if (isset($sessionForm)) {
Sgmov_Component_Log::debug("########################### 649-5-2");
...
//        $ev = $inForm->event_sel;
//        if(@empty($inForm->event_sel) || !is_numeric($inForm->event_sel)) {
//            $inForm->event_sel = Sgmov_View_Sso_Common::$eventInfo[0]; // シーフードショー
//            $inForm->input_mode = $inForm->event_sel;
//            $inForm->eventsub_sel = Sgmov_View_Sso_Common::$eventsubInfo[0]; // シーフードショー
//        }
...
//                $inForm->eventsub_sel = '500';
//            }
////            $inForm->eventsub_sel = "104";
//        }

        $db = Sgmov_Component_DB::getPublic();
        $eventsubInfo =  $this->_EventsubService->getEventIdByShikibetsushi($db, strtolower(self::EVENT_NM));
...
Sgmov_Component_Log::debug("########################### 649-5-3");
...
//                    $eventName = //$eventInfo['name'];
                    $eventName = "シーフードショー大阪・アグリフードＥＸＰＯ大阪";
...
//        }
        ...
        if (isset($eventsubInfo)) {
            $now2 = date('Y-m-d');
            if (@!empty($eventsubInfo['departure_to']) && $eventsubInfo['departure_to'] < $now2
...
//        $outForm->raw_eventsub_cd_sel = '';
    ...
    protected function _createOutFormByInForm($inForm, $param=NULL) {
        $inForm = (array)$inForm;
        // 初期値を設定しておく
        $inForm['comiket_div'] = '1'; // 個人のみ
//        $inForm['comiket_detail_type_sel'] = '2'; // 復路のみ
