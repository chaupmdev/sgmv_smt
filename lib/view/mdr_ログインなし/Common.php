...
///////////////////////////////////////////////////////////////////////////////////
    ...
    protected function createOutFormByInForm($inForm, $outForm = array()) {
        ...
        $eventAll2 = array();
        $eventIds = array();
        $eventNames = array(); // REWRITE
        $eventNames2 = array();

        $week = array("日", "月", "火", "水", "木", "金", "土");
        foreach($eventAll as $val) {
            $eventIds[] = $val["id"];
            $eventNames[] = $val["name"]; // REWRITE
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/mdr/complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/mdr/complete_business_sgmv{$tmplateType}.txt";
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/mdr/complete_individual{$tmplateType}.txt";

                $mailTemplateSgmv[] = "/mdr/complete_individual_sgmv{$tmplateType}.txt";
            ...
            foreach ($comiket_detail_list as $comiket_detail) {
                ...
                //搬出用メールテンプレート
                $mailTemplate[] = '/mdr/parts_complete_choice_2.txt';

                $mailTemplateSgmv[] = '/mdr/parts_complete_choice_2_sgmv.txt';
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        try {
            ...
            $mailTemplateList = array(
                "/mdr/cancel_{$divType}_error.txt",
//                "/mdr/parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
            );
