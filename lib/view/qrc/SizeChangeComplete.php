...
Sgmov_Lib::useServices(array(
    'Comiket', 'ComiketDetail', 'ComiketBox', 'ComiketCargo', 'ComiketCharter', 'CenterMail', 'SgFinancial', 'HttpsZipCodeDll', 'CargoFare', 'BoxFare', 'Charter'
));
 ...
 */
class Sgmov_View_Qrc_SizeChangeComplete extends Sgmov_View_Qrc_Complete
{
    ...
    public function executeInner()
    {
        ...
        if (@empty($boxNumTotal)) {
            ...
            $parentRes = array(
                ...
                'collect_date' => @date('Y年n月j日', strtotime($comiketDetailInfo['collect_date'])),
                'type_sel' => $comiketDetailInfo['type'],
        ...
        if ($comiketInfo['payment_method_cd'] != '1') {
            $renkeiFlg = $this->execWebApiCancelComiket(
                $this->_wsProtocol,
                $this->_wsHost,
                $this->_wsPath,
                $this->_wsPort,
                ...
                $paramOrg,
                '個数・サイズ変更完了しました',
                '個数・サイズ変更完了しました。個数・サイズ変更完了メール送信してますので、ご確認ください。'
            );
        }
