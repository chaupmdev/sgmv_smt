...
Sgmov_Lib::useView('htl/Common', 'CommonConst');
Sgmov_Lib::useView('htl/Bcm');
                        ...
                        if ($resultData["result"] == "OK") {
                            if ($resultData["transactionInfo"]['autoAuthoriresult'] == "審査中"
                                ...
                                if($cancelResArr["result"] == "OK") {
                                    ...
                                    throw new Exception("THERE WAS A FUCKING EXCEPTION! PUT A FUCKING ERROR MESSAGE OR IT IS USELESS!");
                                }
                                throw new Exception("THERE WAS A FUCKING EXCEPTION! PUT A FUCKING ERROR MESSAGE OR IT IS USELESS!");
                            }
                        } else {
...
Sgmov_Component_Log::debug("################# 405-5");
            ...
            Sgmov_Component_Log::debug("リダイレクト /htl/{$backInputPaht}/");
            Sgmov_Component_Log::err('データベースの登録・更新に失敗しました。');
            Sgmov_Component_Log::err($e);
            Sgmov_Component_Redirect::redirectPublicSsl("/htl/{$backInputPaht}/");
            exit;
        }
        $db->commit();
