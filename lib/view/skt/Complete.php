...
Sgmov_Lib::useServices(array('Comiket', 'ComiketDetail', 'ComiketBox', 'ComiketCargo', 'ComiketCharter'
    , 'CenterMail', 'SgFinancial', 'HttpsZipCodeDll', 'CargoFare', 'BoxFare', 'Charter', 'OutBoundCollectCal', 'Eventsub', 'ComiketKanren', 'GyomuApi'));
 ...
 */
class Sgmov_View_Evp_Complete extends Sgmov_View_Evp_Common {
    ...
    public function __construct() {
        ...
        $this->_BcmView             = new Sgmov_View_Evp_Bcm();
    ...
    public function executeInner($argBackInputPaht="") {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 搬入
            ...
            // 各地域ごとの締切日チェック
            /////////////////////////////////////////////////////////////////////////////////////////////
            $this->checkHannyuArrivalDate($db, $inForm, $eveSubData);
        }
        ...
        // 業務連携
        $returnResult = $this->sendDataToGyomu($db, $registerData, $comiketId, $calcDataInfoData, $backInputPath, $sessionForm, $session);
        ...
        return array(
            ...
            'collect_date' => $inForm['comiket_detail_outbound_collect_date_year_sel'].'年'.$inForm['comiket_detail_outbound_collect_date_month_sel'].'月'.$inForm['comiket_detail_outbound_collect_date_day_sel'].'日',
     ...
     */
    protected function checkHannyuArrivalDate($db, $inForm, $eveSubData){
        // 各地域ごとの締切日チェック
     ...
     */
    protected function registerComiket($db, $inForm, $comiketId, $backInputPath){
        $db->begin();
        try {
            ...
            //サイズ変更の場合、ブース情報が変わらないように設定する
            if (empty($calcDataInfoData["flatData"]["comiketData"]["building_name"])) {
                $calcDataInfoData["flatData"]["comiketData"]["building_name"] = $inForm['building_booth_position'].',' . $inForm['comiket_booth_name'];
            }
            $calcDataInfo = $calcDataInfoData["treeData"];
            $comiketDataInfoFlat = $calcDataInfoData["flatData"];

            // sktカスタム：ブース番号エリアコンボボックス：「,」で分割
            // I_04,Imaginary Fo
            $shopList=explode(',',$comiketDataInfoFlat["comiketData"]["building_name"]);
            // sktカスタム：館名は店名
            $comiketDataInfoFlat["comiketData"]["booth_name"]=mb_convert_kana(mb_substr($shopList[1],0,16), 'RNASKV', 'UTF-8');
            // sktカスタム：ブース名は持っていないのでカラ
            $comiketDataInfoFlat["comiketData"]["building_name"]= NULL;
            // sktカスタム：ブース位置
            $comiketDataInfoFlat["comiketData"]["booth_position"]=$shopList[0];

//              // sktカスタム：ブース番号お店名：16文字に切り出す

//            // sktカスタム：ブース番号お店名：16文字に切り出す
//            $comiketDataInfoFlat["comiketData"]["building_name"]=mb_substr(str_replace(",","",$comiketDataInfoFlat["comiketData"]["building_name"]),0,16);
//            // sktカスタム：ブース番号お店名：16文字に切り出す
//            $comiketDataInfoFlat["comiketData"]["booth_name"]=mb_substr($comiketDataInfoFlat["comiketData"]["booth_name"],0,16);
            // sktカスタム：搬出申込はすべて法人
            if($comiketDataInfoFlat["comiketData"]["choice"]==2){
                $comiketDataInfoFlat["comiketData"]["div"]=self::COMIKET_DEV_BUSINESS;
            }

            // sktカスタム：法人名はすべてＳＧムービング
            $comiketDataInfoFlat["comiketData"]["office_name"]='ＳＧムービング';
            ...
            Sgmov_Component_Log::debug("リダイレクト /gmm/{$backInputPath}/");
        ...
        if (@$liTreeData["treeData"]['payment_method_cd'] == self::PAYMENT_METHOD_CONVINI_AFTER) {
            ...
            if ($resultData['result'] == 'OK' && @$bcmResult['sendSts'] == '0') {
                ...
                if ($shipmentCompResArr['result'] == 'OK') {
                    ...
                    Sgmov_Component_Mail::sendTemplateMail($param, dirname(__FILE__) . '/../../mail_template/Evp_error_for_sgfinancial_shipment_report.txt', $mail_to);
    ...
    public function errorInformationForSgFinancial($param) {
        ...
        $mail_to = Sgmov_Component_Config::getLogMailTo();
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($param, dirname(__FILE__) . '/../../mail_template/Evp_error_for_sgfinancial.txt', $mail_to);
    }
...
//        $inForm['delivery_charge'] = "2550";
        ...
        // バシ　変更　2020/03/12 {
        //$inForm['payment_order_id'] = 'sagawa-moving-event_' . $date->format('YmdHis') . '_' . str_shuffle('0123456789ABCDEFGHIJ123KLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz');
...
*/
    ...
    public function errorInformation($parm = array())
    {
        ...
        $mail_to = Sgmov_Component_Config::getLogMailTo();
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($parm, dirname(__FILE__) . '/../../mail_template/Evp_error.txt', $mail_to);
    }
