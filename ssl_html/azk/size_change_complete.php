...
Sgmov_Lib::useView('azk/SizeChangeComplete');
Sgmov_Lib::useForms(array('Error', 'EveSession'));
/**#@-*/

$exceptionFlg = FALSE;
// 処理を実行
$view = new Sgmov_View_Azk_SizeChangeComplete();
$eveOutForm = array();
$eventData = array();
$eventsubData = array();
...
<head>
    ...
    <?php if (@empty($isCancelOnly)) : ?>
        ...
        <!-- 個数・サイズ変更 の場合表示 -->
        <!-- ********************************************************************************************** -->
        <title>催事・イベント手荷物預かりサービスの個数・サイズ変更お申し込みの完了│ＳＧムービング株式会社＜ＳＧホールディングスグループ＞</title>
    <?php else: ?>
        <!-- ********************************************************************************************** -->
        <!-- キャンセルのみの場合表示 -->
        <!-- ********************************************************************************************** -->
        <title>催事・イベント手荷物預かりサービスのキャンセルお申し込みの完了│ＳＧムービング株式会社＜ＳＧホールディングスグループ＞</title>
    <?php endif; ?>
    ...
    <div id="breadcrumb">
        <ul class="wrap">
            ...
            <?php if (@empty($isCancelOnly)) : ?>
                ...
                <!-- 個数・サイズ変更 の場合表示 -->
                <!-- ********************************************************************************************** -->
                <li class="current">催事・イベント手荷物預かりサービスの個数・サイズ変更お申し込みの完了</li>
            <?php else: ?>
                <!-- ********************************************************************************************** -->
                <!-- キャンセルのみの場合表示 -->
                <!-- ********************************************************************************************** -->
                <li class="current">催事・イベント手荷物預かりサービスのキャンセル処理完了</li>
            <?php endif; ?>
        </ul>
    </div>
    ...
    <div id="main">
        <div class="wrap clearfix">
            <?php if (@empty($isCancelOnly)) : ?>
                ...
                <!-- 個数・サイズ変更 の場合表示 -->
                <!-- ********************************************************************************************** -->
                <h1 class="page_title">催事・イベント手荷物預かりサービスの個数・サイズ変更お申し込みの完了</h1>
            <?php else: ?>
                <!-- ********************************************************************************************** -->
                <!-- キャンセルのみの場合表示 -->
                <!-- ********************************************************************************************** -->
                <h1 class="page_title">催事・イベント手荷物預かりサービスのキャンセル処理完了</h1>
            <?php endif; ?>
...
?>
        ...
        <?php if($paymentMethodCd === '3') : ?>
            ...
            ・ イベント当日、受付にて電子マネーでの決済をお願いします。<br/>
        <?php endif; ?>
<?php
    } else {
?>
