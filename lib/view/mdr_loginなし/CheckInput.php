 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Mdr_CheckInput extends Sgmov_View_Mdr_Common {
    ...
    public function executeInner() {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_MDR001, $this->_getTicket());
    ...
    public function _redirectProc($inForm, $errorForm) {
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/mdr/input');
        } else if($inForm->comiket_div == self::COMIKET_DEV_BUSINESS) {  // 法人
            Sgmov_Component_Redirect::redirectPublicSsl('/mdr/confirm');
        }
        ...
        switch ($inForm->comiket_payment_method_cd_sel) {
            case '1': // コンビニ
                ...
                break;
            case '2': // クレジット
                Sgmov_Component_Redirect::redirectPublicSsl('/mdr/credit_card');
                break;
            case '3': // 電子マネー
                Sgmov_Component_Redirect::redirectPublicSsl('/mdr/confirm');
                break;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function _validate($inForm, $db) {
        ...
        if (!$v->isValid()) {
            $errorForm->addError('comiket_detail_type_sel', '用途選択' . $v->getResultMessageTop());
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     * @param string $errMsgOutInbount
     * @param boolean $isEmptyCheck
     */
    protected function checkComiketBoxOutInboundNumAry($inForm, &$errorForm, $targetList, $targetClassName, $errMsgOutInbount, $isEmptyCheck = true, $fromSizeChange = false) {
        ...
        foreach($targetList as $key => $val) {
            ...
            // サイズ変更画面にて、「0」を入力した時、キャンセル画面に遷移する            
