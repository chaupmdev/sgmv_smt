...
<?php
        echo Sgmov_View_Evp_Input::_createPulldown($eve001Out->comiket_detail_outbound_pref_cds(), $eve001Out->comiket_detail_outbound_pref_lbls(), $eve001Out->comiket_detail_outbound_pref_cd_sel());
?>
                                    </select>
                                    <br/>
                                    <br/>
                            ...
                            <dl>
                                ...
                                <dd<?php if (isset($e) && $e->hasErrorForId('comiket_detail_outbound_delivery_date')) { echo ' class="form_error"'; } ?>>
                                    ...
                                    <input type="hidden" id="hid_comiket-detail-outbound_delivery-date-from"
                                      name="hid_comiket-detail-outbound_delivery-date-from"
                                      value="<?php echo @$dispItemInfo["eventsub_selected_data"]["outbound_delivery_fr_dt"]; ?>" />
                                    <input type="hidden" id="hid_comiket-detail-outbound_delivery-date-to"
                                      name="hid_comiket-detail-outbound_delivery-date-to"
                                      value="<?php echo @$dispItemInfo["eventsub_selected_data"]["outbound_delivery_to_dt"]; ?>" />
                                    <div class="comiket_detail_outbound_delivery_date_parts" style="clear: both;">

<?php if(@$dispItemInfo["eventsub_selected_data"]["outbound_delivery_fr_dt"] == @$dispItemInfo["eventsub_selected_data"]["outbound_delivery_to_dt"]): ?>
<?php
// 引き渡し日時1日の場合(from～toが同じ)：日付をhiddenでもたせる
// 時間帯
foreach(@$eve001Out->comiket_detail_outbound_delivery_time_cds() as $v){
    $hikiwatashiJikantai=$v;
    break;
}

// 引き渡し日
$hikiwatashiDay=explode('-',@$dispItemInfo["eventsub_selected_data"]["outbound_delivery_fr_dt"]);
?>
    <?php //javascript:dispOutboundColAndDelyDate()メソッドで表示制御している ?>
                                    <input type="hidden" id="comiket_detail_outbound_delivery_date_year_sel"
                                     name="comiket_detail_outbound_delivery_date_year_sel" 
                                     value="<?php echo @$hikiwatashiDay[0]; ?>" />

                                    <input type="hidden" id="comiket_detail_outbound_delivery_date_month_sel"
                                     name="comiket_detail_outbound_delivery_date_month_sel" 
                                     value="<?php echo @$hikiwatashiDay[1]; ?>" />

                                    <input type="hidden" id="comiket_detail_outbound_delivery_date_day_sel"
                                     name="comiket_detail_outbound_delivery_date_day_sel" 
                                     value="<?php echo @$hikiwatashiDay[2]; ?>" />

                                    <input type="hidden"
                                     name="comiket_detail_outbound_delivery_time_sel" 
                                     value="<?php echo @$hikiwatashiJikantai; ?>" />
                                    <p>
                                        <strong class="red" style="white-space:nowrap;">
                                        ・ブースへお届けいたします
                                        </strong>
                                    </p>

<?php else: // 引き渡し日時が複数日にまたがっている(from～toが違う：日付をコンボボックスでもたせる ?>
    <?php //javascript:dispOutboundColAndDelyDate()メソッドで表示制御している ?>
                                    <p class="comiket-detail-outbound-delivery-date-fr-to" style="float: left;padding: 0px;">
                                      <span class="comiket-detail-outbound-delivery-date-from">
                                      <?php echo @$dispItemInfo["eventsub_selected_data"]["outbound_delivery_fr"]; ?>
                                      </span>
                                      から
                                      <span class="comiket-detail-outbound-delivery-date-to">
                                      <?php echo @$dispItemInfo["eventsub_selected_data"]["outbound_delivery_to"]; ?>
                                      </span>
                                      まで選択できます。
                                    </p>
                                    <br><br>
...
<?php
        echo Sgmov_View_Evp_Input::_createPulldown(
              $eve001Out->comiket_detail_outbound_delivery_time_cds()
            , $eve001Out->comiket_detail_outbound_delivery_time_lbls()
            , $eve001Out->comiket_detail_outbound_delivery_time_sel()
        );
?>
                                            </select>
                                        </span>
                            ...
                            <dl class="class_comiket_detail_outbound_collect_date">
                                ...
                                <dd<?php if (isset($e) && $e->hasErrorForId('comiket_detail_outbound_collect_date')) { echo ' class="form_error"'; } ?> class="comiket_detail_outbound_collect_date_input_part">
                                    <div style="white-space: normal;">
                                        <strong class="red">【選択可能日の範囲について】</strong><br/>
...
<!--
                                        <strong class="red">
                                        ※ 北海道・青森・中国・四国・九州・沖縄の方は4月24日（日）までとなります。
                                        ・1日目(30日)：12月21日～12月26日 北海道、青森、中国、四国、九州、沖縄の方は12月25日まで<br/>
                                        ・2日目(31日)：12月21日～12月27日 北海道、青森、中国、四国、九州、沖縄の方は12月26日まで
                                        </strong><br>
-->
                                        <br>
                                    </div>
                                    ...
                                    <div class="comiket_detail_outbound_collect_date_parts">
                                        ...
                                        時間帯
                                            <select name="comiket_detail_outbound_collect_time_sel">
                                                <option value="">時間帯を選択</option>
    <?php
            echo Sgmov_View_Evp_Input::_createPulldown($eve001Out->comiket_detail_outbound_collect_time_cds(), $eve001Out->comiket_detail_outbound_collect_time_lbls(), $eve001Out->comiket_detail_outbound_collect_time_sel());
    ?>
                                            </select>
                                        </span>
                                    </div>
                                    ...
                                    <div style="white-space: normal;">
                                        ...
                                        <br>
                                        <strong class="red">※ 午後１２時以降のお申込みは、翌日を集荷日にできません。</strong>
                                        <br>
                                        <strong class="red">※ 選択された日時に佐川急便が送り状を持って集荷に参ります。</strong>
                                    </div>
                            ...
                            <dl class="service-outbound-item" service-id="1">
                                ...
                                <dd<?php if (isset($e) && $e->hasErrorForId('comiket_box_outbound_num_ary')) { echo ' class="form_error"'; } ?>>
                                    ...
                                    <div class="outbound_example_boxsize example_boxsize">
                                        ...
                                        <strong class="red">※ １申込み４０個を超える場合は、ＳＧムービング/<?=$dispItemInfo['eventname_all']?>係 03-5857-2462 へお問い合わせください。</strong><br/><br/>
