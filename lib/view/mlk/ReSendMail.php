...
Sgmov_Lib::useServices(array('Comiket', 'ComiketDetail', 'ComiketBox', 'ComiketCargo', 'ComiketCharter','CenterMail'));
...
class Sgmov_View_Eve_ReSendMail extends Sgmov_View_Eve_Common {
    ...
    public function executeInner() {
        ...
        $hachakutenCodeFrom  = $comiketDetailDataList[0]['mlk_hachaku_shikibetu_cd'];
        $hachakutenCodeTo  =   $comiketDetailDataList[1]['mlk_hachaku_shikibetu_cd'];
        ...
        if (empty($hachakutenFromInfo) || empty($hachakutenToInfo)) {
            echo '発着情報が既に削除される為、メール再送信できません。';
            return;
        }
