 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Gmm_CheckInput extends Sgmov_View_Gmm_Common {
    ...
    public function executeInner() {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_GMM001, $this->_getTicket());
        // 入力チェック
    ...
    public function _redirectProc($inForm, $errorForm) {
        ...
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/gmm/input');
                ...
                if(!$errorFlg){
                    $addressResult = $this->_getAddressByZip($inForm->comiket_detail_outbound_zip1.$inForm->comiket_detail_outbound_zip2);
                     if (empty($addressResult)) {
                        ...
                        $isInvalidTownName = strpos($userInputTownName, $systemTownName) !== 0;
                        if ($isInvalidKenName || $isInvalidTownName) {
                            $errorForm->addError('comiket_detail_outbound_zip', '搬入-郵便番号と集荷先住所は合わせてください。');
                        }
                    }
                }
            }
