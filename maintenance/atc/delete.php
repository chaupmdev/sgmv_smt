...
Sgmov_Lib::useView('atc/Delete');
/**#@-*/

// 処理を実行
$view = new Sgmov_View_Atc_Delete();
$forms = $view->execute();
...
/**
 * フォーム
 * @var Sgmov_Form_Atc012Out
 */
$atc012Out = $forms['outForm'];
            ...
            <dl id="globalNav">
                ...
                <dd>
                    <ul>
                        ...
                        <li class="nav02">
<?php if($atc012Out->honsha_user_flag() === '1'){ ?>
...
<?php } ?>
    ...
    <div id="topWrap">
        ...
        <h3 class="register_ttl">ツアー配送料金情報の削除</h3>
...
<?php } ?>
                <ul class="register_item" id="travel_delivery_charge_item">
                    <li id="travel_delivery_charge_item_01">
                        <fieldset>
                            <legend>船名</legend>
                            <?php echo $atc012Out->travel_agency_name() . PHP_EOL; ?>
                        </fieldset>
                    </li>
                    <li id="travel_delivery_charge_item_02">
                        <fieldset>
                            <legend>乗船日名</legend>
                            <?php echo $atc012Out->travel_name() . PHP_EOL; ?>
                        </fieldset>
                    </li>
                    <li id="travel_delivery_charge_item_03">
                        <fieldset>
                            <legend>発着地</legend>
                            <?php echo $atc012Out->travel_terminal_name() . PHP_EOL; ?>
                        </fieldset>
                    </li>
...
<?php
    ...
    $tc_travel_delivery_charge_ids = $atc012Out->travel_delivery_charge_ids();
    $tc_travel_provinces_ids       = $atc012Out->travel_provinces_ids();
    $tc_travel_provinces_names     = $atc012Out->travel_provinces_names();
    $tc_prefecture_names           = $atc012Out->prefecture_names();
    $tc_delivery_chargs            = $atc012Out->delivery_chargs();
    $tc_round_trip_discount        = $atc012Out->round_trip_discount();
    if ($tc_round_trip_discount < 0) {
        $tc_round_trip_discount = 0;
    }
    ...
    for ($i = 0; $i < $count;  ++$i) {
        ...
        if ($tc_travel_provinces_id !== null && isset($tc_delivery_chargs[$tc_travel_provinces_id]) && $tc_delivery_chargs[$tc_travel_provinces_id] !== "") {
            $tc_delivery_charg = number_format($tc_delivery_chargs[$tc_travel_provinces_id]) . '円';
        }

        $tc_round_trip_delivery_charg = null;
        if (!empty($tc_delivery_charg)) {
            $temporary_round_trip_delivery_charg = $tc_delivery_chargs[$tc_travel_provinces_id] * 2 - $tc_round_trip_discount;
            if ($temporary_round_trip_delivery_charg < 0) {
                $temporary_round_trip_delivery_charg = 0;
            }
            $tc_round_trip_delivery_charg = number_format($temporary_round_trip_delivery_charg) . '円';
        }

        $html .= '
                                    <tr>
                                        <th title="' . $tc_prefecture_name . '">' . $tc_travel_provinces_name . '</th>
                                        <td class="number">' . $tc_delivery_charg . '</td>
                                        <td class="number">' . $tc_round_trip_delivery_charg . '</td>
                                    </tr>';
    }
    echo $html;
?>
                ...
                <input id="travel_delivery_charge_id" name="travel_delivery_charge_id" type="hidden" value="<?php echo $atc012Out->travel_delivery_charge_id(); ?>" />
    ...
    <script charset="UTF-8" type="text/javascript" src="/common/js/atcDelete.js"></script>
</body>
</html>