...
Sgmov_Lib::useForms(array('Error', 'QrcSession'));
/**#@-*/


$exceptionFlg = FALSE;
// 処理を実行
$view = new Sgmov_View_Qrc_Complete();
$qrcOutForm = array();
$eventData = array();
$eventsubData = array();
...
try {
    ...
    $qrCodeString = @$qrcOutForm->qr_code_string();
    $merchantResult = false;
    if ($paymentMethodCd != '3') { // 電子マネーではない場合
        if ($typeSel != 1 && $qrcOutForm->merchant_result() == 0 && @!empty($qrCodeString)) {
            $merchantResult = true;
        }
        if ($qrcOutForm->merchant_result() == 0) {
            $merchantResult = true;
        }
    }
...
<!--
                        <table class="default_tbl">
                            <tr>
                                ...
                                <td><?php //echo sprintf('%010d', $qrcOutForm->qr_code_string()); ?></td>
                            </tr>
                        </table>
-->
<!--
...
-->
                ...
                <div style="clear: both;">
                    ...
                    ?>
                        ...
                        ?>
                            <br /><span style="color: red;">※お支払いはお預かり日時の<?php //echo $collectDate;
                                                                            ?>の前日17時までに入金していただきますようお願いいたします。</span><br /><br /><br />
                        <?php endif; ?>
                    <?php endif; ?>
...
<!--
                    <?php if ($eventsubData['manual_display'] == '1') { ?>
                        ...
                        <div style="font-size:20px;">
                            <a class="button_link2" href="/<?= $dirDiv; ?>/pdf/manual/<?php echo (str_replace(' ', '', $eventData['name'] . $eventsubData['name'])); ?>.pdf" target="_blank" style="color:blue;">
...
-->
                    <!-- 貼付票-->
<!--
                    <?php if (($qrcOutForm->comiket_detail_type_sel() == "1" || $qrcOutForm->comiket_detail_type_sel() == "3") // もしくは往復
                        && (
                            ($qrcOutForm->comiket_detail_outbound_service_sel() == "1" || $qrcOutForm->comiket_detail_outbound_service_sel() == "2") //
                        )
                        ...
                        <div style="font-size:20px;">
                            <a href="/<?= $dirDiv ?>/paste_tag/<?php echo sprintf('%010d', $qrcOutForm->qr_code_string()) . Sgmov_View_Qrc_Common::getChkD2($qrcOutForm->qr_code_string()); ?>/" target="_blank" style="color:blue;" download>
...
<!--
                    <?php if (($typeSel == "2") // 2:搬出、3:往復
                        && ($qrcOutForm->comiket_detail_inbound_service_sel() == "1" // 1:宅配、2:カーゴ
                        )
                    ) : ?>
                    ...
                    <b>引き継き宅配をご利用される方は<a href="/<?= $dirDiv ?>/input" target="_self" style="color:blue;font-weight: bold;text-decoration: underline;">こちら</a></b><br><br>
                    ...
                    <script>
                        $(function() {
                            $("#qrcode-output").qrcode({
                                text: "<?php echo $qrcOutForm->qr_code_string(); ?>"
                            });
                        });
                    </script>
-->
