...
Sgmov_Lib::useServices(array('HttpsZipCodeDll', 'Box'));
/**#@-*/
/**
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Sso_CheckInput extends Sgmov_View_Sso_Common {
...
Sgmov_Component_Log::debug("############## 705-1");
...
Sgmov_Component_Log::debug("############## 705-5");
        // リダイレクト処理
...
//Sgmov_Component_Log::debug("########################## 801-5");
        if ($errorForm->hasError()) {
//Sgmov_Component_Log::debug("########################## 801-6");
            Sgmov_Component_Redirect::redirectPublicSsl('/sso/input');
//        } else if($inForm->comiket_div == "1" && $inForm->comiket_customer_cd_sel == "1") {  // 法人 && 顧客コードを使用する
        } else if($inForm->comiket_div == self::COMIKET_DEV_BUSINESS) {  // 法人
//Sgmov_Component_Log::debug("########################## 801-7");
            Sgmov_Component_Redirect::redirectPublicSsl('/sso/confirm');
        }
//Sgmov_Component_Log::debug("########################## 801-8");
        // 個人の場合は、クレジット・コンビニ支払で表示画面切り替え
        switch ($inForm->comiket_payment_method_cd_sel) {
            case '1': // コンビニ
//Sgmov_Component_Log::debug("########################## 801-9");
                Sgmov_Component_Redirect::redirectPublicSsl('/sso/confirm');
                break;
            case '2': // クレジット
...
Sgmov_Component_Log::debug("########################## 1001-1");
        // DB接続
        $db = Sgmov_Component_DB::getPublic();
...
Sgmov_Component_Log::info($_SERVER['HTTP_USER_AGENT']);
        ...
        // 顧客区分:ふるさと祭り対応
        $inForm->comiket_customer_kbn_sel =  filter_input(INPUT_POST, 'comiket_customer_kbn_sel');
...
//        $inForm->comiket_customer_cd_sel = filter_input(INPUT_POST, 'comiket_customer_cd_sel');
...
//        $inForm->booth_name = filter_input(INPUT_POST, 'booth_name');
...
//        }
//        if (!$isSetBuilding) {
//            $inForm->building_name = '';
//            $inForm->building_name_sel = 'ZOTHER';
//        }
...
//        $inForm->building_booth_id_sel = filter_input(INPUT_POST, 'building_booth_id_sel');
//        filter_input
//        $inForm->comiket_booth_num = mb_convert_kana(filter_input(INPUT_POST, 'comiket_booth_num'), 'RNASKV', 'UTF-8');
        $inForm->comiket_booth_num = mb_convert_kana(filter_input(INPUT_POST, 'comiket_booth_num'), 'rnask', 'UTF-8');
...
Sgmov_Component_Log::debug('############# BBBBBB');
//Sgmov_Component_Log::debug($_POST);
Sgmov_Component_Log::debug($inForm->comiket_detail_type_sel);
Sgmov_Component_Log::debug(filter_input(INPUT_POST, 'comiket_detail_type_sel'));
...
//        $inForm->comiket_detail_inbound_note = mb_convert_kana(filter_input(INPUT_POST, 'comiket_detail_inbound_note'), 'RNASKV', 'UTF-8');
        ...
        // 往路-便種区分:シーフードショー対応
        $inForm->comiket_detail_outbound_binshu_kbn_sel = filter_input(INPUT_POST, 'comiket_detail_outbound_binshu_kbn_sel');
        
        // 復路-便種区分:ふるさと祭り対応
        $inForm->comiket_detail_inbound_binshu_kbn_sel = filter_input(INPUT_POST, 'comiket_detail_inbound_binshu_kbn_sel');
Sgmov_Component_Log::debug("########################## 1001-9");
Sgmov_Component_Log::debug($_POST);
Sgmov_Component_Log::debug($inForm);
        
        // 入力情報は保持したいため、ここで イベントサブチェック
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $inForm->comiket_convenience_store_cd_sel = filter_input(INPUT_POST, 'comiket_convenience_store_cd_sel');//$creditCardForm['comiket_convenience_store_cd_sel'];
...
//        $inForm->delivery_charge = @$creditCardForm['delivery_charge'];
//        $inForm->repeater_discount = @$creditCardForm['repeater_discount'];
//        if($inForm->comiket_div == self::COMIKET_DEV_BUSINESS) {
//            $inForm->comiket_payment_method_cd_sel = "5";
//        } else {
        
//        }
Sgmov_Component_Log::debug("############## 801-33");
...
//        $buildingArray = $this->_BuildingService->fetchBuildingByEventId($db, $inForm->eventsub_sel);
        ...
        // 顧客区分 (ラジオボタンデフォルト選択されているので、基本的には以下チェックで引っ掛からない)
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_customer_kbn_sel)->isSelected()->isIn(array('1', '2'));
        if (!$v->isValid()) {
            $errorForm->addError('comiket_customer_kbn_sel', '出展イベントのお客様区分' . $v->getResultMessageTop());
        }
...
//            }
            ...
            // お名前 姓
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_personal_name_sei)->isNotEmpty()->isLengthLessThanOrEqualTo(7)->
                    isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_personal_name-seimei', 'お申込者-姓' . $v->getResultMessageTop());
            }
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_personal_name_mei)->isNotEmpty()->isLengthLessThanOrEqualTo(8)->
                    isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_personal_name-seimei', 'お申込者-名' . $v->getResultMessageTop());
            }
        }
...
//        }
        ...
        if ($inForm->comiket_customer_kbn_sel == '1') { // 出展者の場合
        ...
        if(!empty($eventsubInfo) && $eventsubInfo['building_display'] == '1') {
            ...
            if(!empty($inForm->building_booth_position_sel)) {
                ...
                $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->building_booth_position_sel)->isSelected()->isIn($buildingBoothPostionInfoAry['ids']);
                if (!$v->isValid()) {
                    $errorForm->addError('building_name_sel', 'ブース位置' . $v->getResultMessageTop());
                }
                $checkFlgBoothPostion = FALSE;
Sgmov_Component_Log::debug("##################### 999");
Sgmov_Component_Log::debug($inForm);
//Sgmov_Component_Log::debug($inForm->building_booth_postion_sel);
                foreach($buildingBoothPostionInfoAry["list"] as $key => $val) {
                    if($val["id"] == $inForm->building_booth_position_sel) {
                        $v = Sgmov_Component_Validator::createSingleValueValidator($val["booth_position"])->isLengthLessThanOrEqualTo(16);
                        if(!$v->isValid()) {
                            $errorForm->addError('building_name_sel', 'ブース位置' . $v->getResultMessageTop());
                        }
                        $checkFlgBoothPostion = TRUE;
                        break;
                    }
                }

                if(!$checkFlgBoothPostion) {
                    $errorForm->addError('building_name_sel', '選択ブース位置に該当するデータが存在しません。');
                }
            }


//        // ブース番号-プルダウン
//        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->building_booth_id_sel)->isSelected()->isIn($buildingArray['ids']);
//        if (!$v->isValid()) {
////Sgmov_Component_Log::debug("###################################### 999");
////Sgmov_Component_Log::debug($v->getResultMessageTop());
//            $errorForm->addError('building_booth_id_sel', 'ブース番号' . $v->getResultMessageTop());
//        }
            
            if ($inForm->comiket_customer_kbn_sel == '1'
                    && ($inForm->building_booth_position_sel <= '600' || '606' <=  $inForm->building_booth_position_sel)) { 
                // 出展者の場合 && building.cd が 606 以上の場合はチェックする（601:コンコース、602:ゆるキャラ、603:おみあげコンテスト、604:お祭りひろば、605:ふるさとステージ）
                // ブース番号-テキスト
                $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_booth_num)->isNotEmpty()->isLengthLessThanOrEqualTo(4)->isInteger(0)->
                        isNotHalfWidthKana()->isWebSystemNg();
                if (!$v->isValid()) {
                    $errorForm->addError('building_name_sel', 'ブースNO' . $v->getResultMessageTop());
                }
...
//Sgmov_Component_Log::debug($inForm);
        ...
        if($inForm->comiket_detail_type_sel == "1" || $inForm->comiket_detail_type_sel == "3") { // 搬入の場合
            ...
            // 搬入-集荷先番地・建物名 必須チェック 40文字チェック WEBシステムNG文字チェック
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if($this->checkColAndDelDate("outbound", $inForm->comiket_div, $inForm->comiket_detail_outbound_service_sel, $eventsubInfo)) {
                ...
                if ($inForm->hid_comiket_detail_outbound_collect_date_from == '1900-01-01'
                    ...
                    $dateCollOrg = "{$yearColl}-{$monthColl}-{$dayColl} 00:00:00";
                    $dateCollYmdOrg = "{$yearColl}/{$monthColl}/{$dayColl}";
                    ...
                    if (@!empty($inForm->comiket_detail_outbound_pref_cd_sel)) {
                        $resSyukaDateFromTo = $this->getSyukaDateFromTo($inForm->comiket_detail_outbound_pref_cd_sel, $inForm->eventsub_sel);

                        $shukaFrDate = $resSyukaDateFromTo['shukaFrDateStr'];

                        $shukaToDate = $resSyukaDateFromTo['shukaToDateStr'];

                        if ($shukaFrDate <= $dateCollOrg &&  $dateCollOrg  <= $shukaToDate) {
                            if ($dateColl <= $dateNow && @!empty($yearColl) 
                                    && @!empty($monthColl) && @!empty($dayColl)) {
                                $errorForm->addError('comiket_detail_outbound_collect_date', "搬入-お預かり日時:{$dateCollYmdOrg} の受付時間は終了しました。");
                                $isOutboundCollectDateCheck = false;
                            }
                        }
                    }
    ...
    //                    $date = new DateTime($eventsubInfo["out_bound_unloading_to"]);
    ...
    //                    $date = new DateTime($eventsubInfo["out_bound_unloading_fr"]);
        ...
        //                }
                        if (!$v->isValid()) {
                            ...
                            if($formatDateSt == $formatDateEnd) {
                                ...
                                $errorForm->addError('comiket_detail_outbound_collect_date', "搬入-お預かり日時は、{$formatDateSt}～{$formatDateEnd}までの日付を入力してください。（引渡し希望日期間もご確認ください）");
    //                            throw new Exception("There must be an error in the data in ".__FILE__." at line ".__LINE__.": formatDateSt {$formatDateSt} > formatDateEnd {$formatDateEnd}?!)");
                    ...
                    if($this->checkColAndDelTime("outbound", $inForm->comiket_div, $inForm->comiket_detail_outbound_service_sel, $eventsubInfo)) {
                        ...
                        if ($is_valid && !$v->isValid()) {
                            $errorForm->addError('comiket_detail_outbound_collect_date', '搬入-集荷可能期間の最終日のため、18時間～20時は選択できません。');
                        }
                    }
                }
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_outbound_delivery_date', '搬入-お届け日時' . $v->getResultMessageTop());
            }
...
//                    || ($inForm->comiket_detail_outbound_delivery_time_sel != "00" && empty($inForm->comiket_detail_outbound_delivery_time_sel)
                    ) {
                $max = null;
                $max_year  = null;
                $max_month = null;
                ...
                $v->isSelected()->isDate($min, $max);
                if (!$v->isValid()) {
//                    $errorForm->addError('comiket_detail_outbound_delivery_date', '搬入-お届け日' . $v->getResultMessageTop());
                    if ($formatDateSt < $formatDateEnd) {
                        $errorForm->addError('comiket_detail_outbound_delivery_date', "搬入-引渡し希望日は、{$formatDateSt}～{$formatDateEnd}までの日付を入力してください。");
                    } elseif ($formatDateSt > $formatDateEnd) {
            ...
            // 搬入-お届け日時-時間帯 必須チェック
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_outbound_delivery_time_sel)->isSelected()->
//                    isIn(array_keys($this->comiket_detail_time_lbls_par30m));
//            $is_valid = $v->isValid();
//            if (!$is_valid) {
//                $errorForm->addError('comiket_detail_outbound_delivery_date', '搬入-お届け日時-時間帯' . $v->getResultMessageTop());
//            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            // 搬出-便種区分 (ラジオボタンデフォルト選択されているので、基本的には以下チェックで引っ掛からない)
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_outbound_binshu_kbn_sel)->isSelected()->isIn(array('0', '1', '2'));
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_outbound_binshu_kbn_sel', '便種区分' . $v->getResultMessageTop());
            }
...
Sgmov_Component_Log::debug("################### 401-1");
Sgmov_Component_Log::debug($inForm->comiket_div);
//                if($inForm->comiket_div == self::COMIKET_DEV_BUSINESS
//                        || ($inForm->comiket_div == self::COMIKET_DEV_INDIVIDUA && $inForm->event_sel == '2')) {
Sgmov_Component_Log::debug("################### 401-2");
...
Sgmov_Component_Log::debug("################### 401-3");
Sgmov_Component_Log::debug($inForm->delivery_charge);
...
//                    }
            ...
            // 搬入-備考
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_outbound_note)->isNotEmpty()->isLengthLessThanOrEqualTo(128)->
//                    isNotHalfWidthKana()->isWebSystemNg();
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_detail_outbound_note', '搬入-備考' . $v->getResultMessageTop());
//            }
...
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_outbound_note2)->isLengthLessThanOrEqualTo(16)->
//                    isNotHalfWidthKana()->isWebSystemNg();
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_detail_outbound_note', '搬入-備考-2行目' . $v->getResultMessageTop());
//            }
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_outbound_note3)->isLengthLessThanOrEqualTo(16)->
//                    isNotHalfWidthKana()->isWebSystemNg();
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_detail_outbound_note', '搬入-備考-3行目' . $v->getResultMessageTop());
//            }
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_outbound_note4)->isLengthLessThanOrEqualTo(16)->
//                    isNotHalfWidthKana()->isWebSystemNg();
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_detail_outbound_note', '搬入-備考-4行目' . $v->getResultMessageTop());
//            }
...
////                $inForm->comiket_detail_outbound_address;
////                $inForm->comiket_detail_outbound_building;
...
//                    "szAddress" => $prefectures['names'][$key] . $inForm->comiket_detail_outbound_address . $inForm->comiket_detail_outbound_building,
//                    "szTel" => $inForm->comiket_detail_outbound_tel
//                ));
//                $zipV->zipCodeExistSocket()->zipCodeCollectable();
//                if (!$zipV->isValid()) {
//                    $errorForm->addError('comiket_detail_outbound_zip', "搬入-" . $zipV->getResultMessageTop());
//                }
...
//                $zipV_I->zipCodeExistSocket()->zipCodeCollectable();
//                if (!$zipV_I->isValid()) {
//                    $errorForm->addError('comiket_detail_inbound_zip', "搬出-" . $zipV_I->getResultMessageTop());
//                }
                ...
                if (empty($receive['ShopCodeFlag'])) {
                    $errorForm->addError('comiket_detail_inbound_zip', '配達先住所の入力内容をお確かめください。');
                } elseif (!empty($receive['ExchangeFlag'])) {
                    $errorForm->addError('comiket_detail_inbound_zip', '配達先住所は集荷・配達できない地域の恐れがあります。');
                } elseif (!empty($receive['TimeZoneFlag'])
                    ...
                    $errorForm->addError('comiket_detail_inbound_delivery_date', '配達先住所は時間帯指定できない地域の恐れがあります。');
                } elseif (!empty($receive['RelayFlag'])) {
                    $errorForm->addError('comiket_detail_inbound_zip', '配達先住所は配達できない地域の恐れがあります。');
                }
            }
        }
...
//     * @return boolean
//     */
//    public function _getAddressOut($inForm, $prefectures) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if($inForm->comiket_detail_type_sel == "2" || $inForm->comiket_detail_type_sel == "3") { // 搬出の場合
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_name)->isNotEmpty()->isLengthLessThanOrEqualTo(32)->
                    isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_inbound_name', '配達先名' . $v->getResultMessageTop());
            }
            ...
            if (!$zipV->isValid()) {
                $errorForm->addError('comiket_detail_inbound_zip', '配達先郵便番号' . $zipV->getResultMessageTop());
            }
            ...
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_inbound_pref', '配達先都道府県' . $v->getResultMessageTop());
            }
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_address)->isNotEmpty()->isLengthLessThanOrEqualTo(14)->
                    isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_inbound_address', '配達先市区町村' . $v->getResultMessageTop());
            } else {
                ...
                if (strpos($inForm->comiket_detail_inbound_address, $prefName) !== false) {
                    $errorForm->addError('comiket_detail_inbound_address', '配達先市区町村には都道府県名は入力しないで下さい。');
                }
            }
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_building)->isNotEmpty()->isLengthLessThanOrEqualTo(30)->
                    isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_inbound_building', '配達先番地・建物名・部屋番号' . $v->getResultMessageTop());
            } else {
                $prefName = @$prefectures["names"][$prefectures["ids"][$inForm->comiket_detail_inbound_pref_cd_sel]];
                if (strpos($inForm->comiket_detail_inbound_building, $prefName) !== false) {
                    $errorForm->addError('comiket_detail_inbound_building', '搬出-配送先番地・建物名・部屋番号には都道府県名は入力しないで下さい。');
                }
            }
            ...
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_inbound_tel', '配達先TEL' . $v->getResultMessageTop());
            } else {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_inbound_collect_date', 'カウンターお預かり日' . $v->getResultMessageTop());
            } else {
            ...
            // カウンターお預かり日 範囲チェック
                if (empty($inForm->comiket_detail_inbound_collect_date_year_sel)
                    ...
                    if (!$v->isValid()) {
                        ...
                        $formatDateSt = date('Y/m/d', $min);
                        if($formatDateSt == $formatDateEnd) {
                            $errorForm->addError('comiket_detail_inbound_collect_date', "カウンターお預かり日は、{$formatDateSt}を入力してください。");
                        } elseif ($formatDateSt < $formatDateEnd) {
                            $errorForm->addError('comiket_detail_inbound_collect_date', "カウンターお預かり日は、{$formatDateSt}～{$formatDateEnd}までの日付を入力してください。");
                        } elseif ($formatDateSt > $formatDateEnd) {
                            $errorForm->addError('comiket_detail_inbound_collect_date', "カウンターお預かり日は、{$formatDateSt}～{$formatDateEnd}までの日付を入力してください。（お届け指定日期間もご確認ください）");
                        }
                    } else {
                        /////////////////////////////////////////////////////////////////////////////////////////////
                        // 搬出-クール便申込時間チェック
                        /////////////////////////////////////////////////////////////////////////////////////////////
                        $resInfo = $this->checkCoolbinClosingDate((array)$inForm, false);
                        if ($resInfo['isErr']) {
                            $errorForm->addError('comiket_detail_inbound_collect_date', "カウンターお預かり日-{$resInfo['message']}");
                        }
                        /////////////////////////////////////////////////////////////////////////////////////////////
                    }
                }
...
//            if($inForm->comiket_div == self::COMIKET_DEV_INDIVIDUA) { // 個人
            ...
            if($this->checkColAndDelDate("inbound", $inForm->comiket_div, $inForm->comiket_detail_inbound_service_sel, $eventsubInfo)) {
                if($inForm->comiket_detail_inbound_service_sel == "1") {
                    ...
                    if (!$v->isValid()) {
                        $errorForm->addError('comiket_detail_inbound_delivery_date', '配達希望日' . $v->getResultMessageTop());
                    }
...
//                    $date = new DateTime($eventsubInfo["in_bound_unloading_fr"]);
//                    $date = new DateTime();
//                    $date->modify('+11 day');
//                    $date->modify('+0 day');
...
//                    $date = new DateTime($eventsubInfo["in_bound_unloading_to"]);
//                    $date->modify('+5 day');
//                    $date->modify('+42 day');
                        $max  = intval($date->format('U'));
                        $formatDateEnd = $date->format('Y/m/d');
                        ...
                        if (!$v->isValid()) {
                            if ($formatDateSt < $formatDateEnd) {
                                $errorForm->addError('comiket_detail_inbound_delivery_date', "配達希望日は、{$formatDateSt}～{$formatDateEnd}までの日付を入力してください。");
                            } elseif ($formatDateSt > $formatDateEnd) {
                                $errorForm->addError('comiket_detail_inbound_delivery_date', "配達希望日は、{$formatDateSt}～{$formatDateEnd}までの日付を入力してください。（お預かり日期間もご確認ください）");
//                                throw new Exception("There must be an error in the data in ".__FILE__." at line ".__LINE__.": formatDateSt {$formatDateSt} > formatDateEnd {$formatDateEnd}?!)");
                            }
                        }
                ...
                // 搬出-配達希望日-時間帯 チェック(宅配のみ)
//                    if($inForm->comiket_div == self::COMIKET_DEV_INDIVIDUA && !empty($inForm->comiket_detail_inbound_delivery_time_sel)) { // 個人 && 時間帯が指定されている場合
                    if($this->checkColAndDelTime("inbound", $inForm->comiket_div, $inForm->comiket_detail_inbound_service_sel, $eventsubInfo)) {
                        if($inForm->comiket_div == self::COMIKET_DEV_INDIVIDUA &&
                            ...
                            $is_valid = $v->isValid();
                            if (!$is_valid) {
                                $errorForm->addError('comiket_detail_inbound_delivery_date', '配達希望日-時間帯' . $v->getResultMessageTop());
                            }
                        }
                    }
                }
...
//            $inForm->comiket_detail_inbound_binshu_kbn = '9';
            // 搬出-便種区分 (ラジオボタンデフォルト選択されているので、基本的には以下チェックで引っ掛からない)
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_binshu_kbn_sel)->isSelected()->isIn(array('0', '1', '2'));
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_inbound_binshu_kbn_sel', '便種区分' . $v->getResultMessageTop());
            }

            // 搬出-宅配数量
            if($inForm->comiket_detail_inbound_service_sel == "1") {

//                function _checkComiketBoxIntboundNumAry($inForm, &$errorForm) {
...
//                                isInteger(1)->isLengthLessThanOrEqualTo(3)->isWebSystemNg();
//                        if (!$v->isValid()) {
//                            $errorForm->addError('comiket_box_inbound_num_ary', '搬出-宅配数量' . $v->getResultMessageTop());
//                            break;
//                        }
//                    }
//                }
...
//                if($inForm->comiket_div == self::COMIKET_DEV_BUSINESS) { // 法人
////                    if($inForm->comiket_customer_cd_sel == "1") { // 顧客コード使用する
//                        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_box_inbound_num_ary[0])->isNotEmpty()->
//                                isInteger()->isLengthLessThanOrEqualTo(3)->isWebSystemNg();
//                        if (!$v->isValid()) {
//                            $errorForm->addError('comiket_box_inbound_num_ary', '搬出-宅配数量' . $v->getResultMessageTop());
//                        }
////                    } else { // 顧客コード使用しない
////                        _checkComiketBoxIntboundNumAry($inForm, $errorForm);
////                    }
//                } else if($inForm->comiket_div == self::COMIKET_DEV_INDIVIDUA) { // 個人
////                    _checkComiketBoxIntboundNumAry($inForm, $errorForm);
//                    $this->checkComiketBoxOutInboundNumAry($inForm, $errorForm, $inForm->comiket_box_inbound_num_ary, "comiket_box_inbound_num_ary", "搬出");
...
//                    }
            ...
            // 搬出-備考
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_note)->isNotEmpty()->isLengthLessThanOrEqualTo(128)->
//                    isNotHalfWidthKana()->isWebSystemNg();
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_detail_inbound_note', '搬出-備考' . $v->getResultMessageTop());
//            }
...
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_note2)->isLengthLessThanOrEqualTo(16)->
//                    isNotHalfWidthKana()->isWebSystemNg();
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_detail_inbound_note', '搬出-備考-2行目' . $v->getResultMessageTop());
//            }
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_note3)->isLengthLessThanOrEqualTo(16)->
//                    isNotHalfWidthKana()->isWebSystemNg();
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_detail_inbound_note', '搬出-備考-3行目' . $v->getResultMessageTop());
//            }
//            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_note4)->isLengthLessThanOrEqualTo(16)->
//                    isNotHalfWidthKana()->isWebSystemNg();
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_detail_inbound_note', '搬出-備考-4行目' . $v->getResultMessageTop());
//            }
        }
    ...
    protected function checkComiketBoxOutInboundNumAry($inForm, &$errorForm, $targetList, $targetClassName, $errMsgOutInbount, $isEmptyCheck = true) {
        ...
        $targetBoxIdList = array();
        $db = Sgmov_Component_DB::getPublic();
        
        $binshuKbnSel = $inForm->comiket_detail_outbound_binshu_kbn_sel;
        if ($inForm->comiket_detail_type_sel == '2') { // 搬出
            $binshuKbnSel = $inForm->comiket_detail_inbound_binshu_kbn_sel;
        }

        $targetBoxList = $this->_BoxService->fetchBox2($db, $inForm->eventsub_sel, $inForm->comiket_div, $inForm->comiket_detail_type_sel
                , $inForm->comiket_customer_kbn_sel, $binshuKbnSel);

        foreach($targetBoxList as $key => $val) {
            $targetBoxIdList[] = $val['id'];
        }
        
Sgmov_Component_Log::debug('######################## 401-3');
Sgmov_Component_Log::debug($_POST);
Sgmov_Component_Log::debug($binshuKbnSel);
Sgmov_Component_Log::debug($inForm);
Sgmov_Component_Log::debug($targetList);
Sgmov_Component_Log::debug($targetBoxIdList);
        
        foreach($targetList as $key => $val) {

            if (!@in_array($key,$targetBoxIdList)) {
                $errorForm->addError($targetClassName, "宅配数量の入力テキストにエラーがあります（便種選択を切り替えてみてください）。");
                return;
            }
        ...
        $isErrFlg2 = FALSE;
        if($errorFlg) {
            $errorForm->addError($targetClassName, "宅配数量の入力値を確認してください。（数値のみ）");
            $isErrFlg2 = TRUE;
        }
        if(5 <= $notEmptyCount) {
            $errorForm->addError($targetClassName, "宅配数量の入力は４つまでです。");
            $isErrFlg2 = TRUE;
        }
        if(0 == $notEmptyCount && $isEmptyCheck) {
            $errorForm->addError($targetClassName, "宅配数量を入力してください。");
            $isErrFlg2 = TRUE;
        }
Sgmov_Component_Log::debug('######################## 401-4');
...
Sgmov_Component_Log::debug('######################## 401-5');
    }
