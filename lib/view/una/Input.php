 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Una_Input extends Sgmov_View_Una_Common {
    ...
    public function executeInner() {
        ...
        if(!empty($param)) {
            ...
            if(!is_numeric($param) && $param > self::INT_MAX){
        ...
        if(!empty($ev) && is_numeric($ev)) {
            ...
            $inForm->comiket_div  = $this->_KokyakuDiv; // 識別子
            $inForm->eventsub_sel = self::EVENT_SUB_ID;
        }
        //BOXデフォルトを設定する
        $inForm->comiket_detail_type_sel = "1";
        ...
        if (isset($eventsubInfo)) {
            ...
            if (isset($eventsubInfo['arrival_to_time'])) {
                ...
                //宿泊日チェク 当日+宿泊日制限(最小)　>イベント終了日
                $deliTmpDate1 = new DateTime();
                $minDeliveryDate = $deliTmpDate1->modify(self::DELIVERY_START.'day');
                $maxOutBoundDate = new DateTime($eventsubInfo['out_bound_loading_to']);
                if ($minDeliveryDate->format('Y-m-d') > $maxOutBoundDate->format('Y-m-d')) {
                    $title = urlencode("公開終了しています");
                    $message = urlencode("申込期限が過ぎたので、登録できません");
                    Sgmov_Component_Redirect::redirectPublicSsl("/".$this->_DirDiv."/error?t={$title}&m={$message}");
                    exit;
                }
            } else {
        ...
        // チケット発行
        $ticket = $session->publishTicket(self::FEATURE_ID, self::GAMEN_ID_UNA001);
