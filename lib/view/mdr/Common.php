 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Mdr_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'MDR';

    // 2022-03-25 ToanDD3 implement SMT6-69
    const LOGIN_ID = 'MDR_LOGIN';

    /**
     * MDR001の画面ID
     */
    const GAMEN_ID_MDR001 = 'MDR001';

    /**
     * MDR002の画面ID
     */
    const GAMEN_ID_MDR002 = 'MDR002';

    /**
     * MDR003の画面ID
     */
    const GAMEN_ID_MDR003 = 'MDR003';
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    /**
     * カーゴ運賃マスタサービス
     * @var Sgmov_Service_CargoFare
     */
    protected $_CargoFareService;

    /**
     * イベントデータコンバート情報を扱います。
     * @var Sgmov_Service_EventsubCmb
     */
     ...
     * @param $checkArrival
     * @param $db
     * @param $_TravelService  
     * @param $_CruiseRepeater 
     * @param $inForm
    ...
    protected function createOutFormByInForm($inForm, $outForm = array()) {
        ...
        $eventAll = $this->_EventService->fetchEventListWithinTerm2($db, null, null, 'mdr');
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        foreach ($eventAll2 as $data) {
            $eventSubData = $this->_EventsubService->fetchEventsubListWithinTermByEventId($db, $data['id'], null, null, 'mdr');
            foreach ($eventSubData['list'] as $value) {
                $arrvalDt = $value['arrival_to_time'];
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $dataList = [];
        $dataList = $this->_BoxService->fetchBox2($db, $inForm["eventsub_sel"], $inForm["comiket_div"], "2"); 
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     * @param boolean $isAmountDataFromSession
     */
        ...
        if ($inForm['comiket_detail_type_sel'] == "2") {
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Mdr_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Mdr_Common::CURRENT_TAX));
                        // // 税込
    ...
    public function _cmbTableDataFromInForm($inForm, $comiketId="") {
        ...
        foreach($comiketBoxDataListForHaiso as $key => $val) {
            foreach(array_keys($comiketDataForHaiso["comiket_detail_list"]) as $key2) {
    ...
    public function _createComiketInsertDataByInForm($inForm, $id, $type="") {
        ...
        $data = array(
            ...
            "booth_num" => "",
            "staff_sei" => @empty($inForm['comiket_staff_sei']) ? $inForm['comiket_personal_name_sei'] : $inForm['comiket_staff_sei'],
            "staff_mei" => @empty($inForm['comiket_staff_mei']) ? $inForm['comiket_personal_name_mei'] : $inForm['comiket_staff_mei'],
            ...
            "staff_tel" => @empty($inForm['comiket_staff_tel']) ? $inForm['comiket_tel'] : $inForm['comiket_staff_tel'],
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "2") { // 復路
            if(!$this->isCurrentDateWithInTerm("arrival", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/mdr/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
        } else { // 往復
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/mdr/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/mdr/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/mdr/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
...
//        $departureTo = $eventsubData["departure_to"];
    ...
    /**
     *
     * @param boolean $inOutboundFlg
     * @param integer $comiketDiv
     * @param integer $serviceSel
     * @param array $eventsubInfo
     * @return boolean
     */
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/mdr_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/mdr_complete_business_sgmv{$tmplateType}.txt";
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/mdr_complete_individual{$tmplateType}.txt";

                $mailTemplateSgmv[] = "/mdr_complete_individual_sgmv{$tmplateType}.txt";

                $data['surname'] = $comiket['personal_name_sei'];
                $data['forename'] = $comiket['personal_name_mei'];
              
                $data['comiket_div'] = '出展者'; // みどり会
                ...
                // } else {
                //     $data['comiket_building_name'] = PHP_EOL . '【ブースNO】' . $comiket['booth_position'] . " " . $comiket['booth_num'];
                // }
            ...
            foreach ($comiket_detail_list as $comiket_detail) {
                ...
                //搬出用メールテンプレート
                $mailTemplate[] = '/mdr_parts_complete_choice_2.txt';

                $mailTemplateSgmv[] = '/mdr_parts_complete_choice_2_sgmv.txt';
                ...
                $data['toiawase_no'] = @$comiket_detail["toiawase_no_niugoki"];
                    ...
                    case 1://宅配
                        ...
                        foreach ($comiket_box_list as $comiket_box) {
                            ...
                            if (($comiket_detail['type'] == 2 && $comiket_box['type'] == 2) || ($comiket_detail['type'] == 1 && $comiket_box['type'] == 1)
                                ...
                                $num_area .= '    ' . @strip_tags($boxName) . ' ［' . number_format($comiket_box['num']) . ' 個］' . PHP_EOL;
                            }
                        }
                        break;
                }
                ...
                // 20222-03-25 ToanDD3 implement SMT6-84
            ...
            if ($tmplateType == '_cancel' || $tmplateType == '_sgmv_cancel') {
                ...
                if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) { // 法人
                    $mailTemplate[] = "/mdr/parts{$footerTmpName}_footer_type_1.txt";
                    $mailTemplateSgmv[] = "/mdr/parts{$footerTmpName}_footer_type_1.txt";
                } else { // 個人
                    $mailTemplate[] = "/mdr/parts{$footerTmpName}_footer_type_2.txt";
                    $mailTemplateSgmv[] = "/mdr/parts{$footerTmpName}_footer_type_2.txt";
                }
            } else {
                //セキュリティチェック
                if ($isSecurity) {
            ...
            // 2022-03-25 ToanDD3 implement SMT6-59
            // if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) { // 法人
            //     $mailTemplate[] = "/mdr_parts{$footerTmpName}_footer_type_1.txt";

            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/mdr_parts{$footerTmpName}_footer_type_1.txt";
            // } else { // 個人
            //     $mailTemplate[] = "/mdr_parts{$footerTmpName}_footer_type_2.txt";

            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/mdr_parts{$footerTmpName}_footer_type_2.txt";
            // }
            ...
            // 2022-03-25 ToanDD3 implement SMT6-59
            // $data['edit_url'] = $urlPublicSsl . "/mdr/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
           // 2022-03-28 ToanDD3 fix SMT6-120
            if (!$isSecurity) {
                $data['edit_url'] = $urlPublicSsl . "/mdr/input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
            } else {
                if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                    $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm=mdr'; 
                } else {
                    $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=mdr&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
                }   
            }
            ...
            if($eventsubData['manual_display'] == '1') { // 説明書表示有無 0：無、1：有
                $data['manual_url'] = "【説明書URL】" . PHP_EOL . $urlPublicSsl . "/mdr/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            }
...
//             }
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // 2022-03-25 ToanDD3 implement SMT6-59
            // $data['cancel_url'] = $urlPublicSsl . "/mdr/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            // $data['size_change_url'] = $urlPublicSsl . "/mdr/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;          
            if (!$isSecurity) {
                $data['cancel_url'] = $urlPublicSsl . "/mdr/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
                $data['size_change_url'] = $urlPublicSsl .  "/mdr/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            } else {
                $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=mdr&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
                $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=mdr&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            }
            //-------------------------------------------------
            //メール送信実行
     ...
     * @param string $wsHost
     * @param string $wsPath
     * @param integer $wsPort
     * @param array $param
     * @param string $dispErrTitle
     * @param string $dispErrMsg
     * return boolean
     */
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        $sendFileName = 'MDR_' . date ( 'YmdHis' ) . '.csv';
        $boundary = "-----" . md5 ( uniqid () );
        ...
        try {
            ...
            $message = "みどり会: 業務連携に失敗しました。\n[mdr] {$clasName} : paramOrg = {$paramOrg} / param = {$param}\n\nDB comiket.del_flg（論理削除フラグ）は更新しています。\n\n";
            ...
            $mailTemplateList = array(
                "/mdr_cancel_{$divType}_error.txt",
//                "/mdr_parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
            );
    ...
    protected function checkReqDate($comiketId, $titleParts = "", $MessageParts = "") {
        ...
        foreach ($comiketDetailList as $comiketDetailInfo) {
