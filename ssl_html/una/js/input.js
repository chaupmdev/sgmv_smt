...
//                $('.paste_tag_link').attr("href", "/"+dirDiv+"/pdf/paste_tag/paste_tag_" + dirDiv + ".pdf" + '?' + strCurrentDate);
//            } else {
...
//                $('.manual_link').attr("href", "/"+dirDiv+"/pdf/manual/manual_" + dirDiv + ".pdf" + '?' + strCurrentDate);
//            } else {
...
//                    htmlCode += "<img src='/"+dirDiv+"/images/about_boxsize.png' width='100%' style='visibility:hidden;'/>";
                ...
                htmlCode += "</td>";
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                   // htmlCode += "<img class='dispSeigyoPC' src='/una/images/about_boxsize.png' width='100%'/>";
                }
                htmlCode += "</tr></table>";
                // 搬入
                $('.comiket-box-outbound-num').append(htmlCode);
                ...
                htmlCode2 += "</td>";
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                  //  htmlCode2 += "<img class='dispSeigyoPC' src='/una/images/about_boxsize.png' width='100%'/>";
                }
                htmlCode2 += "</tr></table>";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    $('input[name="adrs_search_btn"]').on('click', (function () {
        ...
        AjaxZip2.zip2addr(
            ...
            if ($('[name=comiket_pref_cd_sel]').val()) {
                // お預かり可能日付範囲を取得
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // 集荷先都道府県プルダウンリスト変更時
    $('[name=comiket_pref_cd_sel]').on('change', function(e) {
	if (!$('[name=comiket_detail_outbound_delivery_date_year_sel]').val()
			|| !$('[name=comiket_detail_outbound_delivery_date_month_sel]').val()
			|| !$('[name=comiket_detail_outbound_delivery_date_day_sel]').val()) {
                resetArrivalSelect();
    ...
    function checkAndSetValueBeforecallOutBoundCollect(){
        //共通県名から搬入の県名に設定する
        var kenValue = $('[name=comiket_pref_cd_sel]').val();
        if (kenValue === '') {
            return false;
        }
        $('[name=comiket_detail_outbound_pref_cd_sel]').val(kenValue);
        //宿泊日設定
        ...
        if (isNullOrEmpty(yearSelect) || isNullOrEmpty(monthSelect) || isNullOrEmpty(daySelect)) {
            return false;
        }
              
        var dateSelect = yearSelect + "-" + monthSelect + "-" + daySelect;
        $('#hid_comiket_detail_outbound_delivery_date').val(dateSelect);
        return true;
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////
    // 往路・搬入：お預かり可能日付範囲を取得
    ...
    function getOutBoundCollectCal() {
        var checkData = checkAndSetValueBeforecallOutBoundCollect();
        if (!checkData) {
            return;
        }
