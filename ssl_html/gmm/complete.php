...
Sgmov_Lib::useView('gmm/Complete');
Sgmov_Lib::useForms(array('Error', 'EveSession'));
/**#@-*/

$merchantResult = FALSE;
// 処理を実行
$view = new Sgmov_View_Gmm_Complete();
$eveOutForm = array();
$eventData = array();
$eventsubData = array();
            ...
            <div style="clear: both;">
                ...
                <?php  if ($typeSel == "1" || $typeSel =="3") : // 搬入の場合?>
                    <?php if($paymentMethodCd == 1): // コンビニ前払いの場合 ?>
                       <!--  <br /><span style="color: red;" class = "lhn">※お支払いはお預かり日時の<?php //echo $collectDate;?>の前日17時までに入金していただきますようお願いいたします。</span><br /><br /> -->
                    <?php endif; ?>
                <?php endif; ?>
                ...
                <?php if(($eveOutForm->comiket_detail_type_sel() == "1" || $eveOutForm->comiket_detail_type_sel() == "3") // 搬入、もしくは往復
                    ...
                    ・ 画面下の「貼付票のDL」からPDFをダウンロードし、貼付票を印刷後、荷物に貼り付けて、発送ください。</span><br/><br/>
                ...
                <?php if ($eventsubData['manual_display'] == '1') { ?>
                   <span class ="lhn">ご不明な点はメールに添付されている説明書、もしくは下記のボタンから説明書をダウンロードして、ご確認ください。</span><br/><br/>
                    <div style="font-size:20px;">
                        <?php if($eveOutForm->comiket_detail_type_sel() == "1"):?>
                ...
                <?php if(!empty($existsComiketId)):?>
                    <b>引き続き搬出申込する方はこちら<a href="/gmm/input/<?php echo $existsComiketId; ?>" target="_self" style="color:blue;font-weight: bold;text-decoration: underline;">こちら</a></b>
                <?php else:?>
                     <b>引き継き宅配をご利用される方は<a href="/gmm/input" target="_self" style="color:blue;font-weight: bold;text-decoration: underline;">こちら</a></b>
                <?php endif;?>
