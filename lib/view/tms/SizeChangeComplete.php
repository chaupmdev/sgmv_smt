 ...
 */
class Sgmov_View_Tms_SizeChangeComplete extends Sgmov_View_Tms_Complete {
    ...
    public function executeInner() {
        ...
        $boxNumTotal = 0;
        if ($inForm['comiket_detail_type_sel'] == "3") { // 往復
        ...
        } else if ($inForm['comiket_detail_type_sel'] == "1") { // 往路
            foreach ($inForm['comiket_box_outbound_num_ary'] as $key => $val) {
                if (@!empty($val)) {
                    $boxNumTotal += (integer) $val;
                }
...
Sgmov_Component_Log::debug($inForm);
        ...
        foreach ($comiketDetailList as $key => $comiketDetailInfo) {
            $comiketBoxList = $this->_ComiketBox->fetchComiketBoxDataListByIdAndType($db, $inForm['comiket_id'], $comiketDetailInfo['type']);
        ...
        foreach ($comiketDetailList as $key => $comiketDetailInfo) {
            if($comiketDetailInfo['type'] == '1') { // 往路
                ...
                if ($collectDate2 <= $toDate) {
                    ...
                    $title = "催事・イベント配送受付サービスの搬入の個数・サイズ変更のお申し込みができませんでした";
                    $message = "既に {$collectDate2} を過ぎているため搬入の個数・サイズ変更のお申し込みができませんでした。(集荷日前日 18:00までサイズ変更可能)";
                    Sgmov_Component_Redirect::redirectPublicSsl("/tms/error?t={$title}&m={$message}");
                }
            } else { // 復路
                ...
                if ($eventTermEndDate <= $toDate) {
                    $eventTermEndDate2 = date('Y年m月d日 H:i', strtotime($eventTermEndDate));

                    $title = "催事・イベント配送受付サービスの搬出の個数・サイズ変更のお申し込みができませんでした";
                    $message = "既に {$eventTermEndDate2} を過ぎているため搬出の個数・サイズ変更のお申し込みができませんでした。(イベント終了日までサイズ変更可能)";
                    Sgmov_Component_Redirect::redirectPublicSsl("/tms/error?t={$title}&m={$message}");
                }
            }
        }
        ...
        if (@empty($boxNumTotal)) {
            ...
            $parentRes = array(
                ...
                'type' => $inForm['comiket_detail_type_sel'],
                'payment_method_cd'=> $comiketInfo['payment_method_cd'],
                'is_cancel_only' => true,
            );
        } else {
            // 先に親処理の実行(セッションはなくなる)
        ...
        // TODO: どのデータが削除したかわからないと思うため、サイズ変更の場合もキャンセル完了メール送信しておく
//        if (@empty($boxNumTotal)) {
            ///////////////////////////////////////////////////////////////////////////////////////////////////
            // ▼ キャンセル完了メール送信
            ///////////////////////////////////////////////////////////////////////////////////////////////////
...
//        }
    ...
    protected function sendCompleteMail2($li, $inForm) {
        ...
        if ((!empty($li["treeData"]['mail']) && !empty($li["treeData"]['merchant_result']))
            ...
            $this->sendCompleteMail($li["treeData"], $li["treeData"]['mail'], $ccMail, $type, 'sizechange');
        }
    }
}