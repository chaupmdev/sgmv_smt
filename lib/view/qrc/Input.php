 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Qrc_Input extends Sgmov_View_Qrc_Common
{
    ...
    public function executeInner()
    {
        ...
        if ($input2DialogFlg == "1") {
            ...
            if (
                $comiketData['tel'] == $input2DialogData1Tel
        ...
        if (isset($eventsubInfo)) {
            $now2 = date('Y-m-d');
            if (
                $eventsubInfo['departure_to'] < $now2
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // チケット発行
        $ticket = $session->publishTicket(self::FEATURE_ID, self::GAMEN_ID_QRC001);
        return array(
            'ticket'    => $ticket,
            'outForm'   => $outForm,
     ...
     * @return Sgmov_Form_Qrc001Out 出力フォーム
     */
    protected function _createOutFormByInForm($inForm, $param = NULL)
    {
        $inForm = (array)$inForm;
        return $this->createOutFormByInForm($inForm, new Sgmov_Form_Qrc001Out());
    }
}
