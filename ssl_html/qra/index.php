...
// エラーページURL
$redirectUrl = "https://" . $_SERVER["HTTP_HOST"] . "/carderr.php";
 ...
 * >　SYSTEM_KBN           区分（0:MV-BASE(引っ越し、設置)、1:SG-ARK）
...
// TODO
if ($_GET["env"] == 'st' || $_GET["env"] == 'dev' || $_GET["env"] == 'prod'){
}
else{
    header("HTTP/1.0 404 Not Found");
    header("Location: $redirectUrl");
...
$_POST['comiket_mail'] =  "mas-fujikawa@spcom.co.jp";
$_POST['comiket_mail_retype'] =  "mas-fujikawa@spcom.co.jp";
...
require_once dirname(__FILE__) . '/../../lib/service/Comiket.php';
// DBへ接続
$db = Sgmov_Component_DB::getPublic();
// ※重複登録のチェック
$_Comiket = new Sgmov_Service_Comiket();
$QR_Check = $_Comiket->fetchQRById($db, $_GET, $_GET["SYSTEM_KBN"]);

if (!$QR_Check) {
    Sgmov_Component_Redirect::redirectPublicSsl("/" . $dirDiv . "/qr_err");
    exit;
}

// 処理を実行
$view = new Sgmov_View_Qra_CheckInput();
$view->execute();
