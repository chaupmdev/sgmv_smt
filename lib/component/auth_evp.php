...
// イベント名
    $eventName = 'コミケットアピール105（サークル（一般）出展者）';

// 公開開始日時
$openDateStr = '2024-12-13 00:00:00';
...
$prm = array(
    'open' => $openDate,     'event' => $eventName, 'dir'  => 'evp' // evp固定で問題ないです
);
...
function redirectKohkaimae($prm){
    ...
    //$message = urlencode("「" . $prm['event'] . "」のお申込は 「" . $day . " " . $yobi . " " . $time . "」 から開始となります。今しばらくお待ちください。");
    $message = urlencode("コミックマーケット102サークルの宅配便のお申込は 2023/07/27から開始となります。今しばらくお待ちください。");
