...
abstract class Sgmov_View_Rms_Common extends Sgmov_View_Public {
     ...
     * 機能ID
     */
    const FEATURE_ID = 'TME';

    // GiapLN implement SMT6-85
    const LOGIN_ID = 'TME_LOGIN';
    ...
    const COLLECT_DAY   = '5';
    // 時間帯
    const COLLECT_TIMEFRAME   = '14:00～22:00';
...
//            $data['event_name'] = $eventData["name"] . " " . $eventsubData["name"]; //【出展イベント】
            ...
            if ($tmplateType === '_cancel' || $tmplateType === '_sgmv_cancel') {
                $mailTemplate[] = "/tme_parts{$footerTmpName}_footer_type_1.txt";
            } else {
...
//            }
            ...
            if ($_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm=tme'; 
            } else {
                $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=tme&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
            }
...
//            $data['manual_url'] = $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            ...
            if(($comiket['choice'] == "1" || $comiket['choice'] == "3")
                ...
                if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 0){
                    ...
                    $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/event/robotCheck?event_nm=tme&destination=paste_tag/{$pasteTagId}";
                }
            ...
            $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=tme&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            // $data['size_change_url'] = $urlPublicSsl . "/".$this->_DirDiv."/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=tme&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
