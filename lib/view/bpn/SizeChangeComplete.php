...
Sgmov_Lib::useServices(array('Comiket', 'ComiketDetail', 'ComiketBox', 'ComiketCargo', 'ComiketCharter'
    , 'CenterMail', 'SgFinancial', 'HttpsZipCodeDll', 'BoxFare', 'Charter'));
Sgmov_Lib::useImageQRCode();
Sgmov_Lib::use3gMdk(array('3GPSMDK'));
/**#@-*/

/**
 * 物販お申し込みサイズ変更の登録完了と完了画面を表示
 * @package    View
 * @subpackage BPN
 * @author     K.Sawada
 * @copyright  2018-2019 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Bpn_SizeChangeComplete extends Sgmov_View_Bpn_Complete {
...
//        Sgmov_Component_ErrorExit::errorExit(Sgmov_Component_ErrorCode::ERROR_VIEW_INVALID_PAGE_ACCESS);
        ...
        // // 商品の申込期間を確認する。
        // $this->checkShohinTerm($db, $inForm);
        $boxNumTotal = 0;
        ...
        if (@empty($boxNumTotal)) {
            ...
            // 出力情報を設定
            $outFormBuppan->raw_qr_code_string = $inForm['comiket_id'];
            ...
            $sessionForm->comiket_id = $inForm['comiket_id'];
            // 先に親処理の実行(セッションはなくなる)
            $parentRes = parent::executeInner("size_change", $comiketInfo["bpn_type"]);
        }
        ...
        // 物販タイプ(１：物販、２：当日物販)
        $parentRes["bpnType"] = $comiketInfo["bpn_type"];
        ...
        //if (@empty($boxNumTotal)) {
            ...
            $comiketInfo['comiket_detail_list'] = [];
    ...
    protected function sendCompleteMail2($li, $inForm, $bpnType) {
        ...
        if ((!empty($li["treeData"]['mail']) && !empty($li["treeData"]['merchant_result']))
            ...
            // }
            if($bpnType == "1"){
                $this->sendCompleteMail($li["treeData"], $li["treeData"]['mail'], $ccMail, '_sizechange');
            }else{
                $toMail = Sgmov_Component_Config::getBuppanCompleteMail();
                $this->sendCompleteMailForActiveShohin($li["treeData"], $toMail, $ccMail, '_sizechange');
            }
        }
    }
}