...
//    getEventsubData();
    ...
    if(comiketDetailTypeSel && comiketDetailTypeSel != "") {
        selectComiketDetailType();
        dispComiketDetailType();
        //GiapLN update show display text box in  宅配数量 2024.12.04
        
        const parent = document.getElementById('main');
        const isErrorChk = parent.querySelectorAll('.err_msg');
        
        let uri = window.location.pathname;
        let position = uri.indexOf("size_change");
        
        let posIsBack = uri.indexOf("clr=1");
        
        if (isErrorChk.length == 0 || position > 0) {
            // Get the query string from the URL
            const queryString = window.location.search;
            const urlParams = new URLSearchParams(queryString);
            
            if (urlParams.has('clr')) {
            } else {
                getBoxData();
            }
        }
...
//                    htmlCode += "<img src='/"+dirDiv+"/images/about_boxsize.png' width='100%' style='visibility:hidden;'/>";
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode2 += "<img src='/"+dirDiv+"/images/about_boxsize.png' width='100%'/>";
                }
...
//            }
            ...
            if(comiketDetailTypeVal == "1") { // 搬入
                // 一旦全てを表示する
                $('div#payment_method>label').show();
                // 売掛の表示とチェックを削除する
                $('span.dispUrikake').remove();
                $('input#pay_convenience_store_comppay').attr("checked", false);
                ...
                // 搬出の場合は売掛扱いにする
                // 支払いのラジオボタンの表示を全て消して、売掛の文言を追加して、売掛のラジオボタンにチェックを入れる
                $('div#payment_method>label').hide(g_fifoVal);
                $('span.dispUrikake').remove();
                $('div#payment_method>span').after('<span class="dispUrikake">　　復路の宅配料金は主催者様がご負担いたしますので、お支払いはございません。</span>');
                $('div.pay_notice_block').hide(g_fifoVal);
                $('input#pay_convenience_store_comppay').prop("checked", true);
                    $('[name=comiket_convenience_store_cd_sel]').hide(g_fifoVal);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // エリアコンボに連動してお店名をセットする
    $('.building_booth_position_sel').on('change', function() {
    $('.building_booth_position_sel').comiket_booth_name
    $('span.shop_name').text('');
    $('input[name=comiket_booth_name]').val('');

    // 店名をセットする
    var list=[];
    list=getList();
    $('span.shop_name').text(list[$(this).val()]);
    $('input[name=comiket_booth_name]').val(list[$(this).val()]);

    });


    // エリアコンボに連動してお店No.のコンボを変更する
    function getBuildingBoothPositionApi(isChangeFlg) {
        sgwns.api('/'+dirDiv+'/api/building_booth_position.php', getFormData(), (function (data) {
...
function getList(){
    var list=[];
    list[1250]='MOTHERのデパート';
    list[1251]='ほぼ日のアースボールの店';
    list[1252]='ほぼ日Supermarche';
    list[1253]='湖池屋とほぼ日のあげたてポテチ(R)直売所';
    list[1254]='BambooCutの梅まつり';
    list[1255]='だいどこ道具ツチキリ ほぼ日支店';
    list[1256]='東京 やまほん';
    list[1257]='斉吉の東北おいしい村';
    list[1258]='飯島奈美 SUPER LIFE MARKET';
    list[1259]='ごはんの土楽。';
    list[1260]='肌とメイクと道具のブランド Shin;Kuu';
    list[1261]='YUKI FUJISAWA 箔と布の世界';
    list[1262]='がまぐちウォレットのお店 Belluria';
    list[1263]='Imaginary Food Stand： YUKI & DAUGHTERS';
    list[1264]='草場妙子化粧品店';
    list[1265]='hibi 10MINUTES AROMA 香りのビュッフェ';
    list[1266]='あでやかなパンプスの店 Relier81';
    list[1267]='KLOKA UTOPIAN "OMIYAGE" SHOP';
    list[1268]='ちくちくフェルトのくまSHOP Sleepy Sheep';
    list[1269]='指時計のお店 moco';
    list[1270]='ビーズ刺繍ジュエリーのお店 tamas';
    list[1271]='ヘアジュエリーのお店 POTTENBURN TOHKII';
    list[1272]='鉱物と彫金のジュエリーのお店 Fillyjonk';
    list[1273]='刺繍のジュエリーのお店 patterie';
    list[1274]='気仙沼お取り寄せ食堂';
    list[1275]='しぼりたて日本酒店';
    list[1276]='ほぼ日ペールエールの店';
    list[1277]='くいしんぼうのパンショップ';
    list[1278]='タイムテーブル洋菓子店';
    list[1279]='たのしみ弁当';
    list[1280]='コーヒーの国';
    list[1281]='weeksdays コンバースと大人のTシャツのお店';
    list[1282]='つきのみせ。心地よいランジェリーとウェアのお店';
    list[1283]='轟木節子がつくる、気持ちのいい服。';
    list[1284]='MHL. 春の蚤の市';
    list[1285]='O2 <BETTER THAN ONE> ';
    list[1286]='LDKWARE　ホームのお店';
    list[1287]='MITTAN ずっと着られるお店。';
    list[1288]='小林和人のあたらしい日用品店';
    list[1289]='ひびのこづえの小さな家';
    list[1290]='洗濯ブラザーズの洗剤量り売り';
    list[1291]='tokyobike＋HOBONICHI わたしの自転車。';
    list[1292]='アンリアレイジのほぼ非日常';
    list[1293]='weeksdays 鋼正堂のほりだしもの市';
    list[1294]='藤子・F・不二雄 characters in HOBONICHI field！';
    list[1295]='HUMAN MADE ＆ HOBONICHI ZUBON store';
    list[1296]='warang wayan バリとモロッコのかごの店';
    list[1297]='VEJA フランスから来たやさしいスニーカー';
    list[1298]='R&D.M.Co- 富士吉田のアトリエから';
    list[1299]='リュック！リュック！リュック！';
    list[1300]='STAMPS CLOTH SHOP';
    list[1301]='talo フィンランドスツールとヴィンテージランプの店';
    list[1302]='NAOTの育てる靴。';
    list[1303]='かざりたくなる、絵と作品のお店';
    list[1304]='みうらじゅん、巨大絵をかざる。';
    list[1305]='HAPPY NUTS DAY ピーナッツバターのお店';
    list[1306]='にほん茶とおせんべいの店';
    list[1307]='林屋海苔店のお店';
    list[1308]='奥山いちご農園のいちご屋さん';
    list[1309]='やさしいタオル よりどりハンド';
    list[1310]='ほぼトリドリ いろトリドリSHOP';
    list[1311]='集合レジ１';
    list[1312]='集合レジ２';
    list[1313]='サブレジ';
    list[1314]='たのしみエントランス';
    list[1315]='なんでも広場';
