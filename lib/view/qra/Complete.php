...
Sgmov_Lib::useForms(array('Error', 'QraSession', 'Qra004Out',));
Sgmov_Lib::useServices(array(
    'Comiket', 'ComiketDetail', 'ComiketBox', 'ComiketCargo', 'ComiketCharter', 'CenterMail', 'SgFinancial', 'HttpsZipCodeDll', 'CargoFare', 'BoxFare', 'Charter', 'ComiketKanren', 'GyomuApi'
));
 ...
 */
class Sgmov_View_Qra_Complete extends Sgmov_View_Qra_Common
{
    ...
    /**
     * 業務連携サービス
     * @var Sgmov_Service_GyomuApi
     */
    public $_gyomuApiService;

    /**
     * 業務連携サービス
     * @var Sgmov_View_Qra_Bcm
     */
    protected $_BcmView;
    ...
    public function __construct()
    {
        ...
        //$this->_BcmView             = new Sgmov_View_Qra_Bcm();
    ...
    public function executeInner($argBackInputPaht = "")
    {
        ...
        //チケットの確認と破棄 TODO
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_QRA003, $this->_getTicket());
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        // comiket登録前に業務側から請求先問番取得  をやめてQRに含まれる問い番に切り替える
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        $toiban = (array) $sessionForm->in;

        $toiawaseNo = $toiban['qr_toiawase_no'];

        // DBへ接続
        $db = Sgmov_Component_DB::getPublic();

        $inForm = $this->_createDataByInForm($db, $sessionForm->in, $toiawaseNo); //取引コードの為にもう一度
        ...
        $inForm['comiket_toiawase_no_niugoki'] = $toiawaseNo;
        ...
        $outForm = new Sgmov_Form_Qra004Out();
        ...
        // ベリトランス
        $registerData = $this->payment($db, $inForm, $comiketId, $argBackInputPaht);

        // エラーが発生した時、戻る画面
        $backInputPath = $argBackInputPaht;
        if (@empty($backInputPath)) {
            $backInputPath = "credit_card";
            // if (@empty($inForm['comiket_id'])) {
            //     $backInputPath = "credit_card";
            // }
            if ($_SESSION['QRError'] != '処理が成功しました。') {
                // $msg = 'クレジットの入力に誤りがあります。入力内容をご確認いただくか、別のお支払方法を選択してください。';
                $msg = '不明なエラーが発生しました。ドライバーに確認してください。';
                $msg_cd = '[ ' . $_SESSION['QRErrorCD'] . " ] ERROR MSG=[" . $_SESSION['QRError'] . "]";
                //$this->_QRError;
                // エラーメッセージを作成する
                $errorForm = new Sgmov_Form_Error();
                $errorForm->addError('payment_method', $msg);
                $errorForm->addError('payment_method_cd', $msg_cd);
        ...
        // コミケ申込情報登録とAPI送信処理
        $calcDataInfoData = $this->registerComiket($db, $registerData, $comiketId, $backInputPath, $sessionForm, $session);
        ...
        // $li = $returnResult['li'];
        // $inForm['delivery_charge'] = $inForm['URIAGE_KINGAKU'];
     ...
     * @param string argBackInputPaht
     * @return array inForm
     */
    protected function payment($db, $inForm, $comiketId, $argBackInputPaht, $type = '')
    {
     ...
     * @param array inform
     * @param integer comiketId
     * @param string backInputPath
     * @return array calcDataInfoData
     */
    protected function registerComiket($db, $inForm, $comiketId, $backInputPath, $sessionForm, $session)
    {
        $db->begin();
        try {
            ...
            $comiketDataInfoFlat["comiketData"]['delivery_slip_no'] = $inForm["delivery_slip_no"];
            //QRコードのデータもまるごと保存
            $comiketDataInfoFlat["comiketData"]['qr_uketsuke_no'] = $inForm["qr_uketsuke_no"];
            $comiketDataInfoFlat["comiketData"]['qr_toiawase_no'] = $inForm["qr_toiawase_no"];
            $comiketDataInfoFlat["comiketData"]['qr_ark_uketsuke_no'] = $inForm["qr_ark_uketsuke_no"];
            $comiketDataInfoFlat["comiketData"]['qr_kessai_meisai_id'] = $inForm["qr_kessai_meisai_id"];
            $comiketDataInfoFlat["comiketData"]['qr_uriage_kingaku'] = $inForm["qr_uriage_kingaku"];
            $comiketDataInfoFlat["comiketData"]['qr_system_kbn'] = $inForm["qr_system_kbn"];
            $comiketDataInfoFlat["comiketData"]['qr_cd'] = $inForm["qr_cd"];
            //顧客区分カラムに決済明細IDを保存（TOIAWASE_NO＋KESSAI_MEISAI_IDで一意）
            $comiketDataInfoFlat["comiketData"]['customer_kbn'] = $inForm["qr_kessai_meisai_id"];

            //重複チェックはindex.phpに移動させた。2022.03.29
            // ※重複登録のチェック時に使用するために保存しておく(ARKの重複チェックは未配慮：未実装)
            // $QR_Check = $this->_Comiket->fetchQRById($db, $inForm["qr_toiawase_no"], $inForm["qr_kessai_meisai_id"]);
            // if (!$QR_Check) {
            //     $msg = '既に登録済みの決済コードです。';
            //     // エラーメッセージを作成する
            //     $errorForm = new Sgmov_Form_Error();
            ...
            //     $sessionForm->status = self::VALIDATION_FAILED;
            //     $session->saveForm(self::FEATURE_ID, $sessionForm);
            //     Sgmov_Component_Redirect::redirectPublicSsl("/" . $this->_DirDiv . "/qr_err");
            //     exit;
            // }
            ...
            //QR決済 プロシージャ新設（TOIBANをdelivery_slip_noに保存）

            //1．問い番をSLIP_NOカラムに保存する場合は下記だがQRコード全部保存する仕様に変わったのでさらに追加
            // $this->_Comiket->insert_add_delivery_slip_no($db, $comiketDataInfoFlat["comiketData"]);

            //2．QRコードのパラメータを全て保存する為カラムを追加したのでそこに保存するプロシージャ
            $this->_Comiket->insert_add_qrcode($db, $comiketDataInfoFlat["comiketData"]);
            //※他のカラムはほぼ不要だが細かい調査をしていないので冗長なロジックになっているので整理する必要あり

            // API送信
            $contents_array = $this->postApiRequest($inForm, $comiketId);
            //$contents_arrayはオブジェクトなので中身へのアクセスは$contents_array->resultでアクセスしてください。
            //APIからのResultを書き込んでおく 流れで保存するだけ ARKは不明
            $this->_Comiket->updateQR_Result($db, $contents_array, $inForm);
            // TODO VASI
            // $comiketIds = array();
            ...
            /** コミケ申込関連データ **/
            $comiketDataInfoFlat["comiketDetailDataList"]["0"]["toiawase_no"] = $inForm["comiket_toiawase_no"];
            $comiketDataInfoFlat["comiketDetailDataList"]["0"]["toiawase_no_niugoki"] = $inForm["comiket_toiawase_no_niugoki"];
            ...
            $db->rollback();
            $_SESSION["Sgmov_View_Qra.inputErrorInfo"] = array("db_insert_error" => "・データベースの登録・更新に失敗しました。");
            Sgmov_Component_Log::debug("リダイレクト " . $this->_DirDiv . "/{$backInputPath}/");
    ...
    protected function sendDataToGyomu($db, $inForm, $comiketId, $calcDataInfoData, $backInputPath, $sessionForm, $session, $toiawaseNo)
    {
        ...
        Sgmov_Component_Log::debug('業務連携バッチ実行開始');
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////
        // オンラインバッチ起動（業務連携）
        $bcmResult = $this->_BcmView->execute($comiketId);

        Sgmov_Component_Log::debug('業務連携バッチ実行終了');
    ...
    public function errorInformationForSgFinancial($param)
    {
        ...
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($param, dirname(__FILE__) . '/../../mail_template/Rms_error_for_sgfinancial.txt', $mail_to);
    }
    ...
    public function _createDataByInForm($db, $inForm, $toiawaseNo = '0123456789')
    {
        ...
        $inForm['delivery_slip_no'] = $toiawaseNo;
        return $inForm;
    }
    ...
    public function _createCheckCreditCardDataByInForm($inForm, $type)
    {
        ...
        // パラメータからの金額を決済様に最終同期
        $inForm['delivery_charge'] = $inForm['qr_uriage_kingaku'];
        $data->setAmount(strval($inForm['qr_uriage_kingaku']));

        // カード番号
        $data->setCardNumber($inForm['card_number']);

        // カード有効期限 MM/YY
        if (self::VERITRANS_ERRKEY == '') {
            $cardExpire = $inForm['card_expire_month_cd_sel'] . '/' . substr($inForm['card_expire_year_cd_sel'], -2);
        } else {
            $cardExpire = $this->VERITRANS_ERRNUM[self::VERITRANS_ERRKEY];
        }
...
*/
    ...
    public function errorInformation($parm = array())
    {
        ...
        $mail_to = Sgmov_Component_Config::getLogMailTo();
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($parm, dirname(__FILE__) . '/../../mail_template/Qra_error.txt', $mail_to);
    }

    /**
     * 決済用データの入力値の妥当性検査を行います。
     * @param $checkForm 決済用データ
     * @param Sgmov_Form_Qra003In $inForm 入力フォーム
     * @return Sgmov_Form_Error エラーフォームを返します。
     */
    public function _transact($checkForm, $inForm)
    {
        Sgmov_Component_Log::debug($checkForm);
        Sgmov_Component_Log::debug($inForm);
        // VeriTrans3G MerchantDevelopmentKitマーチャントCCID、マーチャントパスワード設定
        switch ($inForm['comiket_payment_method_cd_sel']) {
            case '2': // クレジットカード決済
                $props = array(
                    'merchant_ccid'       => Sgmov_Component_Config::getQrhikkoshiMdkCreditCardMerchantCcId(),
                    'merchant_secret_key' => Sgmov_Component_Config::getQrhikkoshiMdkCreditCardMerchantSecretKey(),
                );
                break;
            // コンビニ決済はしない
//            case '1': // コンビニ決済
...
//                break;
        ...
        //この$responseの中身をエラー表示に使う
        $_SESSION['QRError'] = $response->getmerrMsg();
        $_SESSION['QRErrorCD'] = $response->getvResultCode();
    ...
    /**
     * 業務連携でWebAPIにPOST送信実行
     * QR決済完了通知
     *
     * ここでAPIに4つのパラメータを送信してリザルトを受け取る
     * リザルトを受け取っても画面表示はしない
     *
     * result：success/failure
     * error：エラー内容文字列
     *
     * これをDBに保存しておくかで保存先は未定
     * とりあえず、基本的に成功するので何かPHP側でやる必要はなさそう
     * しいて言えばエラーの場合、メールでも送信するくらいで何も出来ない
     */
    public function postApiRequest($inForm, $comiketId)
    {

        // APIのURLを取得
        $apiUrl = $this->_setApiUrl($inForm['qr_env'], $inForm['qr_system_kbn']);
        // URLが取得できなかった場合
        if(empty($apiUrl)){
            ...
            // API失敗の場合エラーメールを飛ばす
            $this->failedApiMail($inForm, $comiketId, $contents_array);
        ...
        //APIに送信するパラメータを設定
        // MV-BASEの場合
        if($inForm['qr_system_kbn']==self::KBN_MV_BASE){
            // APIに送信するパラメータ
            $param = array(
                'uketsuke_no'         => $inForm['qr_uketsuke_no'],
                'toiawase_no'         => $inForm['qr_toiawase_no'],
                'kessai_meisai_id'    => $inForm["qr_kessai_meisai_id"],
                'veritrans_kessai_id' => $inForm["payment_order_id"]
            );
        }
        // SG-ARKの場合
        else if($inForm['qr_system_kbn']==self::KBN_SG_ARK){

            // APIのURLにログインID、パスワードをセット
            $loginAccount = array(
                "userid"   => Sgmov_Component_Config::getWsUserId(),
                "password" => Sgmov_Component_Config::getWsPassword()
            );
            ...
            $loginStr  = http_build_query($loginAccount, "", "&");
            $loginStr .= '&query_request='.json_encode($param);
            $apiUrl.='?'.$loginStr;
        ...
        //リクエスト実行
        $contents = @file_get_contents($apiUrl, false, stream_context_create($options));
        ...
        // 何か値がセットされていればその値を優先
            // MV-BASEの場合
            if($inForm['qr_system_kbn']==self::KBN_MV_BASE){
                if($contents_array->result != 'success'){
                    if(empty($contents_array->error)){
                        $contents_array->error='不明なエラー';
                    }
                }
            }
            // SG-ARKの場合
            else if($inForm['qr_system_kbn']==self::KBN_SG_ARK){
                if($contents_array->result != '0'){
                    $msgList=array(
                      '0'=>'正常',
                      '1'=>'認証エラー',
                      '2'=>'システムエラー',
                    ...
                    $responsCode=$contents_array->result;
                    $contents_array->error='不明なエラー';
                    if(isset($msgList[$responsCode])){
                        $contents_array->error=$msgList[$responsCode];
                    }
                }
            }
        ...
        if(!empty($contents_array->error)){
            ...
            $contents_array = (object)[
                                         'result'=>'error'
                                        ,'error' =>'APIのHTTPステータスが'.$statusCode.'です'
                                    ];
            $this->failedApiMail($inForm, $comiketId, $contents_array);
        }

        Sgmov_Component_Log::debug("API実行：" . $apiUrl);
        Sgmov_Component_Log::debug("API送信パラメータ：" . json_encode($param));
        Sgmov_Component_Log::debug("API結果ステータス：" . $statusCode);
        Sgmov_Component_Log::debug("API結果：" . $contents_array->result);
