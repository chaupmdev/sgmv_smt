 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Twi_CheckInput extends Sgmov_View_Twi_Common {
    ...
    public function executeInner() {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_TWI001, $this->_getTicket());
...
//        $inForm->comiket_detail_inbound_note = mb_convert_kana(filter_input(INPUT_POST, 'comiket_detail_inbound_note'), 'RNASKV', 'UTF-8');
    ...
    public function _validate($inForm, $db) {
        ...
        if(empty($inForm->event_sel)) {
            $errorForm->addError('event_sel', 'イベントが選択されていません。');
            return $errorForm;
        }
        ...
        if (!$v->isValid()) {
            $errorForm->addError('event_sel', 'イベント' . $v->getResultMessageTop());
        }

        // イベントサブ
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->eventsub_sel)->isSelected()->isIn($eventsubArray['ids']);
        if (!$v->isValid()) {
            $errorForm->addError('event_sel', 'イベントサブ' . $v->getResultMessageTop());
        }
        ...
        if(!empty($eventsubInfo) && $eventsubInfo['booth_display'] ==  '1') {
            ...
            if (!$v->isValid()) {
                $errorForm->addError('comiket_booth_name', '出展者名' . $v->getResultMessageTop());
            }
        }
        ...
        if(!empty($eventsubInfo) && $eventsubInfo['building_display'] == '1') {
            ...
            if ($inForm->event_sel != '2') {
                ...
                if (!$v->isValid()) {
                    $errorForm->addError('building_name_sel', 'ブース名' . $v->getResultMessageTop());
                }
            }
            ...
            if (!empty($inForm->building_name_sel)) {
                ...
                if(!$checkFlg) {
                    $errorForm->addError('building_name_sel', '選択ブース名に該当するデータが存在しません。');
                }
            }
