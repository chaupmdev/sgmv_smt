...
Sgmov_Lib::useView('pct/Common', 'CommonConst');
Sgmov_Lib::useForms(array('Error', 'PctSession', 'Pcr004Out'));
 ...
 * @copyright  2009-2010 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Pct_Complete extends Sgmov_View_Pct_Common {
...
//            $inForm['payment_url'] = null;
        ...
        if (!empty($inForm['mail']) && !empty($inForm['merchant_result']) && ($inForm['req_flg'] == self::PCR_WEB_REQUEST || $inForm['payment_method_cd_sel'] == '1')) {
            ...
            switch ($inForm['terminal_cd_sel']) {
                case '1':
                    $mailTemplate = $inForm['req_flg'] == self::PCR_WEB_REQUEST ? '/pct/pct_user_departure.txt' : '/pct/pct_user_departure_ivr.txt';
                    break;
                case '2':
                    $mailTemplate =  $inForm['req_flg'] == self::PCR_WEB_REQUEST ? '/pct/pct_user_arrival.txt' : '/pct/pct_user_arrival_ivr.txt';
                    break;
                case '3':
                default:
                    $mailTemplate = $inForm['req_flg'] == self::PCR_WEB_REQUEST ? '/pct/pct_user.txt' : '/pct/pct_user_ivr.txt';
                    break;
            }
...
*/
    ...
    public function _createMailDataByInForm($db, $inForm, $id) {
        ...
        $data = array(
            ...
            'amount'                   => '\\' . number_format(ceil((string)($inForm['delivery_charge'] / Sgmov_View_Pct_Common::CURRENT_TAX))),
...
*/
    ...
    public function errorInformation($parm = array())
    {
        ...
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($parm, dirname(__FILE__) . '/../../mail_template/pct/pct_error.txt', $mail_to);
    }
