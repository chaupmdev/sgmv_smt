 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Rms_CheckCreditCard extends Sgmov_View_Rms_Common {
    /**
     * コンストラクタでサービスを初期化します。
     */
    ...
    public function executeInner() {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_RMS002, $this->_getTicket());
    ...
    protected function redirectProc($errorForm) {
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/rms/credit_card');
        } else {
            Sgmov_Component_Redirect::redirectPublicSsl('/rms/confirm');
        }
    }
