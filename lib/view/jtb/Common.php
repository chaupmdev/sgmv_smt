 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_JTB_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'JTB';
    // 2022-03-10 ToanDD3 implement SMT6-69
    const LOGIN_ID = 'JTB_LOGIN';
     ...
     * イベントサブID
     */
    const EVENT_SUB_ID = '1901';

    /**
     * JTB001の画面ID
     */
    const GAMEN_ID_JTB001 = 'JTB001';

    /**
     * JTB002の画面ID
     */
    const GAMEN_ID_JTB002 = 'JTB002';

    /**
     * JTB003の画面ID
     */
    const GAMEN_ID_JTB003 = 'JTB003';
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    protected function createOutFormByInForm($inForm, $outForm = array()) {
        ...
        $outForm->raw_comiket_detail_caremark_flg  = @$inForm['comiket_detail_caremark_flg'];
        $outForm->raw_comiket_detail_caremark_flg_enable  = @$inForm['comiket_detail_caremark_flg_enable'];
        
        //GiapLN add 梱包ガイドライン 2022.07.14
        $outForm->raw_comiket_detail_caremark_flg_inbound  = @$inForm['comiket_detail_caremark_flg_inbound'];
        $outForm->raw_comiket_detail_caremark_flg_enable_inbound  = @$inForm['comiket_detail_caremark_flg_enable_inbound'];
        ...
        //GiapLN hard code item 旅館名  
        $arrBoots = ['1' => '旅館名1', '2' => '旅館名2', '3' => '旅館名3', '4' => '旅館名4' , '5' => '旅館名5'];
        $outForm->raw_comiket_booth_cds  = array_keys($arrBoots);
        $outForm->raw_comiket_booth_lbls = array_values($arrBoots); 
        $outForm->raw_comiket_booth_cd_sel = '';
        
        $outForm->raw_comiket_box_num = $inForm["comiket_box_num"];
  
        $outForm->raw_comiket_booth_name = "";
        if(@!empty($inForm["comiket_booth_cd_sel"])) {
            $outForm->raw_comiket_booth_name = $arrBoots[$inForm["comiket_booth_cd_sel"]];
            $outForm->raw_comiket_booth_cd_sel = $inForm["comiket_booth_cd_sel"];
        }
        $arrTravelDelivery = ['1' => '出発地1（出発日時：2023年3月15日）', '2' => '出発地2（出発日時：2023年3月20日）'];
        $outForm->raw_travel_departure_cds  = array_keys($arrTravelDelivery); 
        $outForm->raw_travel_departure_lbls = array_values($arrTravelDelivery);
        $outForm->raw_travel_departure_cd_sel = '';
        
        //travel_departure_cd_sel
        $outForm->raw_travel_departure_nm = '';
        if(@!empty($inForm["travel_departure_cd_sel"])) {
            $outForm->raw_travel_departure_nm = $arrTravelDelivery[$inForm["travel_departure_cd_sel"]];
        ...
        $arrSTTime = ['1' => '12:00～14:00', '2' => '14:00～16:00', '3' => '16:00～18:00', '18:00～21:00'];
        $outForm->raw_cargo_collection_st_time_cds  = array_keys($arrSTTime); 
        $outForm->raw_cargo_collection_st_time_lbls = array_values($arrSTTime);
        $outForm->raw_cargo_collection_st_time_cd_sel = '';
        
        //travel_departure_cd_sel
        $outForm->raw_cargo_collection_st_time_nm = '';
        if(@!empty($inForm["cargo_collection_st_time_cd_sel"])) {
            $outForm->raw_cargo_collection_st_time_nm = $arrSTTime[$inForm["cargo_collection_st_time_cd_sel"]];
            $outForm->raw_cargo_collection_st_time_cd_sel = $inForm["cargo_collection_st_time_cd_sel"];
            
        }
        
        
        $outForm->raw_delivery_collection_date_year_cd_sel  = $inForm['delivery_collection_date_year_cd_sel'];
        $outForm->raw_delivery_collection_date_month_cd_sel = $inForm['delivery_collection_date_month_cd_sel'];
        $outForm->raw_delivery_collection_date_day_cd_sel   = $inForm['delivery_collection_date_day_cd_sel'];
        $outForm->raw_delivery_collection_date    = $this->_appCommon->getYmd($inForm['delivery_collection_date_year_cd_sel'] . $inForm['delivery_collection_date_month_cd_sel'] . $inForm['delivery_collection_date_day_cd_sel']);
...
//echo '</pre>';
        ...
        foreach($buildingListByEventsubId as $key => $val) {
            ...
            // コンボの表示をここで制御
            $boothPostionLbls[] = $val['name'].$val['booth_position'];
        }
...
//        $outForm->raw_building_booth_id_sel = $inForm["building_booth_id_sel"];
        ...
        //// 一番小さいプラス期間(plus_period)と該当レコードを抽出する
        ...
        //// 往路・搬入預かりの開始日を「アクセス日時が12:00まで+1日」、「~アクセス日時が以降+2日」の算出
        ...
        //// '1900-01-01'は往路・搬入預かりが終了した場合は固定でセットしている
        //if ($outBoundColCalFromToData['frDate'] == '1900-01-01'
        //        && $outBoundColCalFromToData['toDate'] == '1900-01-01') {
        //    // 終了の場合下記値をunsetすると、搬入のラジオボタンを強制的に非表示にできる
        //    // マスタデータで制御するので、緊急対応でなければこのソースはいじらない
        //    // unset($this->comiket_detail_type_lbls['1']); 

        //    // 搬入のみの場合は終了画面にリダイレクトする
        ...
        foreach ($timeDataList as $timeData) {
            ...
            // キャンセル画面にお届け指定日表示する。
            if($inForm["comiket_detail_inbound_delivery_time_sel"] == $timeData['name']){
                $inForm["comiket_detail_inbound_delivery_time_sel"] = $timeData['cd'] .','. $timeData['name'];
            }
        }
...
//        $outForm->raw_comiket_detail_inbound_note = $inForm["comiket_detail_inbound_note"];
    ...
    public function setDispEvent(&$dispItemInfo){
        ...
        foreach($dispItemInfo['event_alllist'] as $k=>$v){
            if(isset($dispItemInfo['eventsub_selected_data']['event_id']) && $v['id']==$dispItemInfo['eventsub_selected_data']['event_id']){
                    ...
                    $dispItemInfo['dispEvent']['sgName']='ＳＧムービング';
                    // 例)デザインフェスタ55 などお問い合わせ先の箇所などでマスタと微妙に違う文言を設定
                    $dispItemInfo['dispEvent']['customName']=$v['eventsub_name'] .'手荷物配送係';
                    ...
                    // ロゴ画像ファイル名：例)logo.jtb.gif logo_[イベント識別子].gif
                    //$dispItemInfo['dispEvent']['logoImg']   ='/'.$this->_DirDiv.'/images/logo_'.$this->_DirDiv.'.jpg';
                    $dispItemInfo['dispEvent']['logoImg']   ='/'.$this->_DirDiv.'/images/logo_'.$this->_DirDiv.'.png';
                    ...
                    $dispItemInfo['dispEvent']['manual']    ='/'.$this->_DirDiv.'/pdf/manual/manual_'.$v['eventsub_name'].'.pdf'.$cacheStr;
                    
                    // 引き渡し場所や宅配カウンターの場所：例)宅配カウンター.pdf
                    $dispItemInfo['dispEvent']['takuhai']    ='/'.$this->_DirDiv.'/pdf/manual/宅配カウンター.pdf'.$cacheStr;
            }
        }
    }
    ...
    /**
     * 郵便番号dllから入力住所のjis2、jis5コードを決定する
     *
     * @param type $inForm
     */
        ...
        if ($isAmountDataFromSession) {
            ...
            if (!@empty($sessionTableTreeData)) {
                ...
                foreach ($sessionTableTreeData["comiket_detail_list"] as $key => $val) {
                    ...
                    // カーゴ
                    if(!@empty($val["comiket_cargo_list"])) {
                        foreach($val["comiket_cargo_list"] as $keyComiketCargo => $valComiketCargo) {
                            $tableTreeData["comiket_detail_list"][$key]["comiket_cargo_list"][$keyComiketCargo]['fare_amount'] = $valComiketCargo['fare_amount'];
                        }
                    }
                }
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                    ...
                    if(isset($valDet["comiket_box_list"])){
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Jtb_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Jtb_Common::CURRENT_TAX));
                        // // 税込
        ...
        if($inForm['comiket_detail_type_sel'] == "1"
            ...
            if($inForm['comiket_detail_outbound_service_sel'] == "1") { // 宅配
                ...
                foreach($inForm['comiket_box_outbound_num_ary'] as $key => $val) {
                    ...
                    $boxFareData = $this->_BoxFareService->fetchBoxFareByJis2AndBoxId($db, $outbound_hatsu_jis2code, $outbound_chaku_jis2code, $key, $inForm['eventsub_sel']);
                    if(empty($boxFareData)) {
                        $fare = 0;
                    } else {
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($eventsubData['building_display'] !== '0') {
                //GiapLN fix bug send mail item 【ブースNO】 2024.07.09
                $building = '';
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/".$this->_DirDiv."/complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/".$this->_DirDiv."/complete_business_sgmv{$tmplateType}.txt";
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/".$this->_DirDiv."/complete_individual{$tmplateType}.txt";
                $mailTemplateSgmv[] = "/".$this->_DirDiv."/complete_individual_sgmv{$tmplateType}.txt";
                ...
                if($comiket['payment_method_cd'] == '4') { // お支払い方法 = コンビニ後払
                    ...
                    if (@!empty($comiket['sgf_res_shopOrderId'])) {
                        $data['convenience_store_late'] .="【ご購入店受注番号】{$comiket['sgf_res_shopOrderId']}" . PHP_EOL;
                    }

                    if (@!empty($comiket['sgf_res_transactionId'])) {
                        $data['convenience_store_late'] .="【ご購入店受注番号】{$comiket['sgf_res_transactionId']}" . PHP_EOL;
                    }
                }
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/'.$this->_DirDiv.'/parts_complete_choice_2_sgmv.txt';

                    $data['manual_url'] = "【搬出の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$eventsubData['name']}.pdf";
                    ...
                    //$data['manual_url'] = "【搬入の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$comiket['event_id']}.pdf";
                    $data['manual_url'] = "【搬入の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$eventsubData['name']}.pdf";

                    $comiketType = "type1";//搬入
            ...
            // 2022-03-11 ToanDD3 implement SMT6-59
            if ($tmplateType === '_cancel' || $tmplateType === '_sgmv_cancel') {
                //フッター用メールテンプレート
                if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) { // 法人
                    $mailTemplate[] = "/".$this->_DirDiv."/parts{$footerTmpName}_footer_type_1.txt";
                    $mailTemplateSgmv[] = "/".$this->_DirDiv."/parts{$footerTmpName}_footer_type_1.txt";
                } else { // 個人
                    $mailTemplate[] = "/".$this->_DirDiv."/parts{$footerTmpName}_footer_type_2.txt";
                    $mailTemplateSgmv[] = "/".$this->_DirDiv."/parts{$footerTmpName}_footer_type_2.txt";
                }
            } else {
                //セキュリティチェック
                if ($isSecurity) {
                ...
                } else {//ログイン対応無しの時、completeまたはサイズ変更完了
                    $mailTemplate[] = "/common_footer_type_2.txt";
                }
            }
            ...
            // 2022-03-11 ToanDD3 implement SMT6-59
            // if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) { // 法人
            //     $mailTemplate[] = "/jtb_parts{$footerTmpName}_footer_type_1.txt";

            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/jtb_parts{$footerTmpName}_footer_type_1.txt";
            // } else { // 個人
            //     $mailTemplate[] = "/jtb_parts{$footerTmpName}_footer_type_2.txt";

            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/jtb_parts{$footerTmpName}_footer_type_2.txt";
            // }
            ...
            if (!$isSecurity) {
                ...
                if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                    $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm='.$this->_DirDiv; 
                } else {
                    $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=".$this->_DirDiv."&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
                }   
            }           
            ...
            if(($comiket['choice'] == "1" || $comiket['choice'] == "3")
                ...
                } else if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 0){
                    $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/event/robotCheck?event_nm=".$this->_DirDiv."&destination=paste_tag/{$pasteTagId}";
                }
            }
...
";
            ...
            if (!$isSecurity) {
                ...
                $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=".$this->_DirDiv."&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
                $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=".$this->_DirDiv."&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            }
            ...
            if (!$isAttachment) {
                Sgmov_Component_Log::err('Mail teamplate1');
                Sgmov_Component_Log::err($mailTemplate);
                // 申込者へメール
                ...
                if ($sendCc !== null && $sendCc !== '') {
                    Sgmov_Component_Log::err('Mail teamplate11');
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        //$sendFileName = 'NEN_' . date ( 'YmdHis' ) . '.csv';
...
//                "/".$this->_DirDiv."/parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
     ...
     * @return boolean
     */
    protected function isSecurityTaio() {
        $securityPattern = $this->getSecurityPattern();
        $arrSecurityPatterns = array("1", "2", "3");
        //レコードが取得出来ないまたはsecurity_patternがNULLまたは4の場合、セキュリティ対応無し
        if (isset($securityPattern) && !empty($securityPattern) && in_array($securityPattern, $arrSecurityPatterns)) {
            return true;
        }
        return false;
    }
