...
Sgmov_Lib::useServices(array('CruiseRepeater', 'Prefecture', 'Yubin', 'SocketZipCodeDll'
    ...
    , 'AppCommon', 'HttpsZipCodeDll', 'BoxFare', 'CargoFare', 'Comiket', 'EventsubCmb', 'Time', 'CenterMail', 'OutBoundCollectCal', 'ComiketDetail'));
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Gmm_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'GMM';

    // 2022-03-22 ToanDD3 implement SMT6-84
    const LOGIN_ID = 'GMM_LOGIN';

    /**
     * GMM001の画面ID
     */
    const GAMEN_ID_GMM001 = 'GMM001';

    /**
     * GMM002の画面ID
     */
    const GAMEN_ID_GMM002 = 'GMM002';

    /**
     * GMM003の画面ID
     */
    const GAMEN_ID_GMM003 = 'GMM003';
     ...
     */
     protected function setBoxName($dataList){
        ...
        foreach ($dataList as $key => $value) {
            ...
            if(empty($value['name_display']) || is_null($value['name_display'])){
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $outBoundCollectCalData = array();
        
        $outBoundCollectCalDataarrivalDateMax = array();
        ...
        if ($outBoundColCalFromToData['frDate'] == '1900-01-01'
            ...
            if ($inForm["comiket_detail_type_sel"] == '1') {
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error?t={$title}&m={$message}");
                exit;
            } else {
                $dispItemInfo["comiket_detail_type_lbls"] = $this->comiket_detail_type_lbls;
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $months = array('',11);
        $days   = array('','15','16','17','18','19','20');
        
        array_shift($months);
        array_shift($days);
        $outboundColFromDateTime = new DateTime("2019/11/28 00:00:00");
        $outboundColToDateTime = new DateTime("2019/12/01 00:00:00");
        
        $years = array();
        $months = array();
        $days = array();
        
        for ($outboundColFromDateTime2=$outboundColFromDateTime
                ;$outboundColFromDateTime2->format('Y-m-d') !=  $outboundColToDateTime->format('Y-m-d')
                ;$outboundColFromDateTime2->modify('+1 days')) {
            $year = $outboundColFromDateTime2->format('Y');
            $month = $outboundColFromDateTime2->format('m');
            $day = $outboundColFromDateTime2->format('d');
            
            $years2[$year] = $year;
            $months2[$month] = $month;
            $months2[$day] = $day;
            $dispItemInfo['outboundColYears'][$year] = $year;
            $dispItemInfo['outboundColMonths'][$year][$month] =$month;
            $dispItemInfo['outboundColDays'][$year][$month][$day] =$day;
        }
        ...
        if ($inForm['comiket_detail_type_sel'] == "1" || $inForm['comiket_detail_type_sel'] == "2" || $inForm['comiket_detail_type_sel'] == "3") {
            ...
            foreach ($procList as $keyTree => $valTree) {
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Gmm_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Gmm_Common::CURRENT_TAX));
                        // // 税込
        ...
        if($inForm['comiket_detail_type_sel'] == "2"
            ...
            if($inForm['comiket_detail_inbound_service_sel'] == "1") { // 宅配
                ...
                foreach($inForm['comiket_box_inbound_num_ary'] as $key => $val) {
                    ...
                    // バシ　追加　2020/02/26 TODO
                    $inForm["inbound_hatsu_jis2code"] = 0;
                    $inForm["inbound_chaku_jis2code"] = 0;
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($eventsubData['building_display'] !== '0') {
                // vasi 修正　2020/10/27 ゲームマーケット 302
                // if($eventData['id'] === '2' || $eventsubData["id"] == "302") { // コミケ
                //     $data['comiket_building_name'] = PHP_EOL . '【ブース番号】' . $building . $comiket['booth_position'] . " " . $comiket['booth_num'];
                // } else {
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/gmm_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/gmm_complete_business_sgmv{$tmplateType}.txt";
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/gmm_complete_individual{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/gmm_complete_individual_sgmv{$tmplateType}.txt";
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    //搬出用メールテンプレート
                    $mailTemplate[] = '/gmm_parts_complete_choice_2.txt';

                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/gmm_parts_complete_choice_2_sgmv.txt';

                    $data['manual_url'] = "【搬出の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/gmm/pdf/manual/{$eventData['name']}{$eventsubData['name']}搬出.pdf";
                    ...
                    $prefData = $this->_PrefectureService->fetchPrefecturesById($db, $comiket_detail['pref_id']);
                    //搬入用メールテンプレート
                    $mailTemplate[] = '/gmm_parts_complete_choice_1.txt';

                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/gmm_parts_complete_choice_1_sgmv.txt';

                    $data['manual_url'] = "【搬入の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/gmm/pdf/manual/{$eventData['name']}{$eventsubData['name']}搬入.pdf";
            ...
            if ($tmplateType == '_cancel' || $tmplateType == '_sgmv_cancel') {
                $mailTemplate[] = "/gmm_parts{$footerTmpName}_footer_type_1.txt";
            } else {
            ...
            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/gmm_parts{$footerTmpName}_footer_type_1.txt";
            // } else { // 個人
            //     $mailTemplate[] = "/gmm_parts{$footerTmpName}_footer_type_2.txt";

            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/gmm_parts{$footerTmpName}_footer_type_2.txt";
            // }
            ...
            // $data['edit_url'] = $urlPublicSsl . "/gmm/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            // 2022-03-28 ToanDD3 fix SMT6-120
            if ($_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm=gmm'; 
            } else {
                $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=gmm&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
            }

            // if($eventsubData['manual_display'] == '1') { // 説明書表示有無 0：無、1：有
            //     $data['manual_url'] = "【説明書URL】" . PHP_EOL . $urlPublicSsl . "/gmm/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            // }
            ...
            if(($comiket['choice'] == "1" || $comiket['choice'] == "3")
                ...
                // 2022-03-24 ToanDD3 implement SMT6-84
                // $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/gmm/paste_tag/{$pasteTagId}/";
                $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/event/robotCheck?event_nm=gmm&destination=paste_tag/{$pasteTagId}/";
            }
...
";
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // 2022-03-24 ToanDD3 implement SMT6-84
            // $data['cancel_url'] = $urlPublicSsl . "/gmm/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=gmm&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            // 2022-03-24 ToanDD3 implement SMT6-84
            // $data['size_change_url'] = $urlPublicSsl . "/gmm/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=gmm&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
...
//            $eventsubInfo = $this->_EventsubService->fetchEventsubByEventsubId($db, $comiketInfo['eventsub_id']);
            ...
            $mailTemplateList = array(
                "/gmm_cancel_{$divType}_error.txt",
            );
