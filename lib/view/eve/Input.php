    ...
    public function executeInner() {
        ...
        if ($this->isSecurityTaio()) {
            ...
            if (!isset($_SESSION[self::LOGIN_ID]['user_type'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/event/userSelect?event_nm=eve");
            }   
        }
        
        if(@empty($_SERVER["HTTP_REFERER"]) || strpos($_SERVER["HTTP_REFERER"], "/eve/") == false) {
            // セッション情報を破棄
...
///////////////////////////////////////////////////////////////////////////////////////////////////
        if(strlen($param) == 10) {
            Sgmov_Component_Redirect::redirectPublicSsl("/eve/input2_dialog/{$param}");
        }
        ...
        if($input2DialogFlg == "1") {
            ...
            if(!is_numeric($input2DialogComiketId)){
                Sgmov_Component_Redirect::redirectPublicSsl("/eve/temp_error");
                exit;
            }
            ...
            if($comiketData['tel'] == $input2DialogData1Tel
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/eve/input/{$param}");
            } else {
                Sgmov_Component_Redirect::redirectPublicSsl("/eve/temp_error/{$comiketData['event_id']}");
            }
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(@!empty($_SERVER["REQUEST_URI"]) && strpos($_SERVER["REQUEST_URI"], "/eve/input2") !== false && empty($param)) {
        ...
        if (isset($eventsubInfo)) {
            if (isset($eventsubInfo['departure_fr_time'])) {
                ...
                if ($now<$departureFrTo) {
                    ...
                    $message = urlencode("{$eventName}{$eventsubInfo['name']}の宅配便のお申込は {$departureFrTime} から開始となります。今しばらくお待ちください。");
                    Sgmov_Component_Redirect::redirectPublicSsl("/eve/error?t={$title}&m={$message}");
                    exit;
                }
            } else {
