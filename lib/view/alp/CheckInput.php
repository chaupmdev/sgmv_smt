...
// REWRITE: Eve001Out→Alp001Out Eve002In→Alp002Int
// _createOutFormByInForm($inForm, $param=NULL)内の記述も
Sgmov_Lib::useForms(array('Error', 'AlpSession', 'Alp001In', 'Alp002In'));
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Evp_CheckInput extends Sgmov_View_Evp_Common {
    ...
    public function executeInner() {
        Sgmov_Component_Log::debug('################# DDDDDDDDDDDD-3');
        ...
        if (empty($sessionForm)) {
            $sessionForm = new Sgmov_Form_AlpSession();
            $sessionForm->in = null;
        }
    ...
    public function _redirectProc($inForm, $errorForm) {
        ...
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/'.$this->_DirDiv.'/input/'.$inForm->eventsub_sel);
     ...
     * @param Sgmov_Form_Alp002In $creditCardForm 入力フォーム
     * @return Sgmov_Form_Alp001In 入力フォーム
     */
    public function _createInFormFromPost($post, $creditCardForm) {
        ...
        $inForm = new Sgmov_Form_Alp002In();
        ...
        $inForm->comiket_detail_service_selected = filter_input(INPUT_POST, 'comiket_detail_service_selected');
        ...
        Sgmov_Component_Log::debug('################# DDDDDDDDDDDD-4');
        Sgmov_Component_Log::debug($_POST);

        $inForm->building_cd_and_name_sel = filter_input(INPUT_POST, 'building_cd_and_name_sel');
        $inForm->building_name = filter_input(INPUT_POST, 'building_cd_and_name_sel');
...
// REWRITE 記事欄追加
        $inForm->comiket_detail_inbound_kijiran_cd_sel1 = filter_input(INPUT_POST, 'comiket_detail_inbound_kijiran1');
        $inForm->comiket_detail_inbound_kijiran_cd_sel2 = filter_input(INPUT_POST, 'comiket_detail_inbound_kijiran2');
        $inForm->comiket_detail_inbound_kijiran_cd_sel3 = filter_input(INPUT_POST, 'comiket_detail_inbound_kijiran3');
        $inForm->comiket_detail_inbound_kijiran4        = mb_convert_kana(filter_input(INPUT_POST, 'comiket_detail_inbound_kijiran4'), 'RNASKV', 'UTF-8');
    ...
    public function _validate($inForm, $db) {
        ...
        // サービス選択
        // $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_service_selected)->isSelected()->isIn(array_keys($this->comiket_detail_searvice_selected_lbls));
        // if (!$v->isValid()) {
        //     $errorForm->addError('service_selected', 'サービス選択' . $v->getResultMessageTop());
        // }
        ...
        if($inForm->comiket_div == self::COMIKET_DEV_BUSINESS) { // 法人
            ...
            // 顧客名
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->office_name)->isNotEmpty()->isLengthLessThanOrEqualTo(16)->isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
            ...
            // お名前 姓
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_personal_name_sei)->isNotEmpty()->isLengthLessThanOrEqualTo(8)->isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_personal_name-seimei', 'お名前' . $v->getResultMessageTop());
            }

            // お名前 名 （法人の場合もあるため、必須チェックは外す）
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_personal_name_mei)->isNotEmpty()->isLengthLessThanOrEqualTo(8)->isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_personal_name-seimei', 'お名前' . $v->getResultMessageTop());
            }
        }
        ...
        // sktカスタム ブース名は表示しないが、画面で保持しているので、念のためチェックする
...
//            if (!$v->isValid()) {
//                $errorForm->addError('comiket_booth_name', 'お店名' . $v->getResultMessageTop());
//            }
//        }
        // 未入力と使用不可文字のみチェック
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_booth_name)->isNotEmpty()->isLengthLessThanOrEqualTo(50)->isWebSystemNg();
        if (!$v->isValid()) {
            $errorForm->addError('comiket_booth_name', 'お店名' . $v->getResultMessageTop());
        }
...
//                 }
//             }
// Sgmov_Component_Log::debug('################# DDDDDDDDDDDD-1');

//             if (!empty($inForm->building_name_sel)) {
// Sgmov_Component_Log::debug('################# DDDDDDDDDDDD-2');
// Sgmov_Component_Log::debug($inForm);
// Sgmov_Component_Log::debug($buildingNameInfoAry);
//                 $checkFlg = FALSE;
...
//                         if(!$v->isValid()) {
// Sgmov_Component_Log::debug('################# DDDDDDDDDDDD-5');
//                             $errorForm->addError('building_name_sel', 'ブースNO' . $v->getResultMessageTop());
//                         }
//                         $checkFlg = TRUE;
//                         break;
...
//
//                // building_booth_position_sel
//                $buildingCdAndName = $inForm->building_cd_and_name_sel;
//
//                $buildingCdAndNameArray = explode(',', $buildingCdAndName);
//                $buildingCd = '';
//                if (@!empty($buildingCdAndNameArray['0'])) {
//                    $buildingCd = $buildingCdAndNameArray['0'];
//                }
//                $buildingName = '';
//                if (@!empty($buildingCdAndNameArray['1'])) {
//                    $buildingName = $buildingCdAndNameArray['1'];
//                }
//                $buildingDataList = $this->_BuildingService->getBoothPositionByCdAndName($db, $buildingCd, $buildingName, $inForm->eventsub_sel);
//                $idList = array();
//                foreach ($buildingDataList as $key => $val) {
//                    $idList[] = $val['id'];
//                }
//                Sgmov_Component_Log::debug('################# EEEEEEEEEEEEEEE-1');
//                Sgmov_Component_Log::debug($inForm->building_booth_position_sel);
//                Sgmov_Component_Log::debug($idList);
//                $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->building_booth_position_sel)->isSelected()->isIn($idList);
//                if (!$v->isValid()) {
//                    $errorForm->addError('building_name_sel', 'ブース番号' . $v->getResultMessageTop());
//                }
...
// Sgmov_Component_Log::debug('################# CCCCCCCCCCCCC-4');
//                     $errorForm->addError('building_name_sel', '選択ブース位置に該当するデータが存在しません。');
//                 }
            // }

            // イベント固有実装：ブース番号のチェックなし
            //// ブース番号-テキスト
            //$v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_booth_num)
            //        ->isNotEmpty()
            //        ->isHalfWidthAlphaNumericCharacters()
...
//        }
        ...
        // 手荷物預かりサービス
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //if($inForm->comiket_detail_service_selected == '2') {
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // 荷物預り-取出回数
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_azukari_kaisu_type_sel)->isSelected()->isIn(array_keys($this->comiket_detail_searvice_selected_lbls));
            // if (!$v->isValid()) {
            //     $errorForm->addError('comiket_staff_tel', '荷物預り-取出回数' . $v->getResultMessageTop());
            // }
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // 荷物預り-氏名
            // $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_azukari_one)->isNotEmpty()->isLengthLessThanOrEqualTo(32)->
            //         isNotHalfWidthKana()->isWebSystemNg();
            // if (!$v->isValid()) {
            //     $errorForm->addError('comiket_detail_azukari_one', '荷物預り-氏名' . $v->getResultMessageTop());
            // }
            
            // 荷物預り-電話番号
            // $comiektDetailAzukariTel = @str_replace(array('-', 'ー', '−', '―', '‐', 'ｰ'), "", $inForm->comiket_detail_azukari_tel);
            // $v = Sgmov_Component_Validator::createSingleValueValidator($comiektDetailAzukariTel)->isNotEmpty()->isPhoneHyphen();
            // if (!$v->isValid()) {
            //     $errorForm->addError('comiket_detail_azukari_tel', '荷物預り-TEL' . $v->getResultMessageTop());
            // } else {
            //     $v = Sgmov_Component_Validator::createSingleValueValidator($comiektDetailAzukariTel)->isLengthMoreThanOrEqualTo(8)->isLengthLessThanOrEqualTo(12);
            //     if (!$v->isValid()) {
            //         $errorForm->addError('comiket_detail_azukari_tel', '荷物預り-TELの数値部分' . $v->getResultMessageTop());
            //     }
            // }
        //}
...
//            // sktカスタム
//            // 集荷先の都道府県で申込終了日を切り替える
//            // 受付終了基準日
//            $baseDay=21;
//            // 受付終了時間
//            $ltime=array('h'=>12,'m'=>0, 'd'=>0);
//
//             //デバッグ用
//          // $baseDay=date("d",strtotime("-1 day"));
//          // $ltime=array('h'=>20,'m'=>56,'d'=>0);
//            // 発送する都道府県で終了日を切り替える
//            switch($inForm->comiket_detail_outbound_pref_cd_sel){
//                // 受付終了日+0日：沖縄
//                case '47':
//                    if(time()>mktime($ltime['h'],$ltime['m'],$ltime['d'],4,$baseDay,2022)){
//                        $errorForm->addError('comiket_detail_outbound_pref', '搬入-お預かり日時:受付時間は終了しました。');
//                    }
//                    break;
//                // 受付終了日+1日：北海道、青森、中国、四国、九州
//                case '01': case '02': case '31': case '32': case '33': case '34': case '35': 
//                case '36': case '37': case '38': case '39': case '40': case '41': case '42': 
//                case '43': case '44': case '45': case '46':
//                    if(time()>mktime($ltime['h'],$ltime['m'],$ltime['d'],4,$baseDay+1,2022)){
//                        $errorForm->addError('comiket_detail_outbound_pref', '搬入-お預かり日時:受付時間は終了しました。');
//                    }
//                break;
//                // 受付終了日+2日：上記以外
//                default:
//                    if(time()>mktime($ltime['h'],$ltime['m'],$ltime['d'],4,$baseDay+2,2022)){
//                        $errorForm->addError('comiket_detail_outbound_pref', '搬入-お預かり日時:受付時間は終了しました。');
//                    }
//                break;
//            }
//
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($comiektDetailOutboundTel)->isNotEmpty()->isPhoneHyphen();
            if (!$v->isValid()) {
    ...
    public function _checkInbound($inForm, &$errorForm) {
        ...
        if($inForm->comiket_detail_type_sel == "2" || $inForm->comiket_detail_type_sel == "3") { // 搬出の場合
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_building)->isNotEmpty()->isLengthLessThanOrEqualTo(30)->
                    isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_inbound_building', '搬出-配送先番地・建物名・部屋番号' . $v->getResultMessageTop());
            } else {
                $prefName = @$prefectures["names"][$prefectures["ids"][$inForm->comiket_detail_inbound_pref_cd_sel]];
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($comiektDetailInboundTel)->isNotEmpty()->isPhoneHyphen();
            if (!$v->isValid()) {
