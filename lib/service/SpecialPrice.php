 ...
 * @copyright  2009-2010 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_Service_SpecialPrice
{
    /**
     * 特価区分：閑散繁忙期設定
     */
         ...
         $toAreaIds, $targetDays, $specialPriceDetails = NULL)
    {
        ...
        if (!$this->_updateSpecialPrice($db, $id, $timestamp, $specialPrice, $userAccount)) {
            Sgmov_Component_Log::debug('登録に失敗しました。');
            $db->rollback();
            return FALSE;
        }
     ...
     * @param string $userAccount ユーザーアカウント
     */
    public function _insertSpecialPrice($db, $id, $specialPrice, $userAccount) {
        $params = array(
            $id,
            $specialPrice['draft_flag'],
        ...
        $query .= '            ,draft_flag';
        $query .= '            ,center_id';
        $query .= '            ,special_price_division';
        $query .= '            ,title';
        $query .= '            ,description';
        $query .= '            ,create_user_name';
        $query .= '            ,min_date';
        $query .= '            ,max_date';
        $query .= '            ,priceset_kbn';
        $query .= '            ,batchprice';
        $query .= '            ,create_user_account';
        $query .= '            ,modify_user_account';
        $query .= '            ,created';
        $query .= '            ,modified';
        $query .= '        )';
        $query .= '    VALUES';
        ...
        Sgmov_Component_Log::debug("####### START INSERT special_prices #####");
        $db->executeUpdate($query, $params);
        Sgmov_Component_Log::debug("####### END INSERT special_prices #####");
    }
     ...
     */
    public function _putSpecialPriceDetailsIndividualPrice($db, $specialPriceId, $coursePlanIds, $fromAreaIds, $toAreaIds, $prices) {
     ...
     *
     * @param Sgmov_Component_DB $db DB接続
     * @param string $kind 特価区分(1:閑散・繁忙期料金設定、2:キャンペーン設定)
     * @param string $status 状況(1:公開、2:下書き、3:終了)
     * @return array 特価基本情報の配列
     * array(
     * ['id'] 特価ID
     * ['center_id'] 担当拠点ID
     * ['center_name'] 担当拠点名
     ...
     * )
     */
    public function fetchSpecialPricesByStatus($db, $kind, $status) {
        $query = 'SELECT';
        $query .= '        A.id AS id';
        $query .= '        ,A.center_id AS center_id';
        $query .= '        ,B.name AS center_name';
        ...
        if ($kind === '1') {
            // 閑散・繁忙期料金設定
            $query .= '        A.special_price_division = \'1\'';
        } else if ($kind === '2') {
            // キャンペーン設定
            $query .= '        A.special_price_division = \'2\'';
        } else {
            Sgmov_Component_ErrorExit::errorExit(Sgmov_Component_ErrorCode::ERROR_SYS_ASSERT, '特価区分が不正です。$kind=' . $kind);
        }
        if ($status === '1') {
            // 公開
            $query .= '        AND A.max_date >= current_date';
            $query .= '        AND A.draft_flag = \'0\'';
        } else if ($status === '2') {
            // 下書き
            $query .= '        AND A.max_date >= current_date';
            $query .= '        AND A.draft_flag = \'1\'';
        } else if ($status === '3') {
            // 終了
            $query .= '        AND A.max_date < current_date';
        } else {
            Sgmov_Component_ErrorExit::errorExit(Sgmov_Component_ErrorCode::ERROR_SYS_ASSERT, '状況が不正です。$status=' . $status);
        }
...
//    }
     ...
     */
    public static function fetchAllCampain($db, $cnt) {
        $query = "select
                        title,
                        min_date,
                        max_date,
     ...
     *             ['description'] 説明の文字列配列
     */
    public static function fetchSpecificCampain($db, $course_id, $plan_id, $from_area_id, $to_area_id) {
        $query = "
            SELECT
                sp.id,
