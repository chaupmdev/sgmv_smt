...
//                $('.paste_tag_link').attr("href", "/gmm/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf");
//                $('.manual_link').attr("href", "/gmm/pdf/manual/manual_" + eventsubSel + ".pdf");

                $('.manual').show(g_fifoVal);
                eventSelText = eventSelText.replace(/\s+/g, "");
                $('.manual_link').attr("href", "/gmm/pdf/manual/" + eventSelText +  "搬入.pdf" + '?' + strCurrentDate);
            } else {
                $('.manual').hide(g_fifoVal);
            }
            if(g_eventsubList[eventsubSel]['is_paste_display']){
                $('.paste_tag').show(g_fifoVal);
                $('.paste_tag_link').attr("href", "/gmm/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf" + '?' + strCurrentDate);
            } else {
...
//console.log("############### 32");
        ...
        // イベントの選択がコミケならブースNOプルダウンリストを非表示にする
    ...
    function getBoxData() {
        sgwns.api('/common/php/SearchBox.php', getFormData(), (function(data) {
            ...
            if($('input[name="comiket_detail_type_sel"]:checked').val() == "1" ) { // 搬入
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img class='dispSeigyoPC' src='/gmm/images/about_boxsize.png' width='100%'/>";
                } else {
//                    htmlCode += "<img src='/gmm/images/about_boxsize.png' width='100%' style='visibility:hidden;'/>";
                }
                htmlCode += "</td></tr></table>";

                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL && $('.dispSeigyoSP').length === 0) {
                    htmlCode += "<div class='dispSeigyoSP'><img src='/gmm/images/about_boxsize.png' width='250px' style='margin-top: 1em;' /></div>";
                }
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img src='/gmm/images/about_boxsize.png' width='100%'/>";
                }
                htmlCode += "</td></tr></table>";
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode2 += "<img src='/gmm/images/about_boxsize.png' width='100%'/>";
                }
