 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Tms_Input extends Sgmov_View_Tms_Common {
    ...
    public function executeInner() {
        ...
        if (!isset($_SESSION[self::LOGIN_ID]['user_type'])) {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl('/event/userSelect?event_nm=tms');
        }
        if(@empty($_SERVER["HTTP_REFERER"]) || strpos($_SERVER["HTTP_REFERER"], "/tms/") == false) {
            // セッション情報を破棄
...
//Sgmov_Component_Log::debug($input2DialogComiketId);
            if(!is_numeric($input2DialogComiketId)){
                Sgmov_Component_Redirect::redirectPublicSsl("/tms/temp_error");
                exit;
            }
...
//Sgmov_Component_Log::debug($input2DialogData1Tel);
            ...
            if($comiketData['tel'] == $input2DialogData1Tel
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/tms/input/{$param}");
            } else {
                Sgmov_Component_Redirect::redirectPublicSsl("/tms/temp_error/{$comiketData['event_id']}");
            }
        }
...
Sgmov_Component_Log::debug("########################### 649-4");
        if(@!empty($_SERVER["REQUEST_URI"]) && strpos($_SERVER["REQUEST_URI"], "/tms/input2") !== false && empty($param)) {
...
Sgmov_Component_Log::debug('##################### CCCCC');
