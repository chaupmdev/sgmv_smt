...
Sgmov_Lib::useView('gmm/SizeChangeConfirm');

/**#@-*/

// 処理を実行
$view = new Sgmov_View_Gmm_SizeChangeConfirm();
$forms = $view->execute();
...
?>
    ...
    <div id="main">
        <div class="wrap clearfix">
            ...
            <div class="section">
                <form action="/gmm/size_change_complete" data-feature-id="<?php echo Sgmov_View_Gmm_Common::FEATURE_ID ?>" data-id="<?php echo Sgmov_View_Gmm_Common::GAMEN_ID_GMM001 ?>" method="post">
...
<?php
        echo Sgmov_View_Gmm_Confirm::_getTimeFormatSelectPulldownData($eveOutForm->comiket_detail_outbound_delivery_time_cds(), $eveOutForm->comiket_detail_outbound_delivery_time_lbls(), $eveOutForm->comiket_detail_outbound_delivery_time_sel());
?>
                                    </dd>
                                </dl>
...
<?php if($eveOutForm->comiket_div() == Sgmov_View_Gmm_Common::COMIKET_DEV_INDIVIDUA) : // 個人 ?>
    ...
    <?php if ($eveOutForm->comiket_payment_method_cd_sel() == '2') : // クレジット ?>
                        ...
                        <div class="btn_area">
                            ...
                            <?php if (@empty($dispItemInfo['is_cancel'])) : ?>
                                <a class="back" href="/gmm/size_change_credit_card/">修正する</a>
                                <input id="submit_button" type="submit" name="submit" value="入力内容を送信する" />
                            <?php else: ?>
                                <input id="submit_button" type="submit" name="submit" value="キャンセル送信する" />
                            <?php endif; ?>
    ...
    <?php if ($eveOutForm->comiket_payment_method_cd_sel() == '3') : // 電子マネー ?>
       ...
       <strong class="red">※電子マネーでのお支払いは、￥10,000を超える場合はお取り扱い出来ません。</strong>
        <div class="btn_area">
            <a class="back" href="/gmm/size_change/">修正する</a>
            <?php if (@empty($dispItemInfo['is_cancel'])) : ?>
                <input id="submit_button" type="submit" name="submit" value="入力内容を送信する" />
            <?php else: ?>
                <input id="submit_button" type="submit" name="submit" value="キャンセル送信する" />
