...
abstract class Sgmov_View_Dsn_Common extends Sgmov_View_Public {
     ...
     * イベントサブID
     */
    const EVENT_SUB_ID = '1301';

    /**
     * DSN001の画面ID
     */
    const GAMEN_ID_HOT001 = 'HOT001';

    /**
     * DSN002の画面ID
     */
    const GAMEN_ID_HOT002 = 'HOT002';

    /**
     * DSN003の画面ID
     */
    const GAMEN_ID_HOT003 = 'HOT003';
    ...
    //搬出のお預かり日と届け日は何月後まで選ぶ
    const ATO_NAN_GETSU_F = 1;
    const ATO_NAN_GETSU_T = 2;
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    protected function createOutFormByInForm($inForm, $outForm = array()) {
        ...
        if(!empty($eventsubAry2)) {
            ...
            //預かり日と届け日を変更する
//            if (isset($eventsubCmbAry["selectedData"])) {
//                $this->changeInBoundDate($eventsubCmbAry["selectedData"]);
//            }
//            if (isset($eventsubCmbAry["list"][0])) {
//                $this->changeInBoundDate($eventsubCmbAry["list"][0]);
//            }
    ...
    public function setDispEvent(&$dispItemInfo){
        ...
        foreach($dispItemInfo['event_alllist'] as $k=>$v){
            if(isset($dispItemInfo['eventsub_selected_data']['event_id']) && $v['id']==$dispItemInfo['eventsub_selected_data']['event_id']){
                    ...
                    $dispItemInfo['dispEvent']['customName']=$v['eventsub_name'] .'係';
        ...
        try {
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    $data['manual_url'] = "【搬出の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$comiket['event_id']}.pdf";
...
//                $attach_mime_type = 'image/png';
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        $sendFileName = 'DSN_' . date ( 'YmdHis' ) . '.csv';
        $boundary = "-----" . md5 ( uniqid () );
...
//                "/".$this->_DirDiv."/parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
        ...
        foreach ($comiketDetailList as $key => $comiketDetailInfo) {
            if($comiketDetailInfo['type'] == '1') { // 往路
                ...
                if ($dateChArrival->format('Y-m-d H:i:s') <= $dateChNow->format('Y-m-d H:i:s')) {
                    ...
                    $title = "宿泊ホテルより国内配送受付サービスの搬入の{$titleParts}の受付終了しました";
                ...
                if ($collectDate2 <= $toDate) {
                    ...
                    $title = "宿泊ホテルより国内配送受付サービスの搬入の{$titleParts}お申し込みができませんでした";
                ...
                if ($eventTermEndDate <= $toDate) {
                    ...
                    $title = urlencode("宿泊ホテルより国内配送受付サービスの搬出の{$titleParts}お申し込みができませんでした");
    ...
    protected function changeInBoundDate(&$eventsubCmbAry){
        $week = array("日", "月", "火", "水", "木", "金", "土");
        //お預かり日と届け日を変更
        if (isset($eventsubCmbAry)) {
            $today = new DateTime();
            $strToday = $today->format("Y-m-d");
            if ($today->format('H') > 12) {
                $strToday = date('Y-m-d', strtotime($strToday. ' +1  day'));
                $today = new DateTime($strToday);
            }

            $strMaxAzukariDt = date('Y-m-d', strtotime($strToday. ' + '. self::ATO_NAN_GETSU_F.'  months'));
            $strMaxTodokeDt = date('Y-m-d', strtotime($strToday. ' + '. self::ATO_NAN_GETSU_T.'  months'));
            $eventsubCmbAry["is_eq_inbound_collect"] = false;
            
            if (isset($eventsubCmbAry["inbound_collect_fr"])) {
                $maxAzukariDt = strtotime($strMaxAzukariDt);
                

                $eventsubCmbAry["inbound_collect_fr"] = date('Y年m月d日（' . $week[date('w', strtotime($strToday))] . '）', strtotime($strToday));
                $eventsubCmbAry["inbound_collect_to"] = date('Y年m月d日（' . $week[date('w', strtotime($strMaxAzukariDt))] . '）', strtotime($strMaxAzukariDt));

                $eventsubCmbAry["inbound_collect_fr_year"] = $today->format('Y');
                $eventsubCmbAry["inbound_collect_fr_month"] = $today->format('m');
                $eventsubCmbAry["inbound_delivery_fr_day"] = $today->format('d');

                $eventsubCmbAry["inbound_collect_to_year"] = date('Y',$maxAzukariDt);
                $eventsubCmbAry["inbound_collect_to_month"] = date('m',$maxAzukariDt);
                $eventsubCmbAry["inbound_collect_to_day"] = date('d',$maxAzukariDt);

                $eventsubCmbAry["inbound_collect_fr_dt"] = $today->format('Y-m-d');
                $eventsubCmbAry["inbound_collect_to_dt"] = date('Y-m-d', $maxAzukariDt);
            }
            
            if (isset($eventsubCmbAry["inbound_delivery_fr"])) {
                $maxtodokeDt = strtotime($strMaxTodokeDt);

                $eventsubCmbAry["inbound_delivery_fr"] = date('Y年m月d日（' . $week[date('w', strtotime($strToday))] . '）', strtotime($strToday));
                $eventsubCmbAry["inbound_delivery_to"] = date('Y年m月d日（' . $week[date('w', strtotime($strMaxTodokeDt))] . '）', strtotime($strMaxTodokeDt));

                $eventsubCmbAry["inbound_delivery_fr_year"] = $today->format('Y');
                $eventsubCmbAry["inbound_delivery_fr_month"] = $today->format('m');
                $eventsubCmbAry["inbound_delivery_fr_day"] = $today->format('d');

                $eventsubCmbAry["inbound_delivery_to_year"] = date('Y',$maxtodokeDt);
                $eventsubCmbAry["inbound_delivery_to_month"] = date('m',$maxtodokeDt);
                $eventsubCmbAry["inbound_delivery_to_day"] = date('d',$maxtodokeDt);

                $eventsubCmbAry["inbound_delivery_fr_dt"] = $today->format('Y-m-d');
                $eventsubCmbAry["inbound_delivery_to_dt"] = date('Y-m-d', $maxtodokeDt);
            }
        }
        return $eventsubCmbAry;
    }
}
