...
Sgmov_Lib::useServices(array('CruiseRepeater', 'Prefecture', 'Yubin', 'SocketZipCodeDll'
    ...
    , 'AlpenApp', 'AlpenBox', 'AlpenBoxFare', 'AlpenKijiran', 'AlpenCustomerCd'
    ));
...
abstract class Sgmov_View_Evp_Common extends Sgmov_View_Public {
     ...
     * イベントサブID
//REWRITE
     * アルペンはGET引数をイベントサブIDに使用するため
     * この定数は使用しません
     */
    const EVENT_SUB_ID = '6001';
...
///////////////////////////////////////////////////////////////////////////////////
     ...
     * 記事欄マスタサービス
     */
    protected $_KijiranService;

    /**
     * アルペン顧客マスタサービス
     */
    protected $_CustomerCdService;
...
// REWRITE
    public function __construct() {
        ...
        $this->_ComiketService        = new Sgmov_Service_AlpenApp();         // Comiket()→AlpenApp()
        $this->_PrefectureService     = new Sgmov_Service_Prefecture();
        $this->_EventService          = new Sgmov_Service_Event();
        $this->_EventsubService       = new Sgmov_Service_Eventsub();
        $this->_BoxService            = new Sgmov_Service_AlpenBox();        // Box()→AlpenBox()
        ...
        $this->_BoxFareService        = new Sgmov_Service_AlpenBoxFare();    // BoxFare()→AlpenBoxFare()
        ...
        $this->_KijiranService        = new Sgmov_Service_AlpenKijiran();
        $this->_CustomerCdService     = new Sgmov_Service_AlpenCustomerCd();
     ...
     *
     * @param type $inForm
     * @param Sgmov_Form_Alp001Out $outForm
     * @return type
     */
...
//                        $inForm["comiket_detail_outbound_collect_date_day_sel"] = $today->format('d');
                ...
                if($dispItemInfo["eventsub_selected_data"]["is_eq_inbound_collect"]) {
                    // アルペンはアクセス日が預かり日の為、本日をセットする
                    // 他のイベントではeventsubの搬出預かり開始日をセットしている
                    ...
                    $today = new DateTime();
                    $todayFormat = $today->format("Y-m-d");
                    $comiketDetailInboundCollectDateFormat = $today->format('Y-m-d');
...
//        }
        ...
        // 館名：申込に必須な項目ではないので、ダミーの値をセットする
        $buildingNameInfoAry['ids']  =array($this->_DirDiv);
        $buildingNameInfoAry['names']=array($eventsubAry2['names'][0]);

        $outForm->raw_building_name     = $inForm["building_name"]; // 編集画面のラベル表示用
        $outForm->raw_building_name_sel = $this->_DirDiv.','.$eventsubAry2['names'][0];
...
// REWRITE まるっと封印で
          $outBoundCollectCalDataMax['arrival_date'] = (new DateTime())->format('1900-01-01 00:00:00');
...
//echo('<pre>');var_dump($outBoundCollectCalDataMax);echo('</pre>');
//  'arrival_date' => string '2022-04-25 18:00:00' (length=19)
//        Sgmov_Component_Log::debug($outBoundCollectCalList);

        $dateStr = (new DateTime())->format('Y-m-d h:i:s');
// 2022/04/25 13:25:33
//echo('<pre>');var_dump($outBoundCollectCalDataMax['arrival_date']);echo('</pre>');
//echo('<pre>');var_dump($dateStr);echo('</pre>');exit();
///var/share/VirtualHost/sagawa-mov-test04.media-tec.jp.vh65-1/lib/view/skt/Common.php:1250:string '2022-04-25 18:00:00' (length=19)
///var/share/VirtualHost/sagawa-mov-test04.media-tec.jp.vh65-1/lib/view/skt/Common.php:1251:string '2022-04-25 12:04:39' (length=19)

//'2022-04-25 18:00:00'<='2022/04/25 13:25:33'
//echo('<pre>');var_dump($inForm["comiket_detail_type_sel"]);echo('</pre>');exit();
...
// REWRITE 記事欄追加
        // 搬出-記事欄リスト
        $list=$this->_KijiranService->fetchAllAlpenKijiran($db, $inForm['eventsub_sel']);

        $outForm->raw_comiket_detail_inbound_kijiran_cds  = $list['ids'];
        $outForm->raw_comiket_detail_inbound_kijiran_lbls = $list['notes'];

        $outForm->raw_comiket_detail_inbound_kijiran_cd_sel1 = $inForm["comiket_detail_inbound_kijiran_cd_sel1"];
        $outForm->raw_comiket_detail_inbound_kijiran_cd_sel2 = $inForm["comiket_detail_inbound_kijiran_cd_sel2"];
        $outForm->raw_comiket_detail_inbound_kijiran_cd_sel3 = $inForm["comiket_detail_inbound_kijiran_cd_sel3"];
        $outForm->raw_comiket_detail_inbound_kijiran4        = $inForm["comiket_detail_inbound_kijiran4"];
...
//            }
    ...
    protected function calcEveryKindData($inForm, $comiketId = "") {
        ...
        // 往復区分：1：往路、2：復路、3:ミルクラン、4:手荷物、5:物販
...
//        if($inForm['comiket_div'] == self::COMIKET_DEV_BUSINESS) { // 法人
    ...
    public function _createComiketInsertDataByInForm($inForm, $id) {
        ...
        // アルペン顧客マスタを取得：顧客コード、担当者情報を取得
        $customerInfo=$this->_CustomerCdService->getInfo($db, $inForm['event_sel'], $inForm['eventsub_sel']);
        ...
        if($inForm['comiket_div'] == self::COMIKET_DEV_BUSINESS) { // 法人
            ...
            // 顧客マスタから顧客コードをセット
            $customerCd = $customerInfo['customer_cd'];
        ...
        $data = array(
            ...
            // 担当者情報は顧客マスタから取得
            "staff_sei" => @empty($inForm['comiket_staff_sei']) ? $customerInfo['sei'] : $inForm['comiket_staff_sei'],
            "staff_mei" => @empty($inForm['comiket_staff_mei']) ? $customerInfo['mei'] : $inForm['comiket_staff_mei'],
            ...
            "staff_tel" => @empty($inForm['comiket_staff_tel']) ? $customerInfo['tel'] : $inForm['comiket_staff_tel'],
...
//            "modified" => "",
    ...
    public function _createComiketDetailInsertDataByInForm($inForm, $id) {
        ...
        $db = Sgmov_Component_DB::getPublic();
        // 記事欄マスタ取得
        $kijiranList=$this->_KijiranService->fetchAllAlpenKijiranList($db,$inForm['eventsub_sel']);
...
// REWRITE 記事欄追加
                ...
                "kijiran1" => $this->setKijiranStr($kijiranList, @$inForm['comiket_detail_inbound_kijiran_cd_sel1']),
                "kijiran2" => $this->setKijiranStr($kijiranList, @$inForm['comiket_detail_inbound_kijiran_cd_sel2']),
                "kijiran3" => $this->setKijiranStr($kijiranList, @$inForm['comiket_detail_inbound_kijiran_cd_sel3']),
                "kijiran4" => @$inForm['comiket_detail_inbound_kijiran4'],
            );
...
//                }
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                $kijiran='';
                for($i=1;$i<=4;$i++){
                    if($comiket_detail['kijiran'.$i] != null){
                        // 改行コードが入らないのでこの形式でセットする
                        $kijiran.=$comiket_detail['kijiran'.$i]."
　　　　　";
                    }
                }
...
//                "/dsn_parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
     ...
     * @return bool
     */
    protected function setKijiranStr($kijiranList,$id){

        if(empty($id)){
            return '';
        }

        if(isset($kijiranList[$id])){
            return $kijiranList[$id]['note'];
        }
    }
