...
abstract class Sgmov_View_Evp_Common extends Sgmov_View_Public {
     ...
     * 機能ID
     */
    const FEATURE_ID = 'EVP';

    // 2022-03-22 ToanDD3 implement SMT6-84
    const LOGIN_ID = 'EVP_LOGIN';

    /**
     * EVE001の画面ID
     */
    const GAMEN_ID_EVE001 = 'EVP001';

    /**
     * EVE002の画面ID
     */
    const GAMEN_ID_EVE002 = 'EVP002';

    /**
     * EVE003の画面ID
     */
    const GAMEN_ID_EVE003 = 'EVP003';
...
//        $outForm->raw_building_booth_id_sel = $inForm["building_booth_id_sel"];
        ...
        $outForm->raw_comiket_booth_addition_num = $inForm["comiket_booth_addition_num"];
        // 担当者名
...
// var_dump($outBoundCollectCalDataMin);
// var_dump($outBoundColCalFromToData);
    ...
    public function setDispEvent(&$dispItemInfo){
        ...
        foreach($dispItemInfo['event_alllist'] as $k=>$v){
            if(isset($dispItemInfo['eventsub_selected_data']['event_id']) && $v['id']==$dispItemInfo['eventsub_selected_data']['event_id']){
                $dispItemInfo['dispEvent']['kaijoMap']  ='/evp/pdf/manual/kaijoMap_evp.pdf'.$cacheStr;
...
//        if($inForm['comiket_div'] == self::COMIKET_DEV_BUSINESS) { // 法人
    ...
    public function _createComiketInsertDataByInForm($inForm, $id) {
        ...
        $data = array(
            ...
            "booth_num" => @@sprintf('%02s', $inForm['comiket_booth_num']).@$inForm['comiket_booth_addition_num'],
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/evp/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/evp/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/evp/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/evp/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/evp/evp_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/evp/evp_complete_business_sgmv{$tmplateType}.txt";
                ...
                $data['comiket_building_name'] = PHP_EOL . '【館名】' . $comiket['building_name'] . "ホール " . $blockStr. $comiket['booth_num'];
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/evp/evp_complete_individual{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/evp/evp_complete_individual_sgmv{$tmplateType}.txt";
                ...
                if ($eventData['id'] === '10') {  // 国内クルーズ
                    ...
                    $data['comiket_booth_name'] = PHP_EOL . '【サークル名】' . $comiket['booth_name'];
                }
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    //搬出用メールテンプレート
                    $mailTemplate[] = '/evp/evp_parts_complete_choice_2.txt';

                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/evp/evp_parts_complete_choice_2_sgmv.txt';
                    ...
                    $prefData = $this->_PrefectureService->fetchPrefecturesById($db, $comiket_detail['pref_id']);
                    //搬入用メールテンプレート
                    $mailTemplate[] = '/evp/evp_parts_complete_choice_1.txt';

                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/evp/evp_parts_complete_choice_1_sgmv.txt';
            ...
            if ($tmplateType == '_cancel' || $tmplateType == '_sgmv_cancel') {
                $mailTemplate[] = "/evp/evp_parts{$footerTmpName}_footer_type_1.txt";
            } else {
                $sesUserType = isset($_SESSION[self::LOGIN_ID]['user_type']) ? $_SESSION[self::LOGIN_ID]['user_type'] : 0;
            ...
            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/evp/evp_parts{$footerTmpName}_footer_type_1.txt";
            // } else { // 個人
            //     $mailTemplate[] = "/evp/evp_parts{$footerTmpName}_footer_type_2.txt";

            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/evp/evp_parts{$footerTmpName}_footer_type_2.txt";
            // }
            ...
            // $data['edit_url'] = $urlPublicSsl . "/evp/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            // 2022-03-28 ToanDD3 fix SMT6-120
            if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] == '1') {
                $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm=evp'; 
            } else {
                $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=evp&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
            }

            // 説明書URL
//            $data['manual_url'] = $urlPublicSsl . "/evp/pdf/manual/manual_{$comiket["eventsub_id"]}.pdf";
            $data['manual_url'] = '';
...
//            if($eventData['id'] != '2' && $eventData['id'] !== '4') { // コミケとGoOutCampは記載しない
            if($eventsubData['manual_display'] == '1') { // コミケとGoOutCampは記載しない
                //$data['manual_url'] = "【説明書URL】" . PHP_EOL . $urlPublicSsl . "/evp/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
                $data['manual_url'] = "【説明書URL】" . PHP_EOL . "https://www.comiket.co.jp/info-c/C101/C101appeal.html";
            }
//            $data['manual_url'] = $urlPublicSsl . "/evp/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
...
//                    && $eventData['id'] !== '4')  {
                ...
                if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 0){
                    $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/event/robotCheck?event_nm=evp&destination=paste_tag/{$pasteTagId}/";
                }
            }
...
";
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // 2022-03-25 ToanDD3 implement SMT6-84
            // $data['cancel_url'] = $urlPublicSsl . "/evp/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=evp&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            // 2022-03-25 ToanDD3 implement SMT6-84
            // $data['size_change_url'] = $urlPublicSsl . "/evp/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=evp&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;;

            //-------------------------------------------------
            //メール送信実行
            $objMail = new Sgmov_Service_CenterMail();
            if (!$isAttachment) {
                Sgmov_Component_Log::debug("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT1");
Sgmov_Component_Log::debug($mailTemplate);
Sgmov_Component_Log::debug($sendTo);
Sgmov_Component_Log::debug($data);
                // 申込者へメール
                $objMail->_sendThankYouMail($mailTemplate, $sendTo, $data);
Sgmov_Component_Log::debug($objMail);
                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                ...
                Sgmov_Component_Log::debug("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT2");
Sgmov_Component_Log::debug($mailTemplate);
Sgmov_Component_Log::debug($sendTo);
Sgmov_Component_Log::debug($data);
                // qrコードファイル出力
...
//                "/dsn_parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
    ...
    //GiapLN fix bug redirect to inputHistory or top Url 2022.07.08
    /*
     * redirectBackHistoryOrTopUrl
     */
    protected function redirectBackHistoryOrTopUrl() {
        $baseUrl = Sgmov_Component_Config::getUrlPublicSsl();
        if (isset($_SESSION['EVENT_LOGIN'])) {
            if ($_SESSION['EVENT_LOGIN']['user_type'] == '1') {
                $valBackTop = "申込履歴ページに戻る";
                $urlBackTop = $baseUrl."/event/inputHistory?event_nm=evp";
            } else {
                $valBackTop = "トップページに戻る";
                $urlBackTop = "/";
            }
        } else {
            $valBackTop = "トップページに戻る";
        ...
        return array(
            'valBackTop' => $valBackTop, 
            'urlBackTop' => $urlBackTop
        );
    }
