...
Sgmov_Lib::useView($dirDiv.'/CommonToiawase', 'CommonConst');
 ...
 * #@-
 */
class Sgmov_View_Evp_ReSendMailToiawase extends Sgmov_View_Evp_CommonToiawase {
	 ...
	 * 処理
	 */
    public function executeInner($comiket_id = '') {

		//$comiket_id = filter_input(INPUT_GET, "comiket_id");
		$comiket_id = $_GET["comiket_id"];
		Sgmov_Component_Log::info('イベント申込 メール再送信処理を実行します。' . $comiket_id);

        if (empty($comiket_id) || is_null($comiket_id)) {
            echo '申込み番号がNULLです。<br/>';
            return;
        }
        ...
        if (@empty($comiketData)) {
            Sgmov_Component_Log::info('データが存在しません。' . $comiket_id);
            echo $comiket_id . '：データが存在しません<br/>';
            return;
        }
        if (@$comiketData['del_flg'] != '0') {
            Sgmov_Component_Log::info('データがキャンセルされています。' . $comiket_id);
            echo $comiket_id . '：データがキャンセルされています<br/>';
            return;
        }
...
//        }
        ...
        if ($result) {
            ...
            $mailTo = 'ken-sawada@spcom.co.jp';
            //$mailTo = 'sgmv.smt@spcom.co.jp';
            $result = $this->sendCompleteMail($comiketData, $mailTo, '', $type);
        }

        if ($result) {
            echo $comiket_id . '：再送信の処理を行いました。'.$this->_DirDiv.'1<br/>';
        } else {
            echo $comiket_id . '：再送信の処理を行いました。'.$this->_DirDiv.'2<br/>';
        }

        Sgmov_Component_Log::info('イベント申込 メール再送信処理を終了します。' . $comiket_id);
