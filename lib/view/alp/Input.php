...
Sgmov_Lib::useForms(array('Error', 'AlpSession', 'Alp001Out', 'Alp002In'));
/**#@-*/
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Evp_Input extends Sgmov_View_Evp_Common {
...
// REWRITE
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////
// REWRITE
        if(empty($param) || self::checkQueryShopCode($param) === false) {
        ...
        $db = Sgmov_Component_DB::getPublic();
        // イベントサブを取得
        $eventsubInfo = $this->_EventsubService->fetchEventsubIdAndSubid($db, self::EVENT_ID, $param);
        $eventInfo = array();
        ...
        // イベントID、イベントサブID(店舗コード)セット
        $inForm->event_sel = intval(self::EVENT_ID);
        $inForm->input_mode = self::EVENT_ID;
        $inForm->eventsub_sel = $param;
        ...
        if (isset($eventsubInfo)) {
            if (isset($eventsubInfo['arrival_to_time'])) {
                ...
                if ($now > $arrivalToTime) {
                    ...
                    $eventName = $eventInfo['name'].$eventsubInfo['name'];
        ...
        if (isset($eventsubInfo)) {
            if (isset($eventsubInfo['departure_fr_time'])) {
                ...
                if ($now<$departureFrTo) {
                    ...
                    $message = urlencode("{$eventName}は {$this->_changeDateDisp(array($departureFrTime))} より申込開始となります。");
                    Sgmov_Component_Redirect::redirectPublicSsl("/".$this->_DirDiv."/error?t={$title}&m={$message}");
                    exit;
                }
            } else {
...
// REWRITE
    /**
     * 入力フォームから出力フォームを取得します。
     * @param  Sgmov_Form_Alp001In $inForm 入力フォーム
     * @return Sgmov_Form_Alp001Out 出力フォーム
     */
    protected function _createOutFormByInForm($inForm) {
        $inForm = (array)$inForm;
        return $this->createOutFormByInForm($inForm, new Sgmov_Form_Alp001Out());
    }
}