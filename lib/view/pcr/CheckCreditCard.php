...
Sgmov_Lib::useForms(array('Error', 'PcrSession', 'Pcr002In'));
/**#@-*/
/**
 ...
 * @copyright  2009-2010 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Pcr_CheckCreditCard extends Sgmov_View_Pcr_Common {
    /**
     * コンストラクタでサービスを初期化します。
     */
    ...
    public function executeInner() {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_PCR002, $this->_getTicket());
        ...
        // リダイレクト
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/pcr/credit_card');
        } else {
            Sgmov_Component_Redirect::redirectPublicSsl('/pcr/confirm');
        }
    }
    /**
     * POST値からチケットを取得します。
     ...
     * @param Sgmov_Form_Pcr001In $inputForm 入力フォーム
     * @return Sgmov_Form_Pcr002In クレジット入力フォーム
     */
    public function _createInFormFromPost($post, $inputForm) {
        $inForm = new Sgmov_Form_Pcr002In();
        ...
        $inForm->req_flg = $inputForm->req_flg;
        return $inForm;
    }
    /**
     * 入力値の妥当性検査を行います。
     * @param Sgmov_Form_Pcr002In $inForm 入力フォーム
