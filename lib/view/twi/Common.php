 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Twi_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'TWI';
     ...
     * イベントサブID
     */
    const EVENT_SUB_ID = '2252';

    /**
     * TWI001の画面ID
     */
    const GAMEN_ID_TWI001 = 'TWI001';

    /**
     * TWI002の画面ID
     */
    const GAMEN_ID_TWI002 = 'TWI002';

    /**
     * TWI003の画面ID
     */
    const GAMEN_ID_TWI003 = 'TWI003';
...
///////////////////////////////////////////////////////////////////////////////////
     ...
     * @param type $inForm
     * @param Sgmov_Form_Twi001Out $outForm
     * @return type
     */
    protected function createOutFormByInForm($inForm, $outForm = array()) {
        ...
        $inForm = (array)$inForm;
        //デフォルト値設定：荷送人住所欄
        $inForm["comiket_zip1"] = "112";
        $inForm["comiket_zip2"] = "0004";
        $inForm["comiket_pref_cd_sel"] = "13";
        $inForm["comiket_address"] = "文京区後楽1-3-61";
        $inForm["comiket_building"] = "東京ドームシティプリズムホール";
        $inForm["comiket_detail_type_sel"] = 2;//搬出のみ
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                    ...
                    if(isset($valDet["comiket_box_list"])){
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Twi_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Twi_Common::CURRENT_TAX));
                        // // 税込
    ...
    */
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            $data['event_name'] = $eventData["name"] . " " . $eventsubData["name"]; //【イベント】
            ...
            if ($eventsubData['booth_display'] !== '0') {
                ...
                if ($eventData['id'] === '10') {  // 国内クルーズ
                    ...
                    $data['comiket_booth_name'] = PHP_EOL . '【出展者名】' . $comiket['booth_name'];
                }
            }
            ...
            if ($eventsubData['building_display'] !== '0') {
                //GiapLN fix bug send mail item 【ブース名】 2024.07.09
                $building = '';
                ...
                $data['comiket_building_name'] = PHP_EOL . '【ブース名】' . $building . $comiket['booth_position'] . " " . $comiket['booth_num'];
            }
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    $data['manual_url'] = "【搬出の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$this->_DirDiv}.pdf";
                    ...
                    $data['manual_url'] = "【搬入の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$this->_DirDiv}.pdf";
...
";
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        $sendFileName = 'TWI_' . date ( 'YmdHis' ) . '.csv';
        $boundary = "-----" . md5 ( uniqid () );
