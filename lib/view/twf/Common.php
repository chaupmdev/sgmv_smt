 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Twf_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'TWF';
     ...
     * イベントサブID
     */
    const EVENT_SUB_ID = '2202';

    /**
     * TWF001の画面ID
     */
    const GAMEN_ID_TWF001 = 'TWF001';

    /**
     * TWF002の画面ID
     */
    const GAMEN_ID_TWF002 = 'TWF002';

    /**
     * TWF003の画面ID
     */
    const GAMEN_ID_TWF003 = 'TWF003';
    ...
    public function setDispEvent(&$dispItemInfo){
        ...
        foreach($dispItemInfo['event_alllist'] as $k=>$v){
            if(isset($dispItemInfo['eventsub_selected_data']['event_id']) && $v['id']==$dispItemInfo['eventsub_selected_data']['event_id']){
                    ...
                    $dispItemInfo['dispEvent']['customName']=$v['event_name'] .'係';
                    ...
                    $dispItemInfo['dispEvent']['pasteTag1']  ='/'.$this->_DirDiv.'/pdf/paste_tag/paste_tag_'.$this->_DirDiv.'_1.pdf'.$cacheStr;
                    // 貼付票のサンプル：例)paste_tag_[イベント識別子].pdf
                    $dispItemInfo['dispEvent']['pasteTag2']  ='/'.$this->_DirDiv.'/pdf/paste_tag/paste_tag_'.$this->_DirDiv.'_2.pdf'.$cacheStr;
                    // 貼付票のサンプル：例)paste_tag_[イベント識別子].pdf
                    $dispItemInfo['dispEvent']['pasteTag3']  ='/'.$this->_DirDiv.'/pdf/paste_tag/paste_tag_'.$this->_DirDiv.'_3.pdf'.$cacheStr;
                    // 説明書：例)manual_[イベント識別子].pdf
                    $dispItemInfo['dispEvent']['manual'] ='/'.$this->_DirDiv.'/pdf/manual/manual_'.$this->_DirDiv.'.pdf'.$cacheStr;
                    $dispItemInfo['dispEvent']['manual2'] ='/'.$this->_DirDiv.'/pdf/manual/manual_'.$this->_DirDiv.'_2.pdf'.$cacheStr;
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                    ...
                    if(isset($valDet["comiket_box_list"])){
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Twf_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Twf_Common::CURRENT_TAX));
                        // // 税込
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($eventsubData['building_display'] !== '0') {
                ...
                $data['comiket_building_name'] = PHP_EOL . '【エリア名、ブース番号、エントリーNo】' . $building . $comiket['booth_position'] . " " . $comiket['booth_num'];
            }
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    $data['manual_url2'] = "【搬出の出展ガイドはこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$this->_DirDiv}_2.pdf";
                    ...
                    $data['manual_url2'] = "【搬入の出展ガイドはこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$this->_DirDiv}_2.pdf";
