...
Sgmov_Lib::useServices(array('CruiseRepeater', 'Prefecture', 'Yubin', 'SocketZipCodeDll'
    ...
    , 'AppCommon', 'HttpsZipCodeDll', 'BoxFare', 'CargoFare', 'Comiket', 'EventsubCmb', 'Time', 'CenterMail', 'OutBoundCollectCal'));
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Tms_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
...
Sgmov_Component_Log::debug("################## 301-1 _createPulldown");
        $html = '';
...
Sgmov_Component_Log::debug("################## 301-2 _createPulldown");
Sgmov_Component_Log::debug($cds);
Sgmov_Component_Log::debug("################## 301-3 _createPulldown");
...
Sgmov_Component_Log::debug('######## _getLabelSelectPulldownData 1');
        $html = '';

        if (empty($cds)) {
            return $html;
        }
Sgmov_Component_Log::debug('######## _getLabelSelectPulldownData 2');
        $count = count($cds);
        for ($i = 0; $i < $count; ++$i) {
            if ($select === $cds[$i]) {
Sgmov_Component_Log::debug('######## _getLabelSelectPulldownData 3');
Sgmov_Component_Log::debug($lbls);
                return $lbls[$i];
            }
        }
Sgmov_Component_Log::debug('######## _getLabelSelectPulldownData 4');
        return "";
    }
...
//        $outForm->raw_comiket_detail_type_sel = $inForm["comiket_detail_type_sel"];
        $outForm->raw_comiket_detail_type_sel = '2'; // 東京モータショーは 搬出のみ(type = 2)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // 送料
        $outForm->raw_delivery_charge_free = $inForm["delivery_charge_free"];
...
//            ));
     ...
     * @return type
     */
    protected function getFreeBoxInfoInInForm($inForm, $tableTreeData=array()) {
        // DB接続
        $db = Sgmov_Component_DB::getPublic();
        
        if (@empty($tableTreeData)) {
            $this->setYubinDllInfoToInForm($inForm);
            $tableDataInfo = $this->_cmbTableDataFromInForm($inForm);
            $tableTreeData = $tableDataInfo["treeData"];
        }
        
        $maxBoxInfo = array();
        $ComiketDataForCheck = $this->_ComiketService->fetchFirstComiketByTel($db, @str_replace(array('-', 'ー', '−', '―', '‐'), '', $inForm['comiket_tel'])
                , $tableTreeData['event_id'], $tableTreeData['eventsub_id']);

        if (@empty($ComiketDataForCheck)) {
                    ...
                    if(!empty($valDet["comiket_box_list"])) {
                        foreach($valDet["comiket_box_list"] as $keyComiketBox => $valComiketBox) {
                            if($valComiketBox['type'] == "2" || $valComiketBox['type'] == "3") { // 搬出
                                ...
                                if (@empty($maxBoxInfo) || $maxBoxInfo['boxSize'] < $boxInfo['size']) {
                                    $maxBoxInfo = array(
                                        'boxId' => $boxInfo['id'],
...
Sgmov_Component_Log::debug("####################### 701-2-start");
        ...
        // 無料分計算用
        ///////////////////////////////////////////////////////
        $tableTreeData['amount_tax_free'] = 0;
        $tableTreeData['amount_free'] = 0;
    ...
    //                                    $boxInfo = $this->_BoxService->fetchBoxById($db, $key);
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costAmountTaxOutboundTotal / Sgmov_View_Tms_Common::CURRENT_TAX));
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax'] = floor((string)($fareAmountOutboundTotal * Sgmov_View_Tms_Common::CURRENT_TAX));
...
//                }
        ...
        $fareAmountTaxInboundTotal2ForFree = 0;
        if($inForm['comiket_detail_type_sel'] == "2"
                || $inForm['comiket_detail_type_sel'] == "3" ) { // 搬出（もしくは搬入搬出）
...
//                        }
//                    }
                    $freeBoxInfo = $this->getFreeBoxInfoInInForm($inForm, $tableTreeData);
                    ...
                    foreach($tableTreeData["comiket_detail_list"] as $keyDet => $valDet) {
                        ...
                        if($valDet['type'] == "2" || $valDet['type'] == "3") {
                            ...
                            $fareAmountInboundTotalForFree = 0;
                            $tableTreeData["comiket_detail_list"][$keyDet]['cost'] = 0;
                            $tableTreeData["comiket_detail_list"][$keyDet]['cost_tax'] = 0;
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare'] = 0;
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax'] = 0;
                                ...
                                foreach($valDet["comiket_box_list"] as $keyComiketBox => $valComiketBox) {
                                    if($valComiketBox['type'] == "2" || $valComiketBox['type'] == "3") { // 搬出
                                        ...
                                        if(!empty($boxFareData)) {
                                            ...
                                            // 無料分計算
                                            //////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                            if (@empty($freeBoxInfo)) {
                                                // 有料 (無料なし)
                                                $fareAmountInboundTotalForFree  += intval($boxFareData['fare']) * intval($valComiketBox["num"]);
                                            } else if (@!empty($freeBoxInfo['boxId']) && @$freeBoxInfo['boxId'] != $boxInfo['id']) {
                                                // 有料のみ
                                                $fareAmountInboundTotalForFree  += intval($boxFareData['fare']) * intval($valComiketBox["num"]);
                                            } else if (@!empty($freeBoxInfo['boxId']) && @$freeBoxInfo['boxId'] == $boxInfo['id']) {
                                                // 無料該当サイズ
                                                $fareAmountInboundTotalForFree  += intval($boxFareData['fare']) * intval($valComiketBox["num"]-1 < 0 ? 0 : $valComiketBox["num"]-1); // -1 は無料分
                                                
                                                $tableTreeData["comiket_detail_list"][$keyDet]["comiket_box_list"][$keyComiketBox]["fare_price_free"] = intval($boxFareData['fare']);
                                                $tableTreeData["comiket_detail_list"][$keyDet]["comiket_box_list"][$keyComiketBox]["fare_amount_free"] = intval($boxFareData['fare']) * intval($freeBoxInfo["freeNum"]);
                                            }
                                            //////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                        }
                                    }
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax'] = floor((string)($fareAmountInboundTotal * Sgmov_View_Tms_Common::CURRENT_TAX));
                            ...
                            // 無料分計算
                            //////////////////////////////////////////////////////////////////////////////////////////////////////////////
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_free'] = $fareAmountInboundTotalForFree;
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax_free'] = floor((string)($fareAmountInboundTotalForFree * Sgmov_View_Tms_Common::CURRENT_TAX));
                            $fareAmountTaxInboundTotal2ForFree += $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax_free'];
                        }
                    }
//                }
            }
...
//                        $tableTreeData["comiket_detail_list"][$keyDet]['fare'] = $cargoFareTaxTotalInbound;
    ...
    //                                $fareTaxTotal+=$fareAmountTax;
        ...
        $tableTreeData['amount_tax_free'] = $tableTreeData['amount_free'] = 0;
        $detailAmountTotal = 0;
        $detailAmountTaxTotal = 0;
        
        $detailAmountTotalForFree = 0;
        $detailAmountTaxTotalForFree = 0;
        foreach($tableTreeData["comiket_detail_list"] as $keyDet => $valDet) {
            /////////////////////////////////////////////////////////////////////////////////
            // 税抜計算
            /////////////////////////////////////////////////////////////////////////////////
            ...
            // 税抜計算 - 無料用
            /////////////////////////////////////////////////////////////////////////////////
            $detailAmountTotalForFree += @empty($valDet['fare_free']) ? 0 : $valDet['fare_free'];
            $detailAmountTotalForFree += @empty($valDet['cost_free']) ? 0 : $valDet['cost_free'];
            
            /////////////////////////////////////////////////////////////////////////////////
            // 税込計算 - 無料様
            /////////////////////////////////////////////////////////////////////////////////
            $detailAmountTaxTotalForFree += @empty($valDet['fare_tax_free']) ? 0 : $valDet['fare_tax_free'];
            $detailAmountTaxTotalForFree += @empty($valDet['cost_tax_free']) ? 0 : $valDet['cost_tax_free'];
        }
        $tableTreeData['amount'] = $detailAmountTotal;
        $tableTreeData['amount_tax'] = $detailAmountTaxTotal;
        
        $tableTreeData['amount_free'] = $detailAmountTotalForFree;
        $tableTreeData['amount_tax_free'] = $detailAmountTaxTotalForFree;
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/tms/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/tms/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/tms/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/tms/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
...
//        $result = $target % 7;
     ...
     * @return bool true:成功
     */
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '', $freeBoxInfo = array()) {
        ...
        try {
            if (@empty($tmplateType)) {
                ...
                if ($tmplateType == 'cancel') { // キャンセルメールの場合はqrコードは添付しない
                    $isAttachment = false;
                } else {
...
//            $sendTo = $comiket['mail'];
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/eve_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/eve_complete_business_sgmv{$tmplateType}.txt";
...
//                $data['comiket_building_name'] = $comiket['building_name'] . " " . $comiket['booth_position'] . " " . $comiket['booth_num'];
                ...
                $data['comiket_building_name'] = PHP_EOL . '【館名】' . $comiket['building_name'] . "ホール " . $comiket['booth_position'] . "ブロック " . $comiket['booth_num'];
//                if ($eventData['id'] === '4') {  // GoOutCamp
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/eve_complete_individual{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/eve_complete_individual_sgmv{$tmplateType}.txt";
...
//                $data['comiket_building_name'] = $comiket['building_name'] . " " . $comiket['booth_position'] . " " . $comiket['booth_num'];
                ...
                if($eventData['id'] === '2') { // コミケ
                    $data['comiket_building_name'] = PHP_EOL . '【ブース番号】' . $comiket['building_name'] . "ホール " . $comiket['booth_position'] . "ブロック " . $comiket['booth_num'];
                } else {
                    $data['comiket_building_name'] = PHP_EOL . '【館名】' . $comiket['building_name'] . "ホール " . $comiket['booth_position'] . "ブロック " . $comiket['booth_num'];
                }
                    ...
                    case 1://宅配
                        ...
                        foreach ($comiket_box_list as $cb => $comiket_box) {
                            ...
                            if (($comiket_detail['type'] == 2 && $comiket_box['type'] == 2) || ($comiket_detail['type'] == 1 && $comiket_box['type'] == 1)
                                ...
                                if (@$freeBoxInfo['boxId'] == $comiket_box['box_id']) {
                                    $outputFreeNum = "";
                                    if ($freeBoxInfo['freeNum'] != $comiket_box['num']) {
                                        $outputFreeNum = "{$freeBoxInfo['freeNum']}個";
                                    }
                                    $num_area .= '    ' . $boxInfo['name'] . ' ' . $comiket_box['num'] . ' 個' . "（ {$outputFreeNum}無料 ）" . PHP_EOL;
                                } else {
...
//                                $charterInfo = $this->_CharterService->fetchCharterById($db, $comiket_charter['charter_id']);
            ...
            if ($tmplateType === '_cancel' || $tmplateType === '_sgmv_cancel') {
                $mailTemplate[] = "/tms_parts{$footerTmpName}_footer_type_1.txt";
            } else {
...
//                $mailTemplate[] = "/eve_parts{$footerTmpName}_footer_type_1.txt";
//
//                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
//                $mailTemplateSgmv[] = "/eve_parts{$footerTmpName}_footer_type_1.txt";
//            } else { // 個人
//                $mailTemplate[] = "/eve_parts{$footerTmpName}_footer_type_2.txt";
//
//                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
//                $mailTemplateSgmv[] = "/eve_parts{$footerTmpName}_footer_type_2.txt";
//            }
            
            $data['comiket_amount'] = '\\' . number_format($comiket['amount_free']);
            $data['comiket_amount_tax'] = '\\' . number_format($comiket['amount_tax_free']);
            $urlPublicSsl = Sgmov_Component_Config::getUrlPublicSsl();
            $comiketIdCheckD = self::getChkD(sprintf("%010d", $comiket['id']));
            
            
            // GiapLN implement SMT6-85
            // $data['edit_url'] = $urlPublicSsl . "/tms/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            if ($_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm=tms'; 
            } else {
                $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=tms&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
            }

            // 説明書URL
//            $data['manual_url'] = $urlPublicSsl . "/tms/pdf/manual/manual_{$comiket["eventsub_id"]}.pdf";
            $data['manual_url'] = '';
...
//            if($eventData['id'] != '2' && $eventData['id'] !== '4') { // コミケとGoOutCampは記載しない
            if($eventsubData['manual_display'] == '1') { // コミケとGoOutCampは記載しない
                $data['manual_url'] = "【説明書URL】" . PHP_EOL . $urlPublicSsl . "/tms/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            }
//            $data['manual_url'] = $urlPublicSsl . "/tms/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
...
//                    && $eventData['id'] !== '4')  {
                ...
                if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 0){
                    //$data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/tms/paste_tag/{$pasteTagId}/";
                    $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/event/robotCheck?event_nm=tms&destination=paste_tag/{$pasteTagId}";
                }
...
";
            ...
            // GiapLN implement SMT6-85
            //  $data['cancel_url'] = $urlPublicSsl . "/tms/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=tms&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            // $data['size_change_url'] = $urlPublicSsl . "/tms/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=tms&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
