 ...
 */
class Sgmov_View_Sso_Complete extends Sgmov_View_Sso_Common {
    ...
    public function __construct() {
        ...
        $this->_BcmView             = new Sgmov_View_Sso_Bcm();
...
Sgmov_Component_Log::debug($inForm);
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 搬入
            ...
            if ($dateChArrival->format('Y-m-d H:i:s') <= $dateChNow->format('Y-m-d H:i:s')) {
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/error/?t=受付時間が終了しました&m=お預かり日：{$dispDate}の受付時間が終了しました");
                exit;
            }
            ...
            if ($dateColl <= $dateNow) {
                // セッション情報を破棄
                $session->deleteForm(self::FEATURE_ID);
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/error/?t=受付時間が終了しました&m=お預かり日：{$yearColl}/{$monthColl}/{$dayColl}の受付時間が終了しました");
                exit;
            }
        }
        /////////////////////////////////////////////////////////////////////////////////////////////
...
//            }
            ...
            Sgmov_Component_Log::debug("リダイレクト /sso/{$backInputPaht}/");
            Sgmov_Component_Log::err('データベースの登録・更新に失敗しました。');
            Sgmov_Component_Log::err($e);
            Sgmov_Component_Redirect::redirectPublicSsl("/sso/{$backInputPaht}/");
            exit;
        }
        $db->commit();
...
//                }
                ...
                if ($shipmentCompResArr['result'] == 'OK') {
                    ...
                    Sgmov_Component_Mail::sendTemplateMail($param, dirname(__FILE__) . '/../../mail_template/sso_error_for_sgfinancial_shipment_report.txt', $mail_to);
...
Sgmov_Component_Log::debug($li["treeData"]);
    ...
    public function errorInformationForSgFinancial($param) {
        ...
        $mail_to = Sgmov_Component_Config::getLogMailTo();
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($param, dirname(__FILE__) . '/../../mail_template/sso_error_for_sgfinancial.txt', $mail_to);
    }
...
*/
    ...
    public function errorInformation($parm = array())
    {
        ...
        $mail_to = Sgmov_Component_Config::getLogMailTo();
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($parm, dirname(__FILE__) . '/../../mail_template/sso_error.txt', $mail_to);
    }
