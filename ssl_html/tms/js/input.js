...
//                $('.paste_tag_link').attr("href", "/tms/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf");
//                $('.manual_link').attr("href", "/tms/pdf/manual/manual_" + eventsubSel + ".pdf");

                $('.manual').show(g_fifoVal);
                eventSelText = eventSelText.replace(/\s+/g, "");
                $('.manual_link').attr("href", "/tms/pdf/manual/" + eventSelText +  ".pdf" + '?' + strCurrentDate);
            } else {
                $('.manual').hide(g_fifoVal);
            }
            if(g_eventsubList[eventsubSel]['is_paste_display']){
                $('.paste_tag').show(g_fifoVal);
                $('.paste_tag_link').attr("href", "/tms/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf" + '?' + strCurrentDate);
            } else {
    ...
    function getBoxData() {
        sgwns.api('/common/php/SearchBox.php', getFormData(), (function(data) {
            ...
            if($('input[name="comiket_detail_type_sel"]:checked').val() == "1" ) { // 搬入
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img class='dispSeigyoPC' src='/tms/images/about_boxsize.png' width='100%'/>";
                } else {
//                    htmlCode += "<img src='/tms/images/about_boxsize.png' width='100%' style='visibility:hidden;'/>";
                }
                htmlCode += "</td></tr></table>";

                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL && $('.dispSeigyoSP').length === 0) {
                    htmlCode += "<div class='dispSeigyoSP'><img src='/tms/images/about_boxsize.png' width='250px' style='margin-top: 1em;' /></div>";
                }
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img src='/tms/images/about_boxsize.png' width='100%'/>";
                }
                htmlCode += "</td></tr></table>";
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode2 += "<img src='/tms/images/about_boxsize.png' width='100%'/>";
                }
...
//                $('.comiket_detail_outbound_collect_time_sel').hide(g_fifoVal);
    ...
    // 東京モーターショーは 搬出しかないため、デフォルト搬出選択 /////////////////////////////////////////////////////
    var isComiketBoxItemValue = false;
    $('.comiket_box_item_value_input').each(function() {
        var comiketBoxItemValue = $(this).val();
        if (comiketBoxItemValue && comiketBoxItemValue != '') {
            isComiketBoxItemValue = true;
            return false;
        }
    });
//console.log('#################### 1111');
//console.log(isComiketBoxItemValue);
    if (!isComiketBoxItemValue) {
        $('input[name="comiket_detail_type_sel"]').trigger('change');
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // エラーでリダイレクトした時用
    seigyoEveTimeOver();
