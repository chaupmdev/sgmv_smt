<?php
/**
 * 【シーフードショー大阪・アグリフードEXPO大阪】の貼付票PDFを出力します。
 * @package    /lib/view/sso
 ...
 * #@-
 */
class Sgmov_View_Sso_PasteTag extends Sgmov_View_Sso_Common {
	...
	public function executeInner() {
		// GiapLN implement SMT6-85
		if (!isset($_SESSION['SSO_LOGIN']['user_type'])) {
	...
	private function addSheet(&$pdf, &$comiket, $page) {
		...
		// N日目・本体 【シーフードショー大阪・アグリフードEXPO大阪では、搬入日が一択なので表示不要】
// 		$pdf->SetXY ( $x += 4, $y += 4 );
...
// 		}
		...
		// N日目・単位【シーフードショー大阪・アグリフードEXPO大阪では、搬入日が一択なので表示不要】
// 		$pdf->SetXY ( $x += 14, $y += 5 );
// 		$pdf->SetFont ( 'DejaVu', '', 14 );
// 		if($whichDay > 0){
...
// 		}
		...
		switch ($this->eventid) {
			case '500' :
				...
				$pdf->Cell ( 117, 13, $comiket ['event_name'], 0, 0, 'C' );

				// イベントサブ名
				$pdf->SetXY ( $x, $y += 13 );
				$pdf->SetFont ( 'DejaVu', '', 14 );
				$pdf->Cell ( 117, 13, $comiket ['eventsub_name'], 0, 0, 'L' );
				break;
			case '600' :
				// イベント名
				$pdf->SetXY ( $x += 4, $y += 4 );
				$pdf->SetFont ( 'DejaVu', '', 36 );
				$pdf->Cell ( 117, 13, $comiket ['event_name'].$comiket ['eventsub_name'], 0, 0, 'C' );
		...
		// 配送日【シーフードショー大阪・アグリフードEXPO大阪では、搬入日が一択なので表示不要】
// 		$pdf->SetXY ( $x, $y += 19 );
// 		$pdf->SetFont ( 'DejaVu', '', 14 );
// 		$pdf->Cell ( 90, 7, $comiket ['delivery_date'], 1, 0, 'R' );
