...
Sgmov_Lib::useForms(array('Error', 'UserSession', 'UserRobotCheck001Out'));
Sgmov_Lib::useServices(array('Eventsub'));

/**#@-*/


class Sgmov_View_Event_Robot_Check extends Sgmov_View_Event_Common
{
    /**
     * イベントサブサービス
    ...
    public function executeInner()
    {
        ...
        if ($_SESSION[self::FEATURE_ID]['security_patten'] == 3) {
            Sgmov_Component_Redirect::redirectPublicSsl("/404.html");
        }
        
        $sessionData = $_SESSION[self::FEATURE_ID];
        
        $sessionForm = $session->loadForm(self::ROBOT_CHECK_ID);
        ...
        if (isset($sessionForm)) {
            ...
            // 出力情報を設定
            $outForm = new Sgmov_Form_UserRobotCheck001Out();
     ...
     * @return Sgmov_Form_UserRobotCheck001Out 出力フォーム
     */
