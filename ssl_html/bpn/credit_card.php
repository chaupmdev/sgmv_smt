...
Sgmov_Lib::useView('bpn/CreditCard');
/**#@-*/

// 処理を実行
$view = new Sgmov_View_Bpn_CreditCard();
$forms = $view->execute();
...
// 開催期間開始日
$termFr=explode('-',$bpnOutForm->raw_term_fr);
// クレジットカード有効期限年のカウント
$inputCreditcardCnt=$bpnOutForm->input_creditcard_cnt;
...
<head>
    ...
    <title>卓上飛沫ブロッカーのお申し込み│ＳＧムービング株式会社＜ＳＧホールディングスグループ＞</title>
...
?>
    <div id="breadcrumb">
        <ul class="wrap">
            ...
            <li class="current">卓上飛沫ブロッカーのお申し込み</li>
        </ul>
    </div>
    <div id="main">
        <div class="wrap clearfix">
            <h1 class="page_title">卓上飛沫ブロッカーのお申し込み</h1>
            <form action="/bpn/check_credit_card" method="post">
...
?>
                        ...
                        <p class="under" style="line-height: normal;">
...
<?php } ?>
                    ...
                    <div class="result02" style="font-size:134%;text-align: center;">
                        ...
                        &nbsp;<span style="white-space: nowrap;"><?php echo number_format((int)$bpnOutForm->delivery_charge_buppan()); ?>円（税込）</span>
                    </div>
...
<?php
        echo Sgmov_View_Bpn_CreditCard::_createPulldown($bpnOutForm->card_expire_month_cds(), $bpnOutForm->card_expire_month_lbls(), $bpnOutForm->card_expire_month_cd_sel());
?>
                                </select>
                                月
                            </label>
...
<?php
        echo Sgmov_View_Bpn_CreditCard::_createPulldown($bpnOutForm->card_expire_year_cds(), $bpnOutForm->card_expire_year_lbls(), $bpnOutForm->card_expire_year_cd_sel());
?>
                                    </select>
                                    年
                                </label>
                        ...
                        <dl>
                            ...
                            <dd<?php if (isset($e) && $e->hasErrorForId('top_card_number')) { echo ' class="form_error"'; } ?>>
                                <input autocapitalize="off" class="w_280" id="card_number" inputmode="numeric" maxlength="16" name="card_number" data-pattern="^\d+$" placeholder="例）9999999999999999" type="<?php echo $inputTypeNumber; ?>" value="<?php echo $bpnOutForm->card_number(); ?>" />
                            </dd>
                        </dl>
                        <dl>
                            <dt class="condition">
                            ...
                            <dd<?php if (isset($e) && $e->hasErrorForId('top_security_cd')) { echo ' class="form_error"'; } ?>>
                                <input autocapitalize="off" class="w_70" id="security_cd" inputmode="numeric" maxlength="4" name="security_cd" data-pattern="^\d+$" placeholder="例）9999" type="<?php echo $inputTypeNumber; ?>" value="<?php echo $bpnOutForm->security_cd(); ?>" />
                    ...
                    <div class="btn_area">
                        ...
                        if(empty($comiketId)): ?>
                            <a class="back" href="/bpn/input/<?php echo $bpnOutForm->shikibetsushi();?>/<?php echo $bpnOutForm->bpn_type();?>/<?php echo $bpnOutForm->shohin_pattern();?>?clr=1">修正する</a>
                        <?php else: ?>
                            <a class="back" href="/bpn/input2/<?php echo $bpnOutForm->shikibetsushi();?>/<?php echo $bpnOutForm->bpn_type();?>/<?php echo $bpnOutForm->shohin_pattern();?>?clr=1">修正する</a>
                        <?php endif; ?>
...
?>
    ...
    <!--<![endif]-->
    <script charset="UTF-8" type="text/javascript" src="/js/common.js?<?php echo $strSysdate; ?>"></script>
    ...
    <script charset="UTF-8" type="text/javascript" src="/bpn/js/credit_card.js?<?php echo $strSysdate; ?>"></script>
    <script>
        $(function() {
