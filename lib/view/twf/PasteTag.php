...
class Sgmov_View_Twf_PasteTag {
	...
	private function addSheet(&$pdf, &$comiket, $page) {
		...
		$pdf->Cell ( 90, 19, $comiket ['event_name'] . '2024', 0, 0, 'L' );
		...
		$pdf->SetXY ( $x=57, $y=60 + $btm );
			...
			$pdf->Cell ( 119, 18, $comiket ['building_name'].$comiket ['booth_position'], 0, 0, 'L', 1 );

		// ブース名（固定文言）
		$pdf->SetXY ( $x=26, $y=83 + $btm );
			$this->setColor ( $pdf, $whichDay, 2 );
			$pdf->SetFont ( 'DejaVu', '', 14 );
			$pdf->Cell ( 26, 26, 'ブース名', 0, 0, 'C', 1 );
