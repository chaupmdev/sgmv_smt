 ...
 */
class Sgmov_View_Unf_Complete extends Sgmov_View_Unf_Common {
    ...
    public function __construct() {
        ...
        $this->_BcmView             = new Sgmov_View_Unf_Bcm();
    ...
    public function executeInner($argBackInputPaht="") {
        ...
        //チケットの確認と破棄 TODO
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_UNF003, $this->_getTicket());
        ...
        $inForm['comiket_toiawase_no2'] = $this->getToiawaseNo();
        ...
        // ベリトランス決済処理
        $registerData = $this->payment($inForm);
    ...
    private function checkHannyuArrivalDate($db, $inForm, $eveSubData, $session){
        ...
        if ($dateChArrival->format('Y-m-d H:i:s') <= $dateChNow->format('Y-m-d H:i:s')) {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/".$this->_DirDiv."/error/?t=受付時間が終了しました&m=集荷希望日：{$dispDate}の受付時間が終了しました");
            exit;
        }
        ...
        $dateSelectYMD = $yearColl. '/'.$monthColl.'/'.$dayColl;
        Sgmov_Component_Log::debug('#################### Check Collect Date confirm');
        Sgmov_Component_Log::debug($dateSelectYMD);
        Sgmov_Component_Log::debug($nextDateYMD);
        if ($dateSelectYMD < $nextDateYMD) {
            // セッション情報を破棄
            $session->deleteForm(self::FEATURE_ID);
            Sgmov_Component_Redirect::redirectPublicSsl("/".$this->_DirDiv."/error/?t=受付時間が終了しました&m=集荷希望日：{$yearColl}/{$monthColl}/{$dayColl}の受付時間が終了しました");
            exit;
        }
     ...
     * @param str   type
     */
    protected function payment($inForm){
        ...
        $inForm['sgf_res_transactionId'] = NULL;
        $inForm['sgf_res_autoAuthoriresult'] = NULL;
        $inForm['sgf_res_shopOrderId'] = NULL;
    ...
    protected function registerComiket($db, $inForm, $comiketId, $argBackInputPath){
        ...
        try {
            ...
            $db->rollback();
            $_SESSION["Sgmov_View_Unf.inputErrorInfo"] = array("db_insert_error" => "・データベースの登録・更新に失敗しました。");
    ...
    protected function sendCompleteMail2($li, $inForm, $type= '') {
        ...
        // 往路か復路かの区分
        $comiketChoice = $inForm['comiket_detail_type_sel2'];
        ...
        if ((!empty($li["treeData"]['mail']) && !empty($li["treeData"]['merchant_result']))
            ...
            $this->sendCompleteMail($li["treeData"], $li["treeData"]['mail'], $ccMail, $comiketChoice);
        }
    }
...
Sgmov_Component_Log::debug($pay_limit_convenience_store);
    ...
    public function _createDataByInForm($db, $inForm, $toiawaseNo = '0123456789') {
        ...
        $inForm['pay_limit'] = 0; //$this->_getPayLimit($db, $inForm); //決済が無い為、設定しない
     ...
     * @param Sgmov_Form_Una003In $inForm 入力フォーム
