...
Sgmov_Lib::useForms(array('Error', 'PcrSession', 'Pcr001In'));
/**#@-*/
/**
 ...
 * @copyright  2009-2010 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Pcr_CheckInput extends Sgmov_View_Pcr_Common {
    ...
    public function executeInner() {
        ...
        if ($errorForm->hasError()) {
            if ($inForm->req_flg == '1') {
                ...
                Sgmov_Component_Redirect::redirectPublicSsl('/pcr/input');
            }
        }
        if ($inForm->req_flg == '1') {
            Sgmov_Component_Redirect::redirectPublicSsl('/pcr/confirm_call');
        } else {
    ...
    public function _createInFormFromPost($post, $creditCardForm) {
        ...
        $inForm->chb_agreement = isset($post['chb_agreement']) ? $post['chb_agreement'] : "";
...
//        }
        ...
        if($convenience && $isTravelErr) {
            ...
            if($v2->isValid()) {
                ...
                $errorForm->addError('top_travel_cd_sel_convenience', 'コンビニ決済のご依頼受付期間は終了しています（乗船日の7日前まで）');
            }
        }
