 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Yid_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'YID';
     ...
     * イベントサブID
     */
    const EVENT_SUB_ID = '2601';

    /**
     * TWF001の画面ID
     */
    const GAMEN_ID_TWF001 = 'YID001';

    /**
     * TWF002の画面ID
     */
    const GAMEN_ID_TWF002 = 'YID002';

    /**
     * TWF003の画面ID
     */
    const GAMEN_ID_TWF003 = 'YID003';
...
///////////////////////////////////////////////////////////////////////////////////
     ...
     * @param type $inForm
     * @param Sgmov_Form_Twf001Out $outForm
     * @return type
     */
    protected function createOutFormByInForm($inForm, $outForm = array()) {
        ...
        $outForm->raw_building_name_sel = $inForm["building_cd_and_name_sel"];
...
//        $outForm->raw_comiket_detail_inbound_note = $inForm["comiket_detail_inbound_note"];
    ...
    public function setDispEvent(&$dispItemInfo){
        ...
        foreach($dispItemInfo['event_alllist'] as $k=>$v){
            if(isset($dispItemInfo['eventsub_selected_data']['event_id']) && $v['id']==$dispItemInfo['eventsub_selected_data']['event_id']){
                    ...
                    // ロゴ画像ファイル名：例)logo.twf.gif logo_[イベント識別子].gif
                    $dispItemInfo['dispEvent']['logoImg']   ='/'.$this->_DirDiv.'/images/logo_'.$this->_DirDiv.'.jpg';
                    // 貼付票のサンプル：例)paste_tag_[イベント識別子].pdf
                    $dispItemInfo['dispEvent']['pasteTag1']  ='/'.$this->_DirDiv.'/pdf/paste_tag/paste_tag_'.$this->_DirDiv.'.pdf'.$cacheStr;
                    // 貼付票のサンプル：例)paste_tag_[イベント識別子].pdf
                    //$dispItemInfo['dispEvent']['pasteTag2']  ='/'.$this->_DirDiv.'/pdf/paste_tag/paste_tag_'.$this->_DirDiv.'_2.pdf'.$cacheStr;
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                    ...
                    if(isset($valDet["comiket_box_list"])){
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Yid_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Yid_Common::CURRENT_TAX));
                        // // 税込
    ...
    public function _createComiketInsertDataByInForm($inForm, $id, $type="") {
        ...
        $buildingNameRes = $this->_BuildingService->fetchBuildingNameByCd($db, $buildingCd, $inForm['eventsub_sel']);
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($eventsubData['building_display'] !== '0') {
                //GiapLN fix bug send mail item 【エリア、ブース番号】 2024.07.10
                $building = '';
                ...
                $data['comiket_building_name'] = PHP_EOL . '【エリア、ブース番号】' . $building . $comiket['booth_position'] . " " . $comiket['booth_num'];
            }
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    $data['manual_url'] = "【搬出の出展ガイドはこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$this->_DirDiv}.pdf";
                    ...
                    $data['manual_url'] = "【搬入の出展ガイドはこちら】". PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/pdf/manual/manual_{$this->_DirDiv}.pdf";
            ...
            if(($comiket['choice'] == "1" || $comiket['choice'] == "3")
                ...
                $data['paste_tag_url'] = "【配達カードURL】" . PHP_EOL . $urlPublicSsl . "/".$this->_DirDiv."/paste_tag/{$pasteTagId}/";
            }
...
";
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        $sendFileName = 'TWF_' . date ( 'YmdHis' ) . '.csv';
        $boundary = "-----" . md5 ( uniqid () );
