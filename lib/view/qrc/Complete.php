...
Sgmov_Lib::useForms(array('Error', 'QrcSession', 'Qrc004Out',));
 ...
 */
class Sgmov_View_Qrc_Complete extends Sgmov_View_Qrc_Common
{
    ...
    public function __construct()
    {
        ...
        $this->_BcmView             = new Sgmov_View_Qrc_Bcm();
    ...
    public function executeInner($argBackInputPaht = "")
    {
        ...
        //チケットの確認と破棄 TODO
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_QRC003, $this->_getTicket());
        ...
        $outForm = new Sgmov_Form_Qrc004Out();
        ...
        //下記は実装中で未完成 2022.03.23 Tamashiro
        //QR決済完了通知
        // // $dirDiv = Sgmov_Lib::setDirDiv(dirname(__FILE__));
        // // Sgmov_Lib::useView($dirDiv . '/kessai_kanryo.php');
        // $api_url = 'https://sagawa-mov-test04.media-tec.jp/qrc/kessai_kanryo'; //開発SV
        // // $api_url = $dirDiv . './kessai_kanryo.php'; //開発SV
        // // $api_url = 'https://sgmoving/qrc/kessai_kanryo'; //local
        // $param = array(
        ...
        // );
        // $contents_array = $this->post_request($api_url, $param);
    ...
    protected function registerComiket($db, $inForm, $comiketId, $backInputPath, $sessionForm, $session)
    {
        ...
        try {
            ...
            // ※重複登録のチェック時に使用するために保存しておく
            $QR_Check = $this->_Comiket->fetchQRById($db, $inForm["delivery_slip_no"], $inForm["qr_kessai_meisai_id"]);
            ...
            $db->rollback();
            $_SESSION["Sgmov_View_Qrc.inputErrorInfo"] = array("db_insert_error" => "・データベースの登録・更新に失敗しました。");
...
*/
    ...
    public function errorInformation($parm = array())
    {
        ...
        $mail_to = Sgmov_Component_Config::getLogMailTo();
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($parm, dirname(__FILE__) . '/../../mail_template/Qrc_error.txt', $mail_to);
    }

    /**
     * 決済用データの入力値の妥当性検査を行います。
     * @param $checkForm 決済用データ
     * @param Sgmov_Form_Qrc003In $inForm 入力フォーム
     ...
     * QR決済完了通知
     */
    function post_request($api_url, $param)
    {
        //リクエスト時のオプション指定
        $options = array(
            'http' => array(
        ...
        //リクエスト実行
        $contents = @file_get_contents($api_url, false, stream_context_create($options));
