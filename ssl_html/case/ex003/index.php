<!doctype html>
<html lang="ja">
<head>
<meta charset="utf-8">
<meta name="format-detection" content="telephone=no">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="description" content="国立科学博物館様の事例紹介ページです。">
<meta property="og:locale" content="ja_JP">
<meta property="og:type" content="website">
<meta property="og:url" content="https://www.sagawa-mov.co.jp/case/ex003/">
<meta property="og:site_name" content="<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/meta-ttl-ogp.php'); ?>">
<meta property="og:image" content="/img/ogp/og_image_sgmv.jpg">
<meta property="twitter:card" content="summary_large_image">
<link rel="shortcut icon" href="/img/common/favicon.ico">
<link rel="apple-touch-icon" sizes="192x192" href="/img/ogp/touch-icon.png">
<title>国立科学博物館 様｜事例紹介｜<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/meta-ttl.php'); ?></title>
<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/common_style.php'); ?>
<link rel="stylesheet" type="text/css" href="/css/case_ex007.css">
<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/common_script.php'); ?>
<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/analytics_head.php'); ?>
</head>
<body>
<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/analytics_body.php'); ?>
<div id="container">
	<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/header.php'); ?>
    <main id="main" role="main">
		<div class="pageTitle">
			<div class="comBox">
				<p class="topLead"><span>事例紹介<small class="cor01">美術品輸送</small></span>国立科学博物館 様</p>
				<ul id="pagePath">
					<li><a href="/"><img class="home" src="/img/common/icon54.jpg" alt=""></a></li>
					<li><a href="/case/">事例紹介</a></li>
					<li>国立科学博物館 様</li>
				</ul>
			</div>
		</div>
		<div class="mainBox">
			<article id="conts" class="ex003">
				<div class="titleBox fadeInUp">
					<h1 class="headLine02">資料の借用から展示まで、美術品の取り扱い技術に長けた専門スタッフが作業</h1>
					<div class="btn"><a href="/service/delivery/art/"><small>ご利用いただいたサービス</small>美術品輸送</a></div>
				</div>
				<p class="text01 fadeInUp">山口県の土井ヶ浜遺跡をはじめ、日本各地の弥生時代遺跡から出土した人骨資料を借用・展示いたしました。<br>
					資料の梱包・集荷から作品の展示作業、会期終了後所蔵者へ返却するまでを一貫して美術品の取り扱いに熟練した専門スタッフが行い、また輸送には空調エアサスのついた美術品専用車を使用いたしました。</p>
				<ul class="imgList threeCollumn fadeInUp">
					<li><img src="/img/case/ex003/photo02.jpg" alt="資料の借用から展示まで、美術品の取り扱い技術に長けた専門スタッフが作業"></li>
					<li><img src="/img/case/ex003/photo03.jpg" alt="資料の借用から展示まで、美術品の取り扱い技術に長けた専門スタッフが作業"></li>
				</ul>
				<h2 class="headLine03 fadeInUp">クライアント概要</h2>
				<div class="imgBox fadeInUp">
					<div class="textBox">
						<h3>国立科学博物館　砂丘に眠る弥生人－山口県土井ヶ浜遺跡の半世紀－</h3>
						<dl>
							<dt>所在地</dt>
							<dd>〒110-8718<br>
								東京都台東区上野公園7-20</dd>
						</dl>
					</div>
				</div>
			</article>
			<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/side-2column-case.php'); ?>
		</div>
		<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/footer-contact.php'); ?>
    </main>
	<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/footer.php'); ?>
</div>
<?php require_once($_SERVER['DOCUMENT_ROOT'] . '/include/common_script_foot.php'); ?>
<script src="/js/sidebar.js"></script>
<script>
	$(function(){
		$('#gHeader .hBox #gNavi .hLinkList > li').eq(2).addClass('current');
	})
</script>
</body>
</html>