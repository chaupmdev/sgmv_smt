...
Sgmov_Lib::useView('evp/Common', 'CommonConst');
Sgmov_Lib::useView('evp/Bcm');
...
class Sgmov_View_Evp_Complete extends Sgmov_View_Evp_Common {
    ...
    public function executeInner($argBackInputPaht="") {
        ...
        //GiapLN implement SMT6-129 2022/04/12
    ...
    protected function checkHannyuArrivalDate($db, $inForm, $eveSubData){
        ...
        if ($dateChArrival->format('Y-m-d H:i:s') <= $dateChNow->format('Y-m-d H:i:s')) {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/evp/error/?t=受付時間が終了しました&m=お預かり日：{$dispDate}の受付時間が終了しました");
            exit;
        }
        ...
        if ($dateColl <= $dateNow) {
            // セッション情報を破棄
            $session->deleteForm(self::FEATURE_ID);
            Sgmov_Component_Redirect::redirectPublicSsl("/evp/error/?t=受付時間が終了しました&m=お預かり日：{$yearColl}/{$monthColl}/{$dayColl}の受付時間が終了しました");
            exit;
        }
    }
    ...
    protected function registerComiket($db, $inForm, $comiketId, $backInputPath){
        ...
        try {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/evp/{$backInputPath}/");
            exit;
        }
        $db->commit();
