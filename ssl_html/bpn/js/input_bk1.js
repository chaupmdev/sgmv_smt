...
/////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // バシ　追加 2020/03/04 {
    function dispBuppan(){
        ...
        var selectedVal = $('input[name="comiket_detail_type_sel"]:checked').val();
...
//                $('.paste_tag_link').attr("href", "/eve/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf");
//                $('.manual_link').attr("href", "/eve/pdf/manual/manual_" + eventsubSel + ".pdf");

                $('.manual').show(g_fifoVal);
                eventSelText = eventSelText.replace(/\s+/g, "");
                $('.manual_link').attr("href", "/eve/pdf/manual/" + eventSelText +  ".pdf" + '?' + strCurrentDate);
            } else {
                $('.manual').hide(g_fifoVal);
            }
            if(g_eventsubList[eventsubSel]['is_paste_display']){
                $('.paste_tag').show(g_fifoVal);
                $('.paste_tag_link').attr("href", "/eve/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf" + '?' + strCurrentDate);
            } else {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function setBuppanOpenStatus(){
        var showFlg = false;
        $('.buppanTable > tbody  > tr').each(function(index, tr) {
            var boxName = ($(this).attr('class')).substring(4);
            var txtValue =   $('.service-buppan-item .box_' + boxName + ' input').val();
            if(txtValue !== ""){
                showFlg = true;
            }
        });
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    function getBoxData() {
        ...
        sgwns.api('/common/php/SearchBox.php', getFormData(), (function(data) {
            $('.comiket-box-outbound-num,.comiket-box-inbound-num,.comiket-box-buppan-num').children().remove();
            ...
            if($('input[name="comiket_detail_type_sel"]:checked').val() == "1" ) { // 搬入
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img class='dispSeigyoPC' src='/eve/images/about_boxsize.png' width='100%'/>";
                } else {
//                    htmlCode += "<img src='/eve/images/about_boxsize.png' width='100%' style='visibility:hidden;'/>";
                }
                htmlCode += "</td></tr></table>";

                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL && $('.dispSeigyoSP').length === 0) {
                    htmlCode += "<div class='dispSeigyoSP'><img src='/eve/images/about_boxsize.png' width='250px' style='margin-top: 1em;' /></div>";
                }
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img src='/eve/images/about_boxsize.png' width='100%'/>";
                }
                htmlCode += "</td></tr></table>";
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode2 += "<img src='/eve/images/about_boxsize.png' width='100%'/>";
                }
            ...
            // 搬出、搬入・搬出、物販
            var comiketDetail = ["2", "3", "5"];
            // 物販 追加 2020/03/09
            if(comiketDetail.indexOf($('input[name="comiket_detail_type_sel"]:checked').val()) !== -1) { // 物販
               ...
               var htmlCode = "";
                ...
                htmlCode += "<table class = 'buppanTable'>";
                var boxName = "";
                for(var i = 0; i < data.buppan.length; i++) {
                   boxName = data.buppan[i].name_display;
                    if(data.buppan[i].name_display == null || data.buppan[i].name_display == ""){
                        boxName =  data.buppan[i].name;
                    }
                   htmlCode += "<tr class = 'box_"+data.buppan[i].size_display+"'><td class='comiket_box_item_name'>" + boxName + "&nbsp;</td><td class='comiket_box_item_value' style='white-space: nowrap;'><input autocapitalize='off' class='number-only comiket_box_item_value_input' maxlength='2' inputmode='numeric' name='comiket_box_buppan_num_ary[" + data.buppan[i].id  + "]' data-pattern='^\d+$' placeholder='例）1' type='" + inputTypeNumber + "' style='min-width: 60px;' value='' />枚</td></tr>";
                }
                
                htmlCode += "</table>";

                htmlCode += "</td><td style='vertical-align: middle;text-align: right;'>";
                htmlCode += "<img class='dispSeigyoPC' src='/dsn/images/about_boxsize.png' style='width:80%;'></td>";
                htmlCode += "</tr></table>";

                $('.comiket-box-buppan-num').append(htmlCode);
