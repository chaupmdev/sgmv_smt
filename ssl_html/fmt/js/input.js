...
//                $('.paste_tag_link').attr("href", "/fmt/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf");
//                $('.manual_link').attr("href", "/fmt/pdf/manual/manual_" + eventsubSel + ".pdf");

                $('.manual').show(g_fifoVal);
                eventSelText = eventSelText.replace(/\s+/g, "");
                $('.manual_link').attr("href", "/fmt/pdf/manual/" + eventSelText +  ".pdf" + '?' + strCurrentDate);
            } else {
                $('.manual').hide(g_fifoVal);
            }
            if(g_eventsubList[eventsubSel]['is_paste_display']){
                $('.paste_tag').show(g_fifoVal);
                $('.paste_tag_link').attr("href", "/fmt/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf" + '?' + strCurrentDate);
            } else {
    ...
    function getBoxData() {
        sgwns.api('/common/php/SearchBox.php', getFormData(), (function(data) {
            ...
            if($('input[name="comiket_detail_type_sel"]:checked').val() == "1" ) { // 搬入
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img class='dispSeigyoPC' src='/fmt/images/about_boxsize.png' width='100%'/>";
                } else {
//                    htmlCode += "<img src='/fmt/images/about_boxsize.png' width='100%' style='visibility:hidden;'/>";
                }
                htmlCode += "</td></tr></table>";

                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL && $('.dispSeigyoSP').length === 0) {
                    htmlCode += "<div class='dispSeigyoSP'><img src='/fmt/images/about_boxsize.png' width='250px' style='margin-top: 1em;' /></div>";
                }
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img src='/fmt/images/about_boxsize.png' width='100%'/>";
                }
                htmlCode += "</td></tr></table>";
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode2 += "<img src='/fmt/images/about_boxsize.png' width='100%'/>";
                }
...
//                $('.comiket_detail_outbound_collect_time_sel').hide(g_fifoVal);
    ...
    // 出展イベントの顧客区分
    var customerKbnFnc = (function () {
        var customerKbn = $('input[name=comiket_customer_kbn_sel]:checked').val();
        if (customerKbn == '1') {
            $('.booth_name_no').show(g_fifoVal);
        } else {
            $('.booth_name_no').hide(g_fifoVal);
        }
    });
    customerKbnFnc();
    $('input[name=comiket_customer_kbn_sel]').on('click', function() {
        getBoxData();
        customerKbnFnc();
        $('input[name=comiket_booth_name]').val('');
        $('select[name=building_booth_position_sel]').val('');
    ...
    var clickInbBinshuSel = function() {
        ...
        if (r == '0') {
             $('.msg-hikyaku_coolbin').hide();
             $('.msg-hikyaku_normalbin').show();
             return;
         }
         var r = $('input#comiket_detail_inbound_binshu_kbn_sel2:checked').val();
         if (r == '1') {
             $('.msg-hikyaku_coolbin').show();
             $('.msg-hikyaku_normalbin').hide();
