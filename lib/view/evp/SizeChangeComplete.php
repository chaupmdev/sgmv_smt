 ...
 */
class Sgmov_View_Evp_SizeChangeComplete extends Sgmov_View_Evp_Complete {
...
Sgmov_Component_Log::debug($inForm);
        ...
        //2022/07/27 TuanLK fix bug $comiketInfo['comiket_detail_list'] 2倍
        ...
        $this->checkReqDate($inForm['comiket_id'], 'サイズ変更');
        ...
        if (@empty($boxNumTotal)) {
            ...
            // 先に親処理の実行(セッションはなくなる)
            $parentRes = parent::executeInner("size_change");
        }
...
//        }
    ...
    protected function sendCompleteMail2($li, $inForm) {
        ...
        $type = $li['treeData']['comiket_detail_list'][0]['type']; // 往路か復路かの区分
        // メール送信
