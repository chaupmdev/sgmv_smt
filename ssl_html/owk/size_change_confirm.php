...
Sgmov_Lib::useView('owk/SizeChangeConfirm');

/**#@-*/

// 処理を実行
$view = new Sgmov_View_Owk_SizeChangeConfirm();
$forms = $view->execute();
...
<head>
    ...
    <link href="/owk/css/eve.css" rel="stylesheet" type="text/css" />
    <!--[if lt IE 9]>
...
?>
    ...
    <div id="main">
        <div class="wrap clearfix">
            ...
            <div class="section">
                <form action="/owk/size_change_complete" data-feature-id="<?php echo Sgmov_View_Owk_Common::FEATURE_ID ?>" data-id="<?php echo Sgmov_View_Owk_Common::GAMEN_ID_OWK001 ?>" method="post">
...
<?php
        $eventName = Sgmov_View_Owk_Confirm::_getLabelSelectPulldownData($eveOutForm->event_cds(), $eveOutForm->event_lbls(), $eveOutForm->event_cd_sel());
        echo $eventName;
?>
                                    &nbsp;
...
<?php
        $eventsubName = Sgmov_View_Owk_Confirm::_getLabelSelectPulldownData($eveOutForm->eventsub_cds(), $eveOutForm->eventsub_lbls(), $eveOutForm->eventsub_cd_sel());
        echo $eventsubName;
?>
                                    </span>
                                    ...
                                    <?php foreach($dispItemInfo['comiket_div_lbls'] as $key => $val) : ?>
                                        <?php if ($eveOutForm->comiket_div() == $key) : ?>
                                            <?php if($eveOutForm->event_cd_sel() == '2') :  // コミケの場合?>
                                                ...
                                                <?php if($eveOutForm->comiket_div() == Sgmov_View_Owk_Common::COMIKET_DEV_BUSINESS) : ?>
                                                <?php $val = '請求書にて請求'; ?>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                            <?php echo $val; ?>
                            ...
                            <?php if ($eveOutForm->comiket_div() == Sgmov_View_Owk_Common::COMIKET_DEV_BUSINESS) : // 法人 and 顧客コードを使用する ?>
                                <dl>
...
<?php /**********************************************************************************************************************/ ?>
                            <div class="dl_block input-inbound comiket_block">
                                ...
                                <dl>
                                    ...
                                    <dd>
<?php
        echo Sgmov_View_Owk_Confirm::_getLabelSelectPulldownData($eveOutForm->comiket_detail_inbound_pref_cds(), $eveOutForm->comiket_detail_inbound_pref_lbls(), $eveOutForm->comiket_detail_inbound_pref_cd_sel());
?>
                                    </dd>
                                </dl>
                                <dl>
                                ...
                                <dl>
                                    ...
                                    <dd>
                                        ...
                                        （<?php echo Sgmov_View_Owk_Confirm::_getWeek($eveOutForm->comiket_detail_inbound_collect_date_year_sel(), $eveOutForm->comiket_detail_inbound_collect_date_month_sel(), $eveOutForm->comiket_detail_inbound_collect_date_day_sel()); ?>）
                                        &nbsp;
<?php
      //  echo Sgmov_View_Owk_Confirm::_getTimeFormatSelectPulldownData($eveOutForm->comiket_detail_inbound_collect_time_cds(), $eveOutForm->comiket_detail_inbound_collect_time_lbls(), $eveOutForm->comiket_detail_inbound_collect_time_sel());
?>
<span>17:00～21:00</span>
                                ...
                                <dl>
                                    ...
                                    <dd>
                                        ...
                                        （<?php echo Sgmov_View_Owk_Confirm::_getWeek($eveOutForm->comiket_detail_inbound_delivery_date_year_sel(), $eveOutForm->comiket_detail_inbound_delivery_date_month_sel(), $eveOutForm->comiket_detail_inbound_delivery_date_day_sel()); ?>）
                                        &nbsp;
<?php
        if ( $view->checkColAndDelTime("inbound", $eveOutForm->comiket_div(), $eveOutForm->comiket_detail_inbound_service_sel(), $dispItemInfo["eventsub_selected_data"]) ) :
            echo Sgmov_View_Owk_Confirm::_getTimeFormatSelectPulldownDataForInBound($eveOutForm->comiket_detail_inbound_delivery_time_cds(), $eveOutForm->comiket_detail_inbound_delivery_time_lbls(), $eveOutForm->comiket_detail_inbound_delivery_time_sel());
        endif;
?>
                                        <br/>
                                        <br/>
...
<?php if($eveOutForm->comiket_div() == Sgmov_View_Owk_Common::COMIKET_DEV_BUSINESS) : // 法人 ?>

<?php endif; ?>
<?php if($eveOutForm->comiket_div() == Sgmov_View_Owk_Common::COMIKET_DEV_INDIVIDUA) : // 個人 ?>
                        <!--▼お支払い方法-->

    <?php if ($eveOutForm->comiket_payment_method_cd_sel() == '2') : // クレジット ?>
                        <div class="btn_area">
                            <a class="back" href="/owk/<?php echo $dispItemInfo['back_input_path']; ?>/"> 修正する</a>
                        </div>
                        ...
                        <div class="btn_area">
                            ...
                            <?php if (@empty($dispItemInfo['is_cancel'])) : ?>
                                <a class="back" href="/owk/size_change_credit_card/">修正する</a>
    ...
    <?php if ($eveOutForm->comiket_payment_method_cd_sel() == '3') : // 電子マネー ?>
       ...
       <strong class="red">※電子マネーでのお支払いは、￥10,000を超える場合はお取り扱い出来ません。</strong>
        <div class="btn_area">
            <a class="back" href="/owk/size_change/">修正する</a>
            <?php if (@empty($dispItemInfo['is_cancel'])) : ?>
                <input id="submit_button" type="submit" name="submit" value="入力内容を送信する" />
            <?php else: ?>
                <input id="submit_button" type="submit" name="submit" value="キャンセル送信する" />
