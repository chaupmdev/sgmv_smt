...
class Sgmov_View_Dsn_CheckInput extends Sgmov_View_Dsn_Common {
    ...
    public function executeInner() {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_HOT001, $this->_getTicket());
...
//        $inForm->comiket_staff_tel = @str_replace(array('-', 'ー', '−', '―', '‐', 'ｰ'), "-", mb_convert_kana(filter_input(INPUT_POST, 'comiket_staff_tel'), 'rnask', 'UTF-8'));
        ...
        $inForm->comiket_staff_sei = mb_convert_kana(filter_input(INPUT_POST, 'comiket_personal_name_sei'), 'RNASKV', 'UTF-8');
        $inForm->comiket_staff_mei = mb_convert_kana(filter_input(INPUT_POST, 'comiket_personal_name_mei'), 'RNASKV', 'UTF-8');
        $inForm->comiket_staff_sei_furi = '-';
        $inForm->comiket_staff_mei_furi = '-';
        $inForm->comiket_staff_tel = @str_replace(array('-', 'ー', '−', '―', '‐', 'ｰ'), "-", mb_convert_kana(filter_input(INPUT_POST, 'comiket_tel'), 'rnask', 'UTF-8'));
