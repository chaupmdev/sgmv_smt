...
//                $('.paste_tag_link').attr("href", "/tme/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf");
//                $('.manual_link').attr("href", "/tme/pdf/manual/manual_" + eventsubSel + ".pdf");

                $('.manual').show(g_fifoVal);
                eventSelText = eventSelText.replace(/\s+/g, "");
                $('.manual_link').attr("href", "/tme/pdf/manual/" + eventSelText +  ".pdf" + '?' + strCurrentDate);
            } else {
                $('.manual').hide(g_fifoVal);
            }
            if(g_eventsubList[eventsubSel]['is_paste_display']){
                $('.paste_tag').show(g_fifoVal);
                $('.paste_tag_link').attr("href", "/tme/pdf/paste_tag/paste_tag_" + eventsubSel + ".pdf" + '?' + strCurrentDate);
            } else {
...
//-----------------------------------------------------------------------------------------------------
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img class='dispSeigyoPC' src='/tme/images/about_boxsize.png' width='100%'/>";
                } else {
//                    htmlCode += "<img src='/tme/images/about_boxsize.png' width='100%' style='visibility:hidden;'/>";
                }
                htmlCode += "</td></tr></table>";

                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL && $('.dispSeigyoSP').length === 0) {
                    htmlCode += "<div class='dispSeigyoSP'><img src='/tme/images/about_boxsize.png' width='250px' style='margin-top: 1em;' /></div>";
                }
...
//-----------------------------------------------------------------------------------------------------
                ...
                for(var i = 0; i < data.inbound.length; i++) {
                    ...
                    htmlCode += "<tr><td class='comiket_box_item_name' maxlength='2' >" + data.inbound[i].name + "&nbsp;</td><td class='comiket_box_item_value'>";
                    htmlCode += "<input autocapitalize='off' class='number-only comiket_box_item_value_input' maxlength='2' inputmode='numeric' name='comiket_box_inbound_num_ary["
                                + data.inbound[i].id  + "]' data-pattern='^\d+$' placeholder='例）1' type='" 
                                + inputTypeNumber + "' value='' />個</td></tr>";
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                    htmlCode += "<img src='/tme/images/about_boxsize.png' width='100%'/>";
                }
...
//-----------------------------------------------------------------------------------------------------
                ...
                if($('input[name="comiket_div"]:checked').val() ==  G_DEV_INDIVIDUAL) {
                   htmlCode2 += "<img src='/tme/images/about_boxsize.png' width='100%'/>";
                }
...
//            window.console.log(data);
    ...
    $('#comiket_detail_type_sel2').trigger('click');
