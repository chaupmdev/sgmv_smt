        ...
        if ($inForm['comiket_detail_type_sel'] == "1" || $inForm['comiket_detail_type_sel'] == "2" || $inForm['comiket_detail_type_sel'] == "3" || $inForm['comiket_detail_type_sel'] == "5") {
            ...
            foreach ($procList as $keyTree => $valTree) {
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Eve_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Eve_Common::CURRENT_TAX));
                        // // 税込
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/eve/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/eve/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/eve/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/eve/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
        ...
        try {
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                ...
                if($eventData['id'] === '2') { // コミケ
                    ...
                    $data['comiket_building_name'] = PHP_EOL . '【ブース番号】' . $blockStr . $comiket['booth_num'];
                } else {
                    $data['comiket_building_name'] = PHP_EOL . '【館名】' . $comiket['building_name'] . "ホール " . $blockStr . $comiket['booth_num'];
                }
            ...
            // 2022-03-11 ToanDD3 implement SMT6-63
            ...
            // $data['edit_url'] = $urlPublicSsl . "/eve/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            // 2022-03-28 ToanDD3 fix SMT6-120
            if ($isSecurity) {
                if ($_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                    $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm=eve'; 
                } else {
                    $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=eve&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
                }   
            } else {
                $data['edit_url'] = $urlPublicSsl . "/eve/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            }
            ...
            // 説明書URL
//            $data['manual_url'] = $urlPublicSsl . "/eve/pdf/manual/manual_{$comiket["eventsub_id"]}.pdf";
            $data['manual_url'] = '';
...
//                $data['manual_url'] = "【説明書URL】" . PHP_EOL . $urlPublicSsl . "/eve/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
//            }
//            $data['manual_url'] = $urlPublicSsl . "/eve/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
...
//                    && $eventData['id'] !== '4')  {
                ...
                // 2022-03-24 ToanDD3 implement SMT6-84
                // $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/eve/paste_tag/{$pasteTagId}/";
                if (!$isSecurity) {
                    $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/eve/paste_tag/{$pasteTagId}/";
                } else if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 0){
                    $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/event/robotCheck?event_nm=eve&destination=paste_tag/{$pasteTagId}";
                }
            }
...
";
            ...
            // 2022-03-24 ToanDD3 implement SMT6-84
            // $data['cancel_url'] = $urlPublicSsl . "/eve/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            // $data['size_change_url'] = $urlPublicSsl . "/eve/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;;

            if (!$isSecurity) {
                $data['cancel_url'] = $urlPublicSsl . "/eve/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
                $data['size_change_url'] = $urlPublicSsl . "/eve/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;;
            } else {
                $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=eve&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
                $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=eve&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            }
            //-------------------------------------------------
            //メール送信実行
...
//                "/dsn_parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
    ...
    protected function redirectBackHistoryOrTopUrl() {
        ...
        $isSecurity = $this->isSecurityTaio();
        if ($isSecurity && isset($_SESSION['EVENT_LOGIN'])) {
            if ($_SESSION['EVENT_LOGIN']['user_type'] == '1') {
                $valBackTop = "申込履歴ページに戻る";
                $urlBackTop = $baseUrl."/event/inputHistory?event_nm=eve";
            } else {
