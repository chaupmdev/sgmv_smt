...
// 処理を実行
$view = new Sgmov_View_Qrc_Confirm();
$forms = $view->execute();
...
<body>
    ...
    <div id="main">
        <div class="wrap clearfix">
            ...
            <div class="section">
                <form action="/<?= $dirDiv ?>/complete" id="ev_form" data-feature-id="<?php echo Sgmov_View_Qrc_Common::FEATURE_ID ?>" data-id="<?php echo Sgmov_View_Qrc_Common::GAMEN_ID_QRA001 ?>" method="post">
                    <input name="ticket" type="hidden" value="<?php echo $ticket ?>" />
                    <input name="comiket_id" type="hidden" value="<?php echo $qrcOutForm->comiket_id(); ?>" />

                    <div class="section">


                        <?php if ($qrcOutForm->comiket_div() == Sgmov_View_Qrc_Common::COMIKET_DEV_INDIVIDUA) : // 個人
                        ?>

                            <?php if ($qrcOutForm->comiket_payment_method_cd_sel() == '2') : // クレジット
                            ?>
                                ...
                                <div class="dl_block">
                                    <dl>
                                        <dt>合計金額</dt>
                                        <dd>
                                            ￥<?php echo number_format($qrcOutForm->delivery_charge()) . PHP_EOL; ?>
                                            <?php if (intval($qrcOutForm->repeater_discount()) > 0) : ?>
                                                <span class="f80">※リピータ割引（<?php echo number_format($qrcOutForm->repeater_discount()); ?>円）が適用されました</span>
                                            <?php endif; ?>
                                        </dd>
                                    </dl>
                                    <dl>
                                        <dt>有効期限</dt>
                                        <dd>
                                            <?php echo $qrcOutForm->card_expire_year_cd_sel(); ?>年<?php echo $qrcOutForm->card_expire_month_cd_sel(); ?>月
                                        </dd>
                                    </dl>
                                    <dl>
                                        <dt>カード番号</dt>
                                        <dd>
                                            <?php echo str_repeat('*', strlen($qrcOutForm->card_number()) - 4) . substr($qrcOutForm->card_number(), -4) . PHP_EOL; ?>
                                            <span class="f80">※下4桁のみの表示となります</span>
                                        </dd>
                                    </dl>
                                    <dl>
                                        <dt>セキュリティコード</dt>
                                        <dd><?php echo $qrcOutForm->security_cd(); ?></dd>
                                    </dl>
                                    <dl>
                                        <dt>お支払い方法</dt>
                                        <dd>1回</dd>
                ...
                <script>
                    $(function() {
                        $('input[name="submit"]').on('click', function() {
                            ...
                            let s_tel = "<?php echo $qrcOutForm->comiket_detail_inbound_tel(); ?>";
                            $("<input>", {
                                type: "hidden",
