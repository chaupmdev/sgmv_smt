 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Dsn_CancelComp extends Sgmov_View_Dsn_Common {
...
//        Sgmov_Component_ErrorExit::errorExit(Sgmov_Component_ErrorCode::ERROR_VIEW_INVALID_PAGE_ACCESS);
        ...
        if(strlen($param) == 10 && is_numeric($param) && $param <= self::INT_MAX) {
            // 追加申込画面へ遷移
            Sgmov_Component_Log::debug ( '追加申込へ遷移：From CancelComp' );
        ...
        if(!empty($param)) {
            ...
            if($param > self::INT_MAX){
                Sgmov_Component_Log::debug ( 'int型(2147483648)を超えている' );
                Sgmov_Component_ErrorExit::errorExit(Sgmov_Component_ErrorCode::ERROR_VIEW_INVALID_PAGE_ACCESS);
            }
