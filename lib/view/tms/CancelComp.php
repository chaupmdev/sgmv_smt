 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Tms_CancelComp extends Sgmov_View_Tms_Common {
...
Sgmov_Component_Log::debug ( '################### 100-1' );
        // セッションに情報があるかどうかを確認
        $session = Sgmov_Component_Session::get();
Sgmov_Component_Log::debug ( '################### 100-2' );
        // DB接続
        $db = Sgmov_Component_DB::getPublic();
Sgmov_Component_Log::debug ( '################### 100-3' );
        $param = filter_input(INPUT_POST, 'param');

Sgmov_Component_Log::debug ( '################### 100-4' );
        // 情報
        $sessionForm = $session->loadForm(self::FEATURE_ID);
Sgmov_Component_Log::debug ( '################### 100-5' );
        $inForm = new Sgmov_Form_Eve002In();

        $errorForm = NULL;

        if(strlen($param) == 10) {
            Sgmov_Component_Redirect::redirectPublicSsl("/tms/input2_dialog/{$param}");
        }
Sgmov_Component_Log::debug ( '################### 100-6' );
        if(!empty($param)) {
//            // チェックデジットチェック
            if(strlen($param) <= 10){
...
Sgmov_Component_Log::debug ( '################### 100-7' );
        ///////////////////////////////////////////////////////////////////////////////////////////////////
        // 申込データ存在チェック
        ///////////////////////////////////////////////////////////////////////////////////////////////////
Sgmov_Component_Log::debug ( '################### 100-8' );
        $comiketInfo = $this->_Comiket->fetchComiketById($db, $param);
        if (@empty($comiketInfo)
                || (@$comiketInfo['del_flg'] != '0' || @$comiketInfo['send_result'] != '3' || @$comiketInfo['batch_status'] != '4')) { 
...
Sgmov_Component_Log::debug ( '################### 100-9' );
        ///////////////////////////////////////////////////////////////////////////////////////////////////
        // 集荷日 チェック
        ///////////////////////////////////////////////////////////////////////////////////////////////////
Sgmov_Component_Log::debug ( '################### 100-10' );
        foreach ($comiketDetailList as $key => $comiketDetailInfo) {
            if($comiketDetailInfo['type'] == '1') { // 往路
                ...
                $collectDate2 = date('Y-m-d H:i:s', strtotime("{$collectDate} 18:00:00 -1 day"));
                $toDate = date('Y-m-d H:i:s');

                if ($collectDate2 <= $toDate) {
                    $collectDate2 = date('Y年m月d日 H:i', strtotime($collectDate2));

                    $title = "催事・イベント配送受付サービスの搬入のキャンセルお申し込みができませんでした";
                    $message = "既に {$collectDate2} を過ぎているため搬入のキャンセルのお申し込みができませんでした。(集荷日前日 18:00までキャンセル可能)";
                    Sgmov_Component_Redirect::redirectPublicSsl("/tms/error?t={$title}&m={$message}");
                }
            } else { // 復路
                $eventsubInfo = $this->_EventsubService->fetchEventsubByEventsubId($db, $comiketInfo['eventsub_id']);
                $toDate = date('Y-m-d H:i:s');
                $eventTermEndDate = "{$eventsubInfo['term_to']} 23:59:59";
                if ($eventTermEndDate <= $toDate) {
                    $eventTermEndDate2 = date('Y年m月d日', strtotime($eventTermEndDate));

                    $title = "催事・イベント配送受付サービスの搬出のキャンセルお申し込みができませんでした";
                    $message = "既に {$eventTermEndDate2} を過ぎているため搬出のキャンセルのお申し込みができませんでした。(イベント終了日時までキャンセル可能)";
                    Sgmov_Component_Redirect::redirectPublicSsl("/tms/error?t={$title}&m={$message}");
                }
            }
        }
Sgmov_Component_Log::debug ( '################### 100-11' );
        ///////////////////////////////////////////////////////////////////////////////////////////////////
        // ▼ 業務連携（キャンセルAPI実行）
        ///////////////////////////////////////////////////////////////////////////////////////////////////
...
Sgmov_Component_Log::debug ( '################### 100-12' );
        // HP側DB削除 /////////////////////////////////
        if ($renkeiFlg) {
            // 業務連携成功
        ...
        foreach ($comiketDetailList as $key => $comiketDetailInfo) {
            ...
            $comiketDetailList[$key]['comiket_box_list'] = $comiketBoxList;
        }
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////
Sgmov_Component_Log::debug ( '################### 100-15' );
...
//    Sgmov_Component_Log::debug($param);
