 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Sso_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
...
//        "0" => "0",
    ...
    public static $eventInfo = array(
        '500', // シーフードショー:event_id = 500
        '600', // アグリフードExpo:event_id = 600
    );
    
    public static $eventsubInfo = array(
        '500', // シーフードショー:eventsub_id = 500
        '600', // アグリフードExpo:eventsub_id = 600
    );
...
//            $inForm['eventsub_sel'] = $inForm['input_mode'];
...
//        }
        ...
        // 顧客区分：ふるさと祭り対応
        $outForm->raw_comiket_customer_kbn_sel = @$inForm["comiket_customer_kbn_sel"];
...
Sgmov_Component_Log::debug('############### AAAAAA-1');
//        $buildingListByEventsubId = $this->_BuildingService->fetchBuildingDataByEventsubId($db, $inForm["eventsub_sel"]);
        $buildingListByEventsubId = $this->_BuildingService->fetchBuildingBoothPostionByBuildingCd($db, 'SSO', $inForm["eventsub_sel"]);
//Sgmov_Component_Log::debug($buildingListByEventsubId);
        $boothPostionIds = @$buildingListByEventsubId['ids'];
        $boothPostionLbls = @$buildingListByEventsubId['names'];
        
Sgmov_Component_Log::debug($buildingListByEventsubId);
//        foreach($buildingListByEventsubId as $key => $val) {
//            $boothPostionIds[] =  $val['ids'];
//            $boothPostionLbls[] = $val['names'];
...
//        $outForm->raw_building_booth_id_sel = $inForm["building_booth_id_sel"];
        ...
        $outBoundColCalFromToData = $this->getOutBoundCollectCalFromTo(@$outBoundCollectCalDataMax['eventsub_id'], @$outBoundCollectCalDataMax['hatsu_jis2']);
        if ($outBoundColCalFromToData['frDate'] == '1900-01-01'
                && $outBoundColCalFromToData['toDate'] == '1900-01-01') {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $comiketCustomerKnbSel = @empty($inForm['comiket_customer_kbn_sel']) ? '1' : $inForm['comiket_customer_kbn_sel'];
        $comiketDetailOutboundBinshuKbnSel  = '1';
        $comiketDetailInboundBinshuKbnSel  = '1';
        if($inForm["comiket_detail_type_sel"] == "3") { // 搬入と搬出
            $comiketDetailOutboundBinshuKbnSel = @empty($inForm['comiket_detail_outbound_binshu_kbn_sel']) ? "0" : $inForm['comiket_detail_outbound_binshu_kbn_sel'];
            $comiketDetailInboundBinshuKbnSel = @empty($inForm['comiket_detail_inbound_binshu_kbn_sel']) ? "0" : $inForm['comiket_detail_inbound_binshu_kbn_sel'];
        ...
        if($inForm["comiket_detail_type_sel"] == "3") { // 搬入と搬出
            $dispItemInfo["outbound_box_lbls"] = $this->_BoxService->fetchBox2($db, $inForm["eventsub_sel"], $inForm["comiket_div"], "1"
                    , $comiketCustomerKnbSel, $comiketDetailInboundBinshuKbnSel); // 搬入
            $dispItemInfo["inbound_box_lbls"] = $this->_BoxService->fetchBox2($db, $inForm["eventsub_sel"], $inForm["comiket_div"], "2"
                    , $comiketCustomerKnbSel, $comiketDetailInboundBinshuKbnSel); // 搬出
        } else if($inForm["comiket_detail_type_sel"] == "1") { // 搬入
            $comiketCustomerKnbSel = @empty($inForm['comiket_customer_kbn_sel']) ? '1' : $inForm['comiket_customer_kbn_sel'];
            $dispItemInfo["outbound_box_lbls"] = $this->_BoxService->fetchBox2($db, $inForm["eventsub_sel"], $inForm["comiket_div"], "1"
                    , $comiketCustomerKnbSel, $comiketDetailOutboundBinshuKbnSel); // 搬入
        } else if($inForm["comiket_detail_type_sel"] == "2") { // 搬出
            $dispItemInfo["inbound_box_lbls"] = $this->_BoxService->fetchBox2($db, $inForm["eventsub_sel"], $inForm["comiket_div"], "2"
                    , $comiketCustomerKnbSel, $comiketDetailInboundBinshuKbnSel); // 搬出
        }
...
Sgmov_Component_Log::debug('####################### 123111111');        
Sgmov_Component_Log::debug($date->format('Y'));
Sgmov_Component_Log::debug($years);
        ...
        $years = array('','2020');
        $months = array('','02');
        $days   = array('','22','23','24','25','26','27');
        array_shift($years);
        array_shift($months);
        array_shift($days);
        
        // 便種区分：シーフードショー対応
        $outForm->raw_comiket_detail_outbound_binshu_kbn_sel = @$inForm["comiket_detail_outbound_binshu_kbn_sel"];
        
        // 便種区分：ふるさと祭り対応
        $outForm->raw_comiket_detail_inbound_binshu_kbn_sel = @$inForm["comiket_detail_inbound_binshu_kbn_sel"];
...
//        $fareTaxTotalInbound = 0;
    ...
    //                                    $boxInfo = $this->_BoxService->fetchBoxById($db, $key);
                                        ...
                                        if(!empty($boxInfo)) {
                                            ...
                                            $tableTreeData["comiket_detail_list"][$keyDet]["comiket_box_list"][$keyComiketBox]["cost_price_tax"] = $boxInfo['cost_tax'];
                                            $tableTreeData["comiket_detail_list"][$keyDet]["comiket_box_list"][$keyComiketBox]["cost_amount_tax"] = ceil((string)(intval($boxInfo['cost_tax']) * intval($valComiketBox['num'])));
                                        }
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costAmountTaxOutboundTotal / Sgmov_View_Sso_Common::CURRENT_TAX));
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax'] = floor((string)($fareAmountOutboundTotal * Sgmov_View_Sso_Common::CURRENT_TAX));
...
//                    }
                    ...
                    foreach($tableTreeData["comiket_detail_list"] as $keyDet => $valDet) {
                        ...
                        if($valDet['type'] == "2" || $valDet['type'] == "3") {
                            ...
                            if(!empty($valDet["comiket_box_list"])) {
                                foreach($valDet["comiket_box_list"] as $keyComiketBox => $valComiketBox) {
                                    if($valComiketBox['type'] == "2" || $valComiketBox['type'] == "3") { // 搬出
                                        ...
                                        if(!empty($boxInfo)) {
                                            // 手数料（税込）
                                            $costAmountTaxInboundTotal += intval($boxInfo['cost_tax']) * intval($valComiketBox['num']);
                                            $tableTreeData["comiket_detail_list"][$keyDet]["comiket_box_list"][$keyComiketBox]["cost_price_tax"] = $boxInfo['cost_tax'];
                                            $tableTreeData["comiket_detail_list"][$keyDet]["comiket_box_list"][$keyComiketBox]["cost_amount_tax"] = ceil((string)(intval($boxInfo['cost_tax']) * intval($valComiketBox['num'])));
                                        }
                            ...
                            $tableTreeData["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costAmountTaxInboundTotal / Sgmov_View_Sso_Common::CURRENT_TAX));
                            
                            // 税込
                            $tableTreeData["comiket_detail_list"][$keyDet]['cost_tax'] = $costAmountTaxInboundTotal;
                            $costAmountTaxInboundTotal2 += $costAmountTaxInboundTotal;
                            
                            $tableTreeData["comiket_detail_list"][$keyDet]['fare_tax'] = floor((string)($fareAmountInboundTotal * Sgmov_View_Sso_Common::CURRENT_TAX));
...
//            $customerCd = $this->getCustomerCd($comiketInfo['event_id']);
        ...
        $staffNameMark = '';
        if ($inForm['comiket_customer_kbn_sel'] == '2') {
            $staffNameMark = '★';
        }
        ...
        if ($inForm['comiket_detail_type_sel'] == '1') { // 搬入
            ...
            $prefId = @$inForm['comiket_detail_outbound_pref_cd_sel'];
            ...
            $prefId = @$inForm['comiket_detail_inbound_pref_cd_sel'];
        ...
        $data = array(
            ...
            "zip" => $zip, //'9999999',//$inForm['comiket_zip1'] . $inForm['comiket_zip2'],
            "pref_id" => $prefId, //'99', // $inForm['comiket_pref_cd_sel'],
            "address" => $address, //'使用しない', //$inForm['comiket_address'],
            "building" => $building, //$inForm['comiket_building'],
            ...
            "staff_sei" => $staffNameMark . @$inForm['comiket_personal_name_sei'],
            "staff_mei" => @$inForm['comiket_personal_name_mei'],
            "staff_sei_furi" => '使用しない', //@$inForm['comiket_staff_sei_furi'],
            "staff_mei_furi" => '使用しない', //@$inForm['comiket_staff_mei_furi'],
            "staff_tel" => $inForm['comiket_tel'], //'99999999999',// $inForm['comiket_staff_tel'],
...
//            "modified" => "",
            ...
            "del_flg" => '0',
            "customer_kbn" => @$inForm['comiket_customer_kbn_sel'],
        );
    ...
    public function _createComiketDetailInsertDataByInForm($inForm, $id) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1"
            ...
            $data = array(
                ...
                "collect_date" => @empty($comiket_detail_outbound_collect_date) ? null : $comiket_detail_outbound_collect_date,
                ...
                'binshu_kbn' => @$inForm['comiket_detail_outbound_binshu_kbn_sel'],
            );
...
//                $deliveryTime = "-1"; // TODO -1で良い?
            ...
            $data = array(
                ...
                "collect_date" => @empty($comiket_detail_inbound_collect_date) ? $toDate : $comiket_detail_inbound_collect_date,
                "collect_st_time" => $collectStTime,
                "collect_ed_time" => $collectEdTime,
                ...
                'binshu_kbn' => @$inForm['comiket_detail_inbound_binshu_kbn_sel'],
            );
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/sso/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
...
Sgmov_Component_Log::debug("############## 801-01");

        try {
Sgmov_Component_Log::debug("############## 801-02");
...
Sgmov_Component_Log::debug("############## 801-03");
            //宛先
//            $sendTo = $comiket['mail'];
            if(empty($sendTo2)) {
                $sendTo = $comiket['mail'];
...
Sgmov_Component_Log::debug($comiket['comiket_detail_list']);
...
//            $comiketPrefData = $this->_PrefectureService->fetchPrefecturesById($db, $comiket['pref_id']);
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/sso_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/sso_complete_business_sgmv{$tmplateType}.txt";

                $data['comiket_id'] = sprintf('%010d', $comiket['id']);
                $data['surname'] = $comiket['office_name'];
                $data['forename'] = "";//$comiket['personal_name_mei'];
                $customerKbnName = '';
//                if(@$comiket['customer_kbn'] == '1') {
//                    $customerKbnName = '出展者様';
//                } else if (@$comiket['customer_kbn'] == '2') {
//                    $customerKbnName = '一般のご利用者様（来場者様）';
//                }
                $data['event_name'] = $eventData["name"] . " " . $eventsubData["name"] . " " . $customerKbnName; //【出展イベント】
...
//                $data['comiket_building_name'] = $comiket['building_name'] . " " . $comiket['booth_position'] . " " . $comiket['booth_num'];
                ...
                if(@$comiket['customer_kbn'] = '1') { // 出展者の場合のみ
                    ...
                    if(@!empty($comiket['booth_position'])) {
                        ...
                        if (@$comiket['booth_num'] != '0000') { // 0000 の場合は、A-Z 以外のはず
                             $blockStr .=  "ブロック ";
                        }
                    }
                    $data['comiket_building_name'] = PHP_EOL . '【ブースNO】'. $blockStr. $comiket['booth_num'];
                }
//                if ($eventData['id'] === '4') {  // GoOutCamp
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/sso_complete_individual{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/sso_complete_individual_sgmv{$tmplateType}.txt";
...
//                $data['comiket_building_name'] = $comiket['building_name'] . " " . $comiket['booth_position'] . " " . $comiket['booth_num'];
                ...
                if(@$comiket['customer_kbn'] == '1') { // 出展者の場合のみ
                    ...
                    if($eventData['id'] === '2') { // コミケ
                        $data['comiket_building_name'] = PHP_EOL . '【ブース番号】' . $comiket['building_name'] . "ホール " . $blockStr . $comiket['booth_num'];
                    } else {
                        $data['comiket_building_name'] = PHP_EOL . '【ブースNO】' . $blockStr . $comiket['booth_num'];
                    }
                } else {
...
//                                $charterInfo = $this->_CharterService->fetchCharterById($db, $comiket_charter['charter_id']);
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    //搬出用メールテンプレート
                    $mailTemplate[] = '/sso_parts_complete_choice_2.txt';

                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/sso_parts_complete_choice_2_sgmv.txt';
                    
Sgmov_Component_Log::debug("############## 851-01");
Sgmov_Component_Log::debug($comiket['payment_method_cd']);

                    $data['type2_name'] = $comiket_detail['name'];  //【配送先名】
                    $comiketDetailBinshKbnName = '';
                    if ($comiket_detail['binshu_kbn'] == '0') {
                        $comiketDetailBinshKbnName = '飛脚宅配便';
                    } else if ($comiket_detail['binshu_kbn'] == '1') {
                        $comiketDetailBinshKbnName = '飛脚クール便（冷蔵）';
                    } else if ($comiket_detail['binshu_kbn'] == '2') {
                        $comiketDetailBinshKbnName = '飛脚クール便（冷凍）';
                    }
                    $data['type2_binshu_name'] = $comiketDetailBinshKbnName; // 便種
                    ...
                    if ($_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                        $data['acction_message'] = '当日・新規搬出追加、サイズ変更、キャンセルしたい場合は';
                    }
                    
                } else {
                    $prefData = $this->_PrefectureService->fetchPrefecturesById($db, $comiket_detail['pref_id']);
                    //搬入用メールテンプレート
                    $mailTemplate[] = '/sso_parts_complete_choice_1.txt';

                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/sso_parts_complete_choice_1_sgmv.txt';
                    ...
                    $data['type1_binshu_name'] = $comiketDetailBinshKbnName; // 便種
                    ...
                    // GiapLN implement SMT6-85
                    if ($_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                        $data['acction_message'] = '当日・新規搬出追加、サイズ変更、キャンセル、貼付票ダウンロードしたい場合は';
                    }
                    
                }
            }
Sgmov_Component_Log::debug("############## 861-01");
            ...
            // GiapLN implement SMT6-85
            if ($tmplateType === '_cancel' || $tmplateType === '_sgmv_cancel') {
                $mailTemplate[] = "/sso_parts{$footerTmpName}_footer_type_1.txt";
            } else {
                $sesUserType = $_SESSION[self::LOGIN_ID]['user_type'];
                if ($footerTmpName === '_complete') {    // send mail complete
                    if ($sesUserType === 0) {
                        if ($eventsubData['security_pattern'] == '1') {
                            $mailTemplate[] = "/common_footer_type_3.txt";
                        } else {
                            $mailTemplate[] = "/common_footer_type_2.txt";
                        }
                    } else if ($sesUserType === 1) {
                        $mailTemplate[] = "/common_footer_type_1.txt";
                    }
                } else {     // send mail size change
                    if ($sesUserType === 0) {
                        if ($eventsubData['security_pattern'] != '1') {
                            $mailTemplate[] = "/common_footer_type_2.txt";
                        }
...
Sgmov_Component_Log::debug("############## 861-02");
            //フッター用メールテンプレート
            // GiapLN implement SMT6-85
...
//
//                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
//                $mailTemplateSgmv[] = "/sso_parts{$footerTmpName}_footer_type_1.txt";
//            } else { // 個人
//                $mailTemplate[] = "/sso_parts{$footerTmpName}_footer_type_2.txt";
//
//                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
//                $mailTemplateSgmv[] = "/sso_parts{$footerTmpName}_footer_type_2.txt";
//            }
Sgmov_Component_Log::debug("############## 861-03");
            ...
            // GiapLN implement SMT6-85
            // $data['edit_url'] = $urlPublicSsl . "/sso/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            if ($_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm=sso'; 
            } else {
                $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=sso&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
            }
            
Sgmov_Component_Log::debug("############## 861-04");
            // 説明書URL
//            $data['manual_url'] = $urlPublicSsl . "/sso/pdf/manual/manual_{$comiket["eventsub_id"]}.pdf";
            $data['manual_url'] = '';
...
//            if($eventData['id'] != '2' && $eventData['id'] !== '4') { // コミケとGoOutCampは記載しない
            if($eventsubData['manual_display'] == '1') { // コミケとGoOutCampは記載しない
                $data['manual_url'] = "【説明書URL】" . PHP_EOL . $urlPublicSsl . "/sso/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            }
//            $data['manual_url'] = $urlPublicSsl . "/sso/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
...
//                    && $eventData['id'] !== '4')  {
                ...
                $pasteTagId = sprintf("%010d", $comiket['id']) . self::getChkD2($comiket['id']);
                 ...
                 // GiapLN implement SMT6-85
                if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 0){
                    //$data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/sso/paste_tag/{$pasteTagId}/";
                    $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/event/robotCheck?event_nm=sso&destination=paste_tag/{$pasteTagId}";
                }
                
            }
Sgmov_Component_Log::debug("############## 861-05");
...
Sgmov_Component_Log::debug("############## 861-06");
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // サイズ変更/キャンセルURL
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // GiapLN implement SMT6-85
            // $data['cancel_url'] = $urlPublicSsl . "/sso/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=sso&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            //$data['size_change_url'] = $urlPublicSsl . "/sso/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=sso&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            
Sgmov_Component_Log::debug("############## 861-07");
            //-------------------------------------------------
            //メール送信実行
...
Sgmov_Component_Log::debug("############## 861-08");
                // qrコードファイル出力
                $qr = new Image_QRCode();
Sgmov_Component_Log::debug("############## 861-09");
                ...
                imagedestroy($image);
Sgmov_Component_Log::debug("############## 861-10");
...
Sgmov_Component_Log::debug("############## 861-12");
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        $sendFileName = 'EVE_' . date ( 'YmdHis' ) . '.csv';
        $boundary = "-----" . md5 ( uniqid () );
...
Sgmov_Component_Log::debug('################# 110-3');
            ...
            $mailTemplateList = array(
                "/sso_cancel_{$divType}_error.txt",
//                "/dsn_parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
            );
Sgmov_Component_Log::debug('################# 110-4');
            $mailData['explanation_url_convenience_payment_method'] = " ";
Sgmov_Component_Log::debug('################# 110-4-1');
            // メールを送信する。
            $objMail = new Sgmov_Service_CenterMail();
Sgmov_Component_Log::debug('################# 110-4-2');
            $objMail->_sendThankYouMail($mailTemplateList, $mailTo, $mailData);
Sgmov_Component_Log::debug('################# 110-5');
            // 業務連携失敗
            return false;
            
            // メールを送信する。
//            Sgmov_Component_Mail::sendTemplateMail ( $mailData, $mailTemplateList, $mailTo );
            // エラー画面表示
//            $title = "エラーが発生しました";
//            if (@!empty($dispErrTitle)) {
//                $title = $dispErrTitle;
//            }
//            $message2 = "キャンセル送信に失敗しました。";
//            if (@!empty($dispErrMsg)) {
//                $message2 = $dispErrMsg;
//            } 
//
//            Sgmov_Component_Redirect::redirectPublicSsl("/dsn/error?t={$title}&m={$message2}");
//            exit;
        }
        ///////////////////////////////////////////////////////////////////////////////////////////////////
...
//     * @return string
//     */
//    protected function getLastSyukaTime($binshKbn) {
//        
//        return "16:30:00";
//    }
...
Sgmov_Component_Log::debug('############# 121212');
Sgmov_Component_Log::debug($eventsubInfo);
