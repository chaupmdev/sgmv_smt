...
Sgmov_Lib::useForms(array('Error', 'QrcSession', 'Qrc001In', 'Qrc002In'));
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Qrc_CheckInput extends Sgmov_View_Qrc_Common
{
    ...
    public function executeInner()
    {
        ...
        // チケットの確認と破棄
        //$session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_QRC001, $this->_getTicket());

        // 入力チェック
        $sessionForm = $session->loadForm(self::FEATURE_ID);

        if (empty($sessionForm)) {
            $sessionForm = new Sgmov_Form_QrcSession();
            $sessionForm->in = null;
        }
    ...
    public function _createInFormFromPost($post, $creditCardForm)
    {
        ...
        $inForm->event_sel = $post['event_sel'];
        $inForm->eventsub_sel = $post['eventsub_sel'];
        $inForm->eventsub_zip = $post['eventsub_zip'];
        $inForm->eventsub_address = $post['eventsub_address'];
        $inForm->eventsub_term_fr = $post['eventsub_term_fr'];
        $inForm->eventsub_term_to = $post['eventsub_term_to'];
        ...
        $inForm->comiket_id = $post['comiket_id'];
        $inForm->comiket_div = $post['comiket_div'];
        $inForm->comiket_customer_cd = $post['comiket_customer_cd'];
        $inForm->customer_search_btn = $post['customer_search_btn'];
        $inForm->office_name = mb_convert_kana($post['office_name'], 'RNASKV', 'UTF-8');
        $inForm->comiket_personal_name_sei = mb_convert_kana($post['comiket_personal_name_sei'], 'RNASKV', 'UTF-8');
        $inForm->comiket_personal_name_mei = mb_convert_kana($post['comiket_personal_name_mei'], 'RNASKV', 'UTF-8');
        $inForm->comiket_zip1 = mb_convert_kana($post['comiket_zip1'], 'rnask', 'UTF-8');
        $inForm->comiket_zip2 = mb_convert_kana($post['comiket_zip2'], 'rnask', 'UTF-8');
        $inForm->comiket_pref_cd_sel = $post['comiket_pref_cd_sel'];
        $inForm->comiket_address = mb_convert_kana($post['comiket_address'], 'RNASKV', 'UTF-8');
        $inForm->comiket_building = mb_convert_kana($post['comiket_building'], 'RNASKV', 'UTF-8');
        $inForm->comiket_tel = @str_replace(array('-', 'ー', '−', '―', '‐', 'ｰ'), "-", mb_convert_kana($post['comiket_tel'], 'rnask', 'UTF-8'));
        $inForm->comiket_mail = mb_convert_kana($post['comiket_mail'], 'rnask', 'UTF-8');
        $inForm->comiket_mail_retype = mb_convert_kana($post['comiket_mail_retype'], 'rnask', 'UTF-8');

        /** コミケ申込明細データ　**/
        // $inForm->comiket_detail_type_sel = filter_input(INPUT_POST, 'comiket_detail_type_sel');

        $inForm->comiket_detail_type_sel = $post['comiket_detail_type_sel'];
        ...
        // 搬出
        $inForm->comiket_detail_inbound_name = mb_convert_kana($post['comiket_detail_inbound_name'], 'RNASKV', 'UTF-8');
        $inForm->comiket_detail_inbound_zip1 = mb_convert_kana($post['comiket_detail_inbound_zip1'], 'rnask', 'UTF-8');
        $inForm->comiket_detail_inbound_zip2 = mb_convert_kana($post['comiket_detail_inbound_zip2'], 'rnask', 'UTF-8');
        $inForm->comiket_detail_inbound_pref_cd_sel = $post['comiket_detail_inbound_pref_cd_sel'];
        $inForm->comiket_detail_inbound_address = mb_convert_kana($post['comiket_detail_inbound_address'], 'RNASKV', 'UTF-8');
        $inForm->comiket_detail_inbound_building = mb_convert_kana($post['comiket_detail_inbound_building'], 'RNASKV', 'UTF-8');
        $inForm->comiket_detail_inbound_tel = @str_replace(array('-', 'ー', '−', '―', '‐', 'ｰ'), "-", mb_convert_kana($post['comiket_detail_inbound_tel'], 'rnask', 'UTF-8'));
        $inForm->comiket_detail_inbound_collect_date_year_sel = $post['comiket_detail_inbound_collect_date_year_sel'];
        $inForm->comiket_detail_inbound_collect_date_month_sel = $post['comiket_detail_inbound_collect_date_month_sel'];
        $inForm->comiket_detail_inbound_collect_date_day_sel = $post['comiket_detail_inbound_collect_date_day_sel'];
        $inForm->comiket_detail_inbound_collect_time_sel = $post['comiket_detail_inbound_collect_time_sel'];

        $inForm->comiket_detail_inbound_delivery_date_year_sel = $post['comiket_detail_inbound_delivery_date_year_sel'];
        $inForm->comiket_detail_inbound_delivery_date_month_sel = $post['comiket_detail_inbound_delivery_date_month_sel'];
        $inForm->comiket_detail_inbound_delivery_date_day_sel = $post['comiket_detail_inbound_delivery_date_day_sel'];
        $inForm->comiket_detail_inbound_delivery_time_sel = $post['comiket_detail_inbound_delivery_time_sel'];
        $inForm->comiket_detail_inbound_service_sel = $post['comiket_detail_inbound_service_sel'];
        $inForm->comiket_box_inbound_num_ary = $post['comiket_box_inbound_num_ary'];


        $inForm->comiket_cargo_inbound_num_sel = $post['comiket_cargo_inbound_num_sel'];
        $inForm->comiket_charter_inbound_num_ary = $post['comiket_charter_inbound_num_ary'];
        $inForm->comiket_detail_inbound_note1 = mb_convert_kana($post['comiket_detail_inbound_note1'], 'RNASKV', 'UTF-8');
        $inForm->comiket_detail_inbound_note2 = mb_convert_kana($post['comiket_detail_inbound_note2'], 'RNASKV', 'UTF-8');
        $inForm->comiket_detail_inbound_note3 = mb_convert_kana($post['comiket_detail_inbound_note3'], 'RNASKV', 'UTF-8');
        $inForm->comiket_detail_inbound_note4 = mb_convert_kana($post['comiket_detail_inbound_note4'], 'RNASKV', 'UTF-8');

        $inForm->input_mode = $post['input_mode'];

        // 復路-お届け可能日
        $inForm->hid_comiket_detail_inbound_delivery_date_from = $post['hid_comiket-detail-inbound-delivery-date-from'];
        $inForm->hid_comiket_detail_inbound_delivery_date_to = $post['hid_comiket-detail-inbound-delivery-date-to'];
        ...
        $calcDataInfo = $calcDataInfoData["treeData"];
        $inForm->delivery_charge = @$calcDataInfo['amount_tax'];
        $inForm->comiket_payment_method_cd_sel = $post['comiket_payment_method_cd_sel'];
        $inForm->comiket_convenience_store_cd_sel = $post['comiket_convenience_store_cd_sel'];
        $inForm->card_number = @$creditCardForm['card_number'];
        $inForm->card_expire_month_cd_sel = @$creditCardForm['card_expire_month_cd_sel'];
        $inForm->card_expire_year_cd_sel = @$creditCardForm['card_expire_year_cd_sel'];
        $inForm->security_cd = @$creditCardForm['security_cd'];

        if (isset($calcDataInfo['amount_tax'])) {
            $calcDataInfo['amount_tax'] = $post['URIAGE_KINGAKU'];
            $calcDataInfo['amount'] = ((int)$post['URIAGE_KINGAKU']) * 0.9;
        }
        $inForm->delivery_charge = $post['URIAGE_KINGAKU'];

        // $inForm->UKETSUKE_NO = $post['UKETSUKE_NO'];
        // $inForm->TOIAWASE_NO = $post['TOIAWASE_NO'];
        // $inForm->TOIBAN = $post['TOIBAN'];
        // $inForm->ARK_UKETSUKE_NO = $post['ARK_UKETSUKE_NO'];
        // $inForm->KESSAI_MEISAI_ID = $post['KESSAI_MEISAI_ID'];
        // $inForm->URIAGE_KINGAKU = $post['URIAGE_KINGAKU'];
        // $inForm->SYSTEM_KBN = $post['SYSTEM_KBN'];
        // $inForm->CD = $post['CD'];

        $inForm->qr_uketsuke_no = $post['UKETSUKE_NO'];
        $inForm->qr_toiawase_no = $post['TOIAWASE_NO'];
        $inForm->qr_toiban = $post['TOIBAN'];
        $inForm->qr_ark_uketsuke_no = $post['ARK_UKETSUKE_NO'];
        $inForm->qr_kessai_meisai_id = $post['KESSAI_MEISAI_ID'];
        $inForm->qr_uriage_kingaku = $post['URIAGE_KINGAKU'];
        $inForm->qr_system_kbn = $post['SYSTEM_KBN'];
        $inForm->qr_cd = $post['CD'];
    ...
    public function _validate($inForm, $db)
    {
        ...
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_address)->isNotEmpty()->isLengthLessThanOrEqualTo(14)->isNotHalfWidthKana()->isWebSystemNg();
        if (!$v->isValid()) {
        ...
        $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_building)->isNotEmpty()->isLengthLessThanOrEqualTo(30)->isNotHalfWidthKana()->isWebSystemNg();
        if (!$v->isValid()) {
        ...
        if (!$errorForm->hasError()) {
            ...
            $addressResult = $this->_getAddress(
                $inForm->comiket_zip1 . $inForm->comiket_zip2,
    ...
    public function _checkPaymentMethod($inForm, &$errorForm)
    {
        if ($inForm->comiket_div == self::COMIKET_DEV_INDIVIDUA) { // 個人
            ...
            if ($inForm->comiket_payment_method_cd_sel === '1') {
                // お支払い店舗 必須チェック
                $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_convenience_store_cd_sel)->isSelected()->isIn(array_keys($this->convenience_store_lbls));
                if (!$v->isValid()) {
                    $errorForm->addError('payment_method', 'お支払い方法' . $v->getResultMessageTop());
                }
            }
    ...
    public function _checkInbound($inForm, &$errorForm)
    {
        ...
        if ($inForm->comiket_detail_type_sel == "2" || $inForm->comiket_detail_type_sel == "3") { // 搬出の場合
            ...
            // 搬出-集荷先名
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_name)->isNotEmpty()->isLengthLessThanOrEqualTo(32)->isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_address)->isNotEmpty()->isLengthLessThanOrEqualTo(14)->isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_building)->isNotEmpty()->isLengthLessThanOrEqualTo(30)->isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
            ...
            if ($this->checkColAndDelDate("inbound", $inForm->comiket_div, $inForm->comiket_detail_inbound_service_sel, $eventsubInfo)) {
                if ($inForm->comiket_detail_inbound_service_sel == "1") {
                    ...
                    if ($this->checkColAndDelTime("inbound", $inForm->comiket_div, $inForm->comiket_detail_inbound_service_sel, $eventsubInfo)) {
                        if (
                            $inForm->comiket_div == self::COMIKET_DEV_INDIVIDUA &&
                            $inForm->comiket_detail_inbound_service_sel == '1'
                        ) { // 個人 && 選択サービスが宅配の場合
                            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_delivery_time_sel)->isSelected()->isIn(array_keys($this->comiket_detail_delivery_timezone));
                            $is_valid = $v->isValid();
                            if (!$is_valid) {
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_note1)->isLengthLessThanOrEqualTo(16)->isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
