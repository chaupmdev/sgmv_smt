 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Rms_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'RMS';
    
    // GiapLN implement SMT6-85
    const LOGIN_ID = 'RMS_LOGIN';
     ...
     * イベントサブID
     */
    const EVENT_SUB_ID = '710';

    /**
     * RMS001の画面ID
     */
    const GAMEN_ID_RMS001 = 'RMS001';

    /**
     * RMS002の画面ID
     */
    const GAMEN_ID_RMS002 = 'RMS002';

    /**
     * RMS003の画面ID
     */
    const GAMEN_ID_RMS003 = 'RMS003';
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $outForm->raw_comiket_detail_inbound_collect_date_year_sel = "2021";
        $outForm->raw_comiket_detail_inbound_collect_date_month_sel = "10";
        $outForm->raw_comiket_detail_inbound_collect_date_day_sel = "23";
        ...
        if ($inForm['comiket_detail_type_sel'] == "1" || $inForm['comiket_detail_type_sel'] == "2" || $inForm['comiket_detail_type_sel'] == "3") {
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Rms_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Rms_Common::CURRENT_TAX));
                        // // 税込
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/rms/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/rms/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/rms/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/rms/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/rms_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/rms_complete_business_sgmv{$tmplateType}.txt";
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/rms_complete_individual{$tmplateType}.txt";

                // 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/rms_complete_individual_sgmv{$tmplateType}.txt";
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    //搬出用メールテンプレート
                    $mailTemplate[] = '/rms_parts_complete_choice_2.txt';

                    // 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/rms_parts_complete_choice_2_sgmv.txt';
            ...
            if ($tmplateType === '_cancel' || $tmplateType === '_sgmv_cancel') {
                $mailTemplate[] = "/rms_parts{$footerTmpName}_footer_type_1.txt";
            } else {
...
//
//                //  申込者と営業所で使用するメールテンプレートをわけ別々に送信する
//                $mailTemplateSgmv[] = "/rms_parts{$footerTmpName}_footer_type_1.txt";
//            } else { // 個人
//                $mailTemplate[] = "/rms_parts{$footerTmpName}_footer_type_2.txt";
//
//                //  申込者と営業所で使用するメールテンプレートをわけ別々に送信する
//                $mailTemplateSgmv[] = "/rms_parts{$footerTmpName}_footer_type_2.txt";
//            }
            ...
            // GiapLN implement SMT6-85
            // $data['edit_url'] = $urlPublicSsl . "/rms/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            if ($_SESSION[self::LOGIN_ID]['user_type'] === 1) {
                $data['edit_url'] = $urlPublicSsl.'/event/login?event_nm=rms'; 
            } else {
                $data['edit_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=rms&destination=input/" . sprintf('%010d',$comiket["id"]) . $comiketIdCheckD;
            }
            
            

            // 説明書URL
//            $data['manual_url'] = $urlPublicSsl . "/rms/pdf/manual/manual_{$comiket["eventsub_id"]}.pdf";
            $data['manual_url'] = '';

            if($eventsubData['manual_display'] == '1') { // コミケとGoOutCampは記載しない
                $data['manual_url'] = "【宅配便WEB申込みのご案内】" . PHP_EOL . $urlPublicSsl . "/rms/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            }
//            $data['manual_url'] = $urlPublicSsl . "/rms/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            ...
            if(($comiket['choice'] == "1" || $comiket['choice'] == "3")
                ...
                if (isset($_SESSION[self::LOGIN_ID]['user_type']) && $_SESSION[self::LOGIN_ID]['user_type'] === 0){
                    // $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/rms/paste_tag/{$pasteTagId}/";
                    $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/event/robotCheck?event_nm=rms&destination=paste_tag/{$pasteTagId}";
                }
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // GiapLN implement SMT6-85
            // $data['cancel_url'] = $urlPublicSsl . "/rms/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['cancel_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=rms&destination=cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            // $data['size_change_url'] = $urlPublicSsl . "/rms/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['size_change_url'] = $urlPublicSsl . "/event/robotCheck?event_nm=rms&destination=size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
        ...
        foreach ($comiketDetailList as $key => $comiketDetailInfo) {
            if($comiketDetailInfo['type'] == '2') { // 往路
                ...
                if ($eventTermEndDate <= $toDate) {
                    ...
                    Sgmov_Component_Redirect::redirectPublicSsl("/rms/error?t={$title}&m={$message}");
                }
