 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Msb_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'MSB';

    /**
     * MSB001の画面ID
     */
    const GAMEN_ID_MSB001 = 'MSB001';

    /**
     * MSB002の画面ID
     */
    const GAMEN_ID_MSB002 = 'MSB002';

    /**
     * MSB003の画面ID
     */
    const GAMEN_ID_MSB003 = 'MSB003';
...
//        $outForm->raw_building_booth_id_sel = $inForm["building_booth_id_sel"];
        ...
        $dataList = $this->_BoxService->fetchBox2($db, $inForm["eventsub_sel"], $inForm["comiket_div"], "1");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // 取出回数
        $outForm->raw_comiket_detail_azukari_kaisu_type_sel  = $inForm["comiket_detail_azukari_kaisu_type_sel"];
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/dsn/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/dsn/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/dsn/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/dsn/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/dsn_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/dsn_complete_business_sgmv{$tmplateType}.txt";
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/dsn_complete_individual{$tmplateType}.txt";
                $mailTemplateSgmv[] = "/dsn_complete_individual_sgmv{$tmplateType}.txt";
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    //搬出用メールテンプレート
                    $mailTemplate[] = '/dsn_parts_complete_choice_2.txt';

                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/dsn_parts_complete_choice_2_sgmv.txt';

                    $data['manual_url'] = "【搬出の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/dsn/pdf/manual/{$eventData['name']}{$eventsubData['name']}搬出.pdf";
                    $comiketType = "type2";//搬出
                } else {
                    $prefData = $this->_PrefectureService->fetchPrefecturesById($db, $comiket_detail['pref_id']);
                    //搬入用メールテンプレート
                    $mailTemplate[] = '/dsn_parts_complete_choice_1.txt';

                    // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/dsn_parts_complete_choice_1_sgmv.txt';

                    $data['manual_url'] = "【搬入の説明書はこちら】". PHP_EOL . $urlPublicSsl . "/dsn/pdf/manual/{$eventData['name']}{$eventsubData['name']}搬入.pdf";
            ...
            if(($comiket['choice'] == "1" || $comiket['choice'] == "3")
                ...
                $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/dsn/paste_tag/{$pasteTagId}/";
            }
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        try {
            ...
            $mailTemplateList = array(
                "/dsn_cancel_{$divType}_error.txt",
