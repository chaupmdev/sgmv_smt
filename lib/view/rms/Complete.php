...
Sgmov_Lib::useView('rms/Common', 'CommonConst');
Sgmov_Lib::useView('rms/Bcm');
 ...
 */
class Sgmov_View_Rms_Complete extends Sgmov_View_Rms_Common {
    ...
    public function __construct() {
        ...
        $this->_BcmView             = new Sgmov_View_Rms_Bcm();
    ...
    public function executeInner($argBackInputPaht="") {
        ...
        //チケットの確認と破棄 TODO
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_RMS003, $this->_getTicket());
    ...
    protected function registerComiket($db, $inForm, $comiketId, $backInputPath){
        ...
        try {
            ...
            $db->rollback();
            $_SESSION["Sgmov_View_Rms.inputErrorInfo"] = array("db_insert_error" => "・データベースの登録・更新に失敗しました。");
            Sgmov_Component_Log::debug("リダイレクト /rms/{$backInputPath}/");
            Sgmov_Component_Log::err('データベースの登録・更新に失敗しました。');
            Sgmov_Component_Log::err($e);
            Sgmov_Component_Redirect::redirectPublicSsl("/rms/{$backInputPath}/");
            exit;
        }
        $db->commit();
...
*/
    ...
    public function errorInformation($parm = array())
    {
        ...
        $mail_to = Sgmov_Component_Config::getLogMailTo();
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($parm, dirname(__FILE__) . '/../../mail_template/Rms_error.txt', $mail_to);
    }
