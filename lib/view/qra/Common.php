 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Qra_Common extends Sgmov_View_Public
{

    /**
     * クレジットカード決済 エラーテストフラグ
     * フォーム上で設定した有効期限を無視して上書き設定します。
     * VERITRANS_ERRKEYにVERITRANS_ERRNUMのキー値を設定
     * すると該当のエラーが発生します。
     *'AG39' => '取引判定保留（有人判定）です。 [G30]'
     *'AG41' => 'セキュリティコード誤りです。 [G44]'
     *'AG44' => '1口座利用回数または金額オーバーです。 [G54]'
     *'AG45' => '1日限度額オーバーです。 [G55]'
     *'AG46' => 'クレジットカードが無効です。[G56]'
     *'AG47' => '事故カードです。 [G60]'
     *'AG48' => '無効カードです。[G61]'
     *'AG49' => '会員番号エラーです。 [G65]'
     *'AG51' => '金額エラーです。 [G68]'
     ...
     *'AG64' => '有効期限エラーです。 [G83]'
     *'AG70' => '当該要求拒否です。[G97]'
     *'AG71' => '当該自社対象業務エラーです。 [G98]'
     *'AG72' => '接続要求自社受付受付拒否です。[G99]'
     */
    const VERITRANS_ERRKEY = ''; //通常はこちら
    // const VERITRANS_ERRKEY = 'AG70'; //エラーを発生したい場合はエラーキーを設定
    //エラーキー
    public $VERITRANS_ERRNUM = array(
        'AG33' => '01/99',
        'AG41' => '02/99',
        'AG64' => '03/99',
        'AG39' => '04/99',
     ...
     * RMS001の画面ID
     */
    const GAMEN_ID_QRA001 = 'QRA001';

    /**
     * RMS002の画面ID
     */
    const GAMEN_ID_QRA002 = 'QRA002';

    /**
     * RMS003の画面ID
     */
    const GAMEN_ID_QRA003 = 'QRA003';
    ...
    /**
     * SG-ARKで決済時のWebAPIURL
     * @var array
     */
    public $systemKbnName = array(
         '0' => 'MV-BASE'
        ,'1' => 'SG-ARK'
    );
    ...
    /**
     * MV-BASEで決済時のWebAPIURL
     * @var array
     */
    public $mv_base_api_url = array(
         'DEVELOPMENT' => 'https://sgmv.media-tec.jp/sgmv-api/kessai_kanryo'
//        ,'STAGING'     => 'https://sgmvweb-test.sagawa-mov.co.jp/sgmv-api/kessai_kanryo'
//        ,'PRODUCT'     => 'https://sgmvweb.sagawa-mov.co.jp/sgmv-api/kessai_kanryo'
        ,'STAGING'     => 'https://sgmv.media-tec.jp/sgmv-api/kessai_kanryo'
        ,'PRODUCT'     => 'https://sgmvweb.sagawa-mov.co.jp/sgmv-api/kessai_kanryo'
    );
     ...
     * @var array
     */
    public $sg_ark_api_url = array(
         'DEVELOPMENT' => 'http://172.16.101.224/sg-ark-webapi/sa0303/index'
        ,'STAGING'     => 'http://172.16.101.224/sg-ark-webapi/sa0303/index'
        ,'PRODUCT'     => 'https://sgmvweb.sagawa-mov.co.jp/sg-ark-webapi/sa0303/index'
    );
     ...
     * @return str
     */
    protected function _setApiUrl($env, $kbn){

        // WebApiの接続先URLを取得
        $urlList=array();
        // MV-BASEの場合
        if($kbn == self::KBN_MV_BASE){
            $urlList=$this->mv_base_api_url;
        }
        // SG-ARKの場合
        else if($kbn == self::KBN_SG_ARK){
            $urlList=$this->sg_ark_api_url;
        }
        ...
        switch($env){
            // テスト環境
            case 'dev':
                $apiUrl = $urlList['DEVELOPMENT'];
                break;
            // 検証環境
            case 'st':
                $apiUrl = $urlList['STAGING'];
                break;
            // 本番環境
            case 'prod':
                $apiUrl = $urlList['PRODUCT'];
                break;
            default:
                return '';
                break;
     ...
     *
     * @param type $inForm
     * @param Sgmov_Form_Qra001Out $outForm
     * @return type
     */
    ...
    /**
     * 処理に失敗した場合、エラーメールを送信する
     * @param array  $inForm
     * @param str    $comiketId
     * @param object $errorObj
     */
    public function failedApiMail($inForm, $comiketId='', $errorObj){

            // メールテンプレート
            $mailTemplateList = array('/qra_api_error.txt');
            // 送信先はシステム管理者のメールアドレス
            $mailTo = Sgmov_Component_Config::getLogMailTo();
            ...
            // 発生日時
            $dObj = new DateTime();
            $mailData['errorDate'] = $dObj->format('Y-m-d H:i:s');
            // comiket_id
            $mailData['comiketId'] = $comiketId;
            // システム区分 0：MV-BASE / 1：SG-ARK
            $systemKbnName=$this->systemKbnName;
            $mailData['systemKbn'] = $systemKbnName[$inForm['qr_system_kbn']];
            // 決済情報
            $mailData['param'] = "";
            $mailData['param'] .= "受付番号："            .$inForm['qr_uketsuke_no'];
            $mailData['param'] .= "\n問合番号："          .$inForm['qr_toiawase_no'];
            $mailData['param'] .= "\n決済明細ID："        .$inForm['qr_kessai_meisai_id'];
            $mailData['param'] .= "\nARK受付番号："       .$inForm['qr_ark_uketsuke_no'];
            $mailData['param'] .= "\nベリトランス決済ID：".$inForm['payment_order_id'];

            // エラーメッセージ
            $mailData['errMsg']    = $errorObj->error;
