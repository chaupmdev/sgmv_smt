...
Sgmov_Lib::useView($dirDiv . '/CreditCard');
/**#@-*/

// 処理を実行
$view = new Sgmov_View_Qra_CreditCard();
$forms = $view->execute();
...
// 開催期間開始日
$termFr = explode('-', $qraOutForm->raw_term_fr);
// クレジットカード有効期限年のカウント
$inputCreditcardCnt = $qraOutForm->input_creditcard_cnt;
...
<body>
    ...
    <div id="main">
        <div class="wrap clearfix">
            ...
            <form action="/<?= $dirDiv ?>/check_credit_card" method="post">
                <input name="ticket" type="hidden" value="<?php echo $ticket ?>" />
                <div class="section">
                    <p class="sentence br">
                        金額に誤りがないかご確認の上、クレジットカード情報をご入力ください。
                        <br />ブラウザおよびスマートフォンの戻るボタンは押さないようお願いします。
                    </p>
                    <dl class="plain_dl">
                    </dl>
                    ...
                    <?php if (isset($e) && $e->hasError()) { ?>
                        <div class="err_msg">
                            <?php if (!$e->hasErrorForId('payment_method')) { ?>
                            ...
                            <ul>
                                ...
                                if ($e->hasErrorForId('payment_method')) {
                                    echo '<li><b>' . $e->getMessage('payment_method') . '</b></li>'
                                        . '<p>ERROR-CD' . $e->getMessage('payment_method_cd') . '</p>';
                                }
                                ?>
                            </ul>
                        </div>
                    ...
                    <div class="result02" style="font-size:134%;text-align: center;">
                        <span style="white-space: nowrap;"> 問合せ番号：</span>
                        <span style="white-space: nowrap;"><?= $qraOutForm->toiban(); ?></span>
                        <input name="toiawase_number" type="hidden" value="<?= $qraOutForm->toiawase_no(); ?>" />
                    </div>
                    <div class="result02" style="font-size:134%;text-align: center;">
                        <span style="white-space: nowrap;">合計金額：</span>
                        &nbsp;<span style="white-space: nowrap;"><?php echo number_format((int)$qraOutForm->uriage_kingaku()); ?>円（税込）</span>
                        <input name="uriage_kingaku" type="hidden" value="<?= $qraOutForm->uriage_kingaku(); ?>" />
                    </div>
                    ...
                    <div class="dl_block">
                        ...
                        <dl class="form_list clearfix" id="card_list">
                            <dt>有効期限</dt>
                            <dd<?php if (isset($e) && ($e->hasErrorForId('top_card_expire_year_cd_sel') || $e->hasErrorForId('top_card_expire_month_cd_sel') || $e->hasErrorForId('top_card_expire'))) {
                                ...
                                <label for="card_expire_month">
                                    <select id="card_expire_month" name="card_expire_month_cd_sel">
                                        ...
                                        <?php
                                        echo Sgmov_View_Qra_CreditCard::_createPulldown($qraOutForm->card_expire_month_cds(), $qraOutForm->card_expire_month_lbls(), $qraOutForm->card_expire_month_cd_sel());
                                        ?>
                                    </select>
                                    月
                                </label>
                                <label for="card_expire_year">
                                    <select id="card_expire_year" name="card_expire_year_cd_sel">
                                        ...
                                        <?php
                                        echo Sgmov_View_Qra_CreditCard::_createPulldown($qraOutForm->card_expire_year_cds(), $qraOutForm->card_expire_year_lbls(), $qraOutForm->card_expire_year_cd_sel());
                                        ?>
                                    </select>
                                    年
                                </label>
                        ...
                        <dl>
                            ...
                            <dd<?php if (isset($e) && $e->hasErrorForId('top_card_number')) {
                                    echo ' class="form_error"';
                                } ?>>
                                <input autocapitalize="off" class="w_280" id="card_number" inputmode="numeric" maxlength="16" name="card_number" data-pattern="^\d+$" placeholder="例）9999999999999999" type="<?php echo $inputTypeNumber; ?>" value="<?php echo $qraOutForm->card_number(); ?>" />
                                </dd>
                        </dl>
                        <dl>
                            <dt class="condition">
                            ...
                            <dd<?php if (isset($e) && $e->hasErrorForId('top_security_cd')) {
                                    echo ' class="form_error"';
                                } ?>>
                                <input autocapitalize="off" class="w_70" id="security_cd" inputmode="numeric" maxlength="4" name="security_cd" data-pattern="^\d+$" placeholder="例）9999" type="<?php echo $inputTypeNumber; ?>" value="<?php echo $qraOutForm->security_cd(); ?>" />
    ...
    <script charset="UTF-8" type="text/javascript" src="/<?= $dirDiv ?>/js/credit_card.js"></script>
    <script>
        $(function() {
