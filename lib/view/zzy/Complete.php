...
Sgmov_Lib::useView('zzy/Common', 'CommonConst');
Sgmov_Lib::useView('zzy/Bcm');
 ...
 */
class Sgmov_View_Zzy_Complete extends Sgmov_View_Zzy_Common {
    ...
    public function __construct() {
        ...
        $this->_BcmView             = new Sgmov_View_Zzy_Bcm();
    ...
    public function executeInner($argBackInputPaht="") {
        ...
        //チケットの確認と破棄 TODO
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_ZZY003, $this->_getTicket());
...
//         }
    ...
    protected function registerComiket($db, $inForm, $comiketId, $argBackInputPath){
        ...
        try {
            ...
            $db->rollback();
            $_SESSION["Sgmov_View_Zzy.inputErrorInfo"] = array("db_insert_error" => "・データベースの登録・更新に失敗しました。");
            Sgmov_Component_Log::debug("リダイレクト /zzy/{$argBackInputPath}/");
            Sgmov_Component_Log::err('データベースの登録・更新に失敗しました。');
            Sgmov_Component_Log::err($e);
            Sgmov_Component_Redirect::redirectPublicSsl("/zzy/{$argBackInputPath}/");
            exit;
        }
        $db->commit();
    ...
    protected function sendDataToGyomu($db, $inForm, $comiketId, $calcDataInfoData, $backInputPath, $sessionForm, $session, $toiawaseNo){
        ...
        if ($inForm['merchant_result'] == '0' &&
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/zzy/{$backInputPath}/");
            exit;
        }
...
Sgmov_Component_Log::debug($bcmResult);
    ...
    public function errorInformationForSgFinancial($param) {
        ...
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($param, dirname(__FILE__) . '/../../mail_template/zzy_error_for_sgfinancial.txt', $mail_to);
    }
...
Sgmov_Component_Log::debug($pay_limit_convenience_store);
    ...
    public function errorInformation($parm = array())
    {
        ...
        $mail_to = Sgmov_Component_Config::getLogMailTo();
        //テンプレートメールを送信する。
        Sgmov_Component_Mail::sendTemplateMail($parm, dirname(__FILE__) . '/../../mail_template/zzy_error.txt', $mail_to);
    }
