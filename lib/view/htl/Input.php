...
Sgmov_Component_Log::debug("########################### 651-0");
        if(strlen($param) == 10) {
            Sgmov_Component_Redirect::redirectPublicSsl("/htl/input2_dialog/{$param}");
        }
...
//Sgmov_Component_Log::debug($input2DialogComiketId);
            if(!is_numeric($input2DialogComiketId)){
                Sgmov_Component_Redirect::redirectPublicSsl("/htl/temp_error");
                exit;
            }
...
//Sgmov_Component_Log::debug($input2DialogData1Tel);
            ...
            if($comiketData['tel'] == $input2DialogData1Tel
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/htl/input/{$param}");
            } else {
                Sgmov_Component_Redirect::redirectPublicSsl("/htl/temp_error/{$comiketData['event_id']}");
            }
        }
...
Sgmov_Component_Log::debug("########################### 649-4");
        if(@!empty($_SERVER["REQUEST_URI"]) && strpos($_SERVER["REQUEST_URI"], "/htl/input2") !== false && empty($param)) {
...
Sgmov_Component_Log::debug("########################### 649-5");
        if(@!empty($_SERVER["HTTP_REFERER"]) && strpos($_SERVER["HTTP_REFERER"], "/htl/") !== false ){
...
Sgmov_Component_Log::debug("########################### 649-6");
        ...
        if (isset($dispItemInfo['eventsub_selected_data'])) {
            if (isset($dispItemInfo['eventsub_selected_data']['arrival_to_time'])) {
                $now = new DateTime();
                $arrivalToTime = new DateTime($dispItemInfo['eventsub_selected_data']['arrival_to_time']);
                if ($now>$arrivalToTime) {
                    $title = urlencode("公開日時終了");
                    $event = get_object_vars($outForm);
                    $eventName = $event['raw_event_lbls'][$event['raw_event_cd_sel']];
                    $message = urlencode("{$eventName}の申込期間は {$dispItemInfo['eventsub_selected_data']['arrival_to_time']} をもって始まります。<br />お申込みはまだできませんので、ご了承ください。");
                    Sgmov_Component_Redirect::redirectPublicSsl("/htl/error?t={$title}&m={$message}");
                    exit;
                }
            } else {
        ...
        if (isset($dispItemInfo['eventsub_selected_data'])) {
            if (isset($dispItemInfo['eventsub_selected_data']['departure_fr_time'])) {
                $now = new DateTime();
                $departureFrTo = new DateTime($dispItemInfo['eventsub_selected_data']['departure_fr_time']);
                if ($now<$departureFrTo) {
                    $title = urlencode("公開開始日時の期間外");
                    $event = get_object_vars($outForm);
                    $eventName = $event['raw_event_lbls'][$event['raw_event_cd_sel']];
                    $message = urlencode("{$eventName}の申込期間は {$dispItemInfo['eventsub_selected_data']['departure_fr_time']} に開始します。<br />お申込みはまだできませんので、ご了承ください。");
                    Sgmov_Component_Redirect::redirectPublicSsl("/htl/error?t={$title}&m={$message}");
                    exit;
                }
            } else {
