...
<?php
                echo Sgmov_View_Evp_Input::_createPulldown(
                          $eve001Out->comiket_detail_inbound_collect_time_cds()
                        , $eve001Out->comiket_detail_inbound_collect_time_lbls()
                        , $eve001Out->comiket_detail_inbound_collect_time_sel()
                );
?>
                                                </select>
                                            </span>
...
<!--
                                    <p class="comiket-detail-inbound-delivery-date-fr-to">
                                        <span class="comiket-detail-inbound-delivery-date-from">
                                        <?php echo @$dispItemInfo["eventsub_selected_data"]["inbound_delivery_fr"]; ?>
                                        </span>
                                        から
                                        <span class="comiket-detail-inbound-delivery-date-to">
                                        <?php echo @$dispItemInfo["eventsub_selected_data"]["inbound_delivery_to"]; ?>
                                        </span>
                                        まで選択できます。
                                    </p>
-->
                                    <?php /* js getInBoundUnloadingCal()で文言表示を制御している↑ */
                                          /* コンボ生成について
                                             画面初期表示時は下記hiddenにセットされたfr-toの日付の範囲で
                                             ssl_html/js/from_to_pulldate.jsのfromToPlldateInit()メソッドで
                                             コンボボックスが生成される
                                             配送先の郵便番号を選択すると、input.jsのgetInBoundUnloadingCal()で
                                             リードタイムマスタを郵便番号で絞り込んで配送できる期間をコンボボックスに
                                             セットしなおします
                                         */

                                    ?>
                                    <input type="hidden" id="hid_comiket-detail-inbound-delivery-date-from"
                                      name="hid_comiket-detail-inbound-delivery-date-from"
                                      value="<?php echo @$dispItemInfo["eventsub_selected_data"]["inbound_delivery_fr_dt"]; ?>" />
                                    <input type="hidden" id="hid_comiket-detail-inbound-delivery-date-to"
                                      name="hid_comiket-detail-inbound-delivery-date-to"
                                      value="<?php echo @$dispItemInfo["eventsub_selected_data"]["inbound_delivery_to_dt"]; ?>" />
                                    <input type="hidden" id="hid_comiket-detail-inbound-delivery-date-from_ori"
                                      name="hid_comiket-detail-inbound-delivery-date-from_ori"
                                      value="<?php echo @$dispItemInfo["eventsub_selected_data"]["inbound_delivery_fr_dt"]; ?>" />
                                    <input type="hidden" id="hid_comiket-detail-inbound-delivery-date-to_ori"
                                      name="hid_comiket-detail-inbound-delivery-date-to_ori"
                                      value="<?php echo @$dispItemInfo["eventsub_selected_data"]["inbound_delivery_to_dt"]; ?>" />
...
<?php
            echo Sgmov_View_Evp_Input::_createPulldown(
                $eve001Out->comiket_detail_inbound_delivery_time_cds()
              , $eve001Out->comiket_detail_inbound_delivery_time_lbls()
              , $eve001Out->comiket_detail_inbound_delivery_time_sel()
            );
?>
                                            </select>
                                        </span>
...
<!-- ここから追加 -->
                            <dl>
                                ...
                                <dd<?php if (isset($e) && $e->hasErrorForId('comiket_detail_inbound_kijiran')) { echo ' class="form_error"'; } ?>>
記事欄1
                                    <select name="comiket_detail_inbound_kijiran1">
                                        <option value="">選択してください</option>
                                        <?php
                                        echo(Sgmov_View_Evp_Input::_createPulldown(
                                                  $eve001Out->comiket_detail_inbound_kijiran_cds()
                                                , $eve001Out->comiket_detail_inbound_kijiran_lbls()
                                                , $eve001Out->comiket_detail_inbound_kijiran_cd_sel1()
                                        ));
                                        ?>
                                    </select><p>
記事欄2
                                    <select name="comiket_detail_inbound_kijiran2">
                                        <option value="">選択してください</option>
                                        <?php
                                        echo(Sgmov_View_Evp_Input::_createPulldown(
                                                  $eve001Out->comiket_detail_inbound_kijiran_cds()
                                                , $eve001Out->comiket_detail_inbound_kijiran_lbls()
                                                , $eve001Out->comiket_detail_inbound_kijiran_cd_sel2()
                                        ));
                                        ?>
                                    </select><p>
記事欄3
                                    <select name="comiket_detail_inbound_kijiran3">
                                        <option value="">選択してください</option>
                                        <?php
                                        echo(Sgmov_View_Evp_Input::_createPulldown(
                                                  $eve001Out->comiket_detail_inbound_kijiran_cds()
                                                , $eve001Out->comiket_detail_inbound_kijiran_lbls()
                                                , $eve001Out->comiket_detail_inbound_kijiran_cd_sel3()
                                        ));
                                        ?>
                                    </select><p>

記事欄4
                                    <input autocapitalize="off" class="number-p-only" 
                                    style="width:50%;" maxlength="32" name="comiket_detail_inbound_kijiran4" 
                                    placeholder="フリー入力" 
                                    type="text" value="<?php echo $eve001Out->comiket_detail_inbound_kijiran4();?>" />
                                </dd>
                            ...
                            <dl class="service-inbound-item" service-id="1">
                                ...
                                <dd<?php if (isset($e) && $e->hasErrorForId('comiket_box_inbound_num_ary')) { echo ' class="form_error"'; } ?>>
                                    ...
                                    <div class="outbound_example_boxsize example_boxsize">
                                        <strong class="red">※ 最大4種類(タイプ)までご指定頂けます</strong><br/><br/>
