 ...
 */
class Sgmov_View_Azk_SizeChangeCheckInput extends Sgmov_View_Azk_CheckInput {
    ...
    public function _redirectProc($inForm, $errorForm) {
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/azk/size_change');
        }
        ...
        if (@empty($boxNumTotal)) {
            Sgmov_Component_Redirect::redirectPublicSsl('/azk/size_change_confirm');
        }
        ...
        switch ($inForm->comiket_payment_method_cd_sel) {
            case '1': // コンビニ
                ...
                break;
            case '2': // クレジット
                Sgmov_Component_Redirect::redirectPublicSsl('/azk/size_change_credit_card');
                break;
            default:
                Sgmov_Component_Redirect::redirectPublicSsl('/azk/size_change_confirm');
                break;
    ...
    public function _validate($inForm, $db) {
        ...
        $this->comiketBoxValidate($inForm, $errorForm, $inForm->comiket_box_num_ary, "comiket_box_num", false, true);
        ...
        foreach ($inForm->comiket_box_num_ary as $key => $val) {
            ...
            if ($inForm->comiket_box_num_ary[$key] == "0") {
                $inForm->comiket_box_num_ary[$key] = "";
            }
        }
        ...
        if (@!empty($boxNumTotal)) {
            ...
            if(!$errorForm->hasError()) {
                ...
                if ($totalNumDb < $totalNumDisp) {
                    $errorForm->addError("comiket_box_num", "宅配数量合計が元の数量合計より大きくなっています。（元の数量合計：{$totalNumDb}個）");
                }
            }
            ...
            if(!$errorForm->hasError()) {
                ...
                if (@!empty($comiketInfo) && $comiketInfo['amount_tax'] < $inForm->delivery_charge) { // 金額が前回の時より多かった場合はエラー
                    ...
                    $errorForm->addError("comiket_box_num", "合計金額が元の金額より大きくなっています。（元の金額：￥{$amountTaxOld}／今回の金額￥{$amountTaxNew}）");
                }
            }
        }
