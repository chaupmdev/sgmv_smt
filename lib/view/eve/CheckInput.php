...
class Sgmov_View_Eve_CheckInput extends Sgmov_View_Eve_Common {
    ...
    public function _redirectProc($inForm, $errorForm) {
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/eve/input');
//        } else if($inForm->comiket_div == "1" && $inForm->comiket_customer_cd_sel == "1") {  // 法人 && 顧客コードを使用する
            ...
            Sgmov_Component_Redirect::redirectPublicSsl('/eve/confirm');
        }
        ...
        switch ($inForm->comiket_payment_method_cd_sel) {
            case '1': // コンビニ
                ...
                break;
            case '2': // クレジット
                Sgmov_Component_Redirect::redirectPublicSsl('/eve/credit_card');
                break;
            case '3': // 電子マネー
                Sgmov_Component_Redirect::redirectPublicSsl('/eve/confirm');
                break;
    ...
    public function _createInFormFromPost($post, $creditCardForm) {
        ...
        // 2022-03-18 ToanDD3 implement SMT6-63
    ...
    public function _validate($inForm, $db) {
        ...
        if(!empty($eventsubInfo) && $eventsubInfo['building_display'] == '1') {
            ...
            if(!empty($inForm->building_booth_position_sel)) {
                $buildingBoothPostionInfoAry = $this->_BuildingService->fetchBuildingDataByEventsubId($db, $inForm->eventsub_sel);
                Sgmov_Component_Log::debug('##################### LLLLLLLLLLLLLLLL-1');
                Sgmov_Component_Log::debug($inForm);
                Sgmov_Component_Log::debug($buildingBoothPostionInfoAry);
                $checkIdList = array();
                foreach ($buildingBoothPostionInfoAry as $key => $val) {
                    $checkIdList[] = $val['id'];
                }

                $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->building_booth_position_sel)->isSelected()->isIn($checkIdList);
                if (!$v->isValid()) {
