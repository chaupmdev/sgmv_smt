 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Tms_CheckInput extends Sgmov_View_Tms_Common {
...
Sgmov_Component_Log::debug("############## 705-5");
    ...
    public function _redirectProc($inForm, $errorForm) {
        ...
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/tms/input');
//        } else if($inForm->comiket_div == "1" && $inForm->comiket_customer_cd_sel == "1") {  // 法人 && 顧客コードを使用する
            ...
            Sgmov_Component_Redirect::redirectPublicSsl('/tms/confirm');
        }
        
        $calcDataInfoData = $this->calcEveryKindData((array)$inForm);
        $calcDataInfo = $calcDataInfoData["treeData"];
        if (@empty($calcDataInfo['amount_tax_free'])) {
            // 無料のデータのみの場合
            Sgmov_Component_Redirect::redirectPublicSsl('/tms/confirm');
        }
        ...
        switch ($inForm->comiket_payment_method_cd_sel) {
            case '1': // コンビニ
                ...
                break;
            case '2': // クレジット
                Sgmov_Component_Redirect::redirectPublicSsl('/tms/credit_card');
                break;
            case '3': // 電子マネー
                Sgmov_Component_Redirect::redirectPublicSsl('/tms/confirm');
                break;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        $inForm->delivery_charge_free = @$calcDataInfo['amount_tax_free'];
...
//Sgmov_Component_Log::debug("########### customr_cd 1 =" . $inForm->comiket_div );
        // 顧客コード
...
//            }
        ...
        if(!empty($eventsubInfo) && $eventsubInfo['building_display'] == '1') {
            ...
            if ($inForm->event_sel != '2') {
                ...
                if (!$v->isValid()) {
                    $errorForm->addError('building_name_sel', '館名' . $v->getResultMessageTop());
                }
            }
            ...
            if (!empty($inForm->building_name_sel)) {
                ...
                if(!$checkFlg) {
                    $errorForm->addError('building_name_sel', '選択館名に該当するデータが存在しません。');
                }
            }
...
//Sgmov_Component_Log::debug($inForm);
        ...
        if($inForm->comiket_detail_type_sel == "1" || $inForm->comiket_detail_type_sel == "3") { // 搬入の場合
            ...
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_outbound_building)->/*isNotEmpty()->*/isLengthLessThanOrEqualTo(30)->
                    isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('comiket_detail_outbound_building', '搬入-集荷先番地・建物名' . $v->getResultMessageTop());
            } else {
                $prefName = @$prefectures["names"][$prefectures["ids"][$inForm->comiket_detail_outbound_pref_cd_sel]];
                if (strpos($inForm->comiket_detail_outbound_building, $prefName) !== false) {
                    $errorForm->addError('comiket_detail_outbound_building', '搬入-集荷先番地・建物名には都道府県名は入力しないで下さい。');
                }
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if($this->checkColAndDelDate("outbound", $inForm->comiket_div, $inForm->comiket_detail_outbound_service_sel, $eventsubInfo)) {
                ...
                if ($inForm->hid_comiket_detail_outbound_collect_date_from == '1900-01-01'
                    ...
                    $dateColl = date('Y-m-d 12:00:00', strtotime("{$yearColl}-{$monthColl}-{$dayColl}-1day"));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if($inForm->comiket_detail_type_sel == "2" || $inForm->comiket_detail_type_sel == "3") { // 搬出の場合
            ...
            // 搬出-集荷先番地・建物名 必須チェック 40文字チェック WEBシステムNG文字チェック
...
//                        || ($inForm->comiket_detail_inbound_collect_time_sel != "00" && empty($inForm->comiket_detail_inbound_collect_time_sel))
                        ) {
//Sgmov_Component_Log::debug("##################### 610");
                    $max = null;
                    $max_year  = null;
                    $max_month = null;
                    $max_day   = null;
...
//Sgmov_Component_Log::debug("##################### 612");
//Sgmov_Component_Log::debug($min);
//Sgmov_Component_Log::debug($max);

                // 搬出-お預かり日時-時間帯 必須チェック
//                $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_detail_inbound_collect_time_sel)->isSelected()->
//                        isIn(array_keys($this->comiket_detail_time_lbls_par30m));
//                $is_valid = $v->isValid();
//                if (!$is_valid) {
//                    $errorForm->addError('comiket_detail_inbound_collect_date', '搬出-お預かり日時-時間帯' . $v->getResultMessageTop());
//                }
//Sgmov_Component_Log::debug("##################### 613");
//Sgmov_Component_Log::debug($_POST);
            }

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Sgmov_Component_Log::debug("##################### 613-1");
Sgmov_Component_Log::debug($inForm);
//            if($inForm->comiket_div == self::COMIKET_DEV_INDIVIDUA) { // 個人
            $eventsubInfo = $this->_EventsubService->fetchEventsubByEventsubId($db, $inForm->eventsub_sel);
            if($this->checkColAndDelDate("inbound", $inForm->comiket_div, $inForm->comiket_detail_inbound_service_sel, $eventsubInfo)) {
Sgmov_Component_Log::debug("##################### 613-2");
