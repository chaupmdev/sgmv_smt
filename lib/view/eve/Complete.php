...
Sgmov_Lib::useView('eve/Common', 'CommonConst');
Sgmov_Lib::useView('eve/Bcm');
...
class Sgmov_View_Eve_Complete extends Sgmov_View_Eve_Common {
    ...
    protected function checkHannyuArrivalDate($db, $inForm, $eveSubData){
        ...
        if ($dateChArrival->format('Y-m-d H:i:s') <= $dateChNow->format('Y-m-d H:i:s')) {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/eve/error/?t=受付時間が終了しました&m=お預かり日：{$dispDate}の受付時間が終了しました");
            exit;
        }
        ...
        if ($dateColl <= $dateNow) {
            // セッション情報を破棄
            $session->deleteForm(self::FEATURE_ID);
            Sgmov_Component_Redirect::redirectPublicSsl("/eve/error/?t=受付時間が終了しました&m=お預かり日：{$yearColl}/{$monthColl}/{$dayColl}の受付時間が終了しました");
            exit;
        }
    }
    ...
    protected function registerComiket($db, $inForm, $comiketId, $backInputPath){
        ...
        try {
            ...
            Sgmov_Component_Redirect::redirectPublicSsl("/eve/{$backInputPath}/");
            exit;
        }
        $db->commit();
