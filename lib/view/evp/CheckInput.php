...
Sgmov_Component_Log::debug($_POST);
    ...
    public function _redirectProc($inForm, $errorForm) {
        if ($errorForm->hasError()) {
            Sgmov_Component_Redirect::redirectPublicSsl('/evp/input');
//        } else if($inForm->comiket_div == "1" && $inForm->comiket_customer_cd_sel == "1") {  // 法人 && 顧客コードを使用する
            ...
            Sgmov_Component_Redirect::redirectPublicSsl('/evp/confirm');
        }
        ...
        switch ($inForm->comiket_payment_method_cd_sel) {
            case '1': // コンビニ
                ...
                break;
            case '2': // クレジット
                Sgmov_Component_Redirect::redirectPublicSsl('/evp/credit_card');
                break;
            case '3': // 電子マネー
                Sgmov_Component_Redirect::redirectPublicSsl('/evp/confirm');
                break;
     ...
     * @param Sgmov_Form_Eve002In $creditCardForm 入力フォーム
     * @return Sgmov_Form_Eve001In 入力フォーム
     */
...
Sgmov_Component_Log::debug($_POST);
        ...
        //GiapLN implement task SMT6-225 2022.07.25
        $inForm->comiket_booth_num = filter_input(INPUT_POST, 'comiket_booth_num');
        $inForm->comiket_booth_addition_num = filter_input(INPUT_POST, 'comiket_booth_addition_num');//@$post['comiket_booth_addition_num'];
...
//        $inForm->comiket_detail_inbound_note = mb_convert_kana(filter_input(INPUT_POST, 'comiket_detail_inbound_note'), 'RNASKV', 'UTF-8');
        ...
        if(!empty($post['comiket_detail_caremark_flg'])) {
            $inForm->comiket_detail_caremark_flg = 1;
        } else {
            $inForm->comiket_detail_caremark_flg = 0;
        }
        //GiapLN add 梱包ガイドライン 2022.07.14
        if(!empty($post['comiket_detail_caremark_flg_inbound'])) {
            $inForm->comiket_detail_caremark_flg_inbound = 1;
        } else {
            $inForm->comiket_detail_caremark_flg_inbound = 0;
        }
        //$inForm->comiket_detail_caremark_flg_enable = $post['comiket_detail_caremark_flg_enable'];
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function _validate($inForm, $db) {
        ...
        $eventsubArray = $this->_EventsubService->fetchEventsubListWithinTermByEventId($db, $inForm->event_sel, NULL, NULL, self::FEATURE_ID);
        $eventsubInfo = NULL;
        ...
        if(!empty($eventsubInfo) && $eventsubInfo['booth_display'] ==  '1') {
            ...
            if (!$v->isValid()) {
                $errorForm->addError('comiket_booth_name', 'サークル名' . $v->getResultMessageTop());
            }
        }
...
//                 }
            ...
            // ブース番号-テキスト
           //GiapLN implement task SMT6-225 2022.07.25
            //$v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_booth_num)->isNotEmpty()->isHalfWidthAlphaNumericCharacters()->isLengthLessThanOrEqualTo(4)->isNotHalfWidthKana()->isWebSystemNg();
                
            $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->comiket_booth_num)->isNotEmpty()->isInteger()->isLengthLessThanOrEqualTo(2)->isNotHalfWidthKana()->isWebSystemNg();
            if (!$v->isValid()) {
                $errorForm->addError('building_name_sel', 'ブースNO' . $v->getResultMessageTop());
            }
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //                            throw new Exception("There must be an error in the data in ".__FILE__." at line ".__LINE__.": formatDateSt {$formatDateSt} > formatDateEnd {$formatDateEnd}?!)");
                    ...
                    if($this->checkColAndDelTime("outbound", $inForm->comiket_div, $inForm->comiket_detail_outbound_service_sel, $eventsubInfo)) {
						...
						// 引き渡し日13日を選択した場合はお預かり日最終日を-1する
						if($inForm->comiket_detail_outbound_delivery_date_day_sel==13){
							$max_day=$max_day-1;
						}
...
//            }
            ...
            //GiapLN add 梱包ガイドライン 2022.07.15
            if ($inForm->comiket_detail_caremark_flg == 0) {
                $errorForm->addError('comiket_detail_caremark_flg', '梱包ガイドラインを入力してください。');
            }
        }
...
//            }
            ...
            //GiapLN add 梱包ガイドライン 2022.07.15
            if ($inForm->comiket_detail_caremark_flg_inbound == 0) {
                $errorForm->addError('comiket_detail_caremark_flg_inbound', '梱包ガイドラインを入力してください。');
            }
        }
