...
Sgmov_Lib::useForms(array('Error', 'AspSession', 'Asp014Out'));
/**#@-*/

 /**
 * カレンダー用特価編集確認画面を表示します。
 * @package    View
 * @subpackage ASP
 * @author     M.Shiiba(TPE)
 * @copyright  2009-2010 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Asp_CalDetail extends Sgmov_View_Asp_Common
{
    /**
     * ログインサービス
    ...
    {
        return self::FEATURE_ID_CALDETAIL;
    }
    ...
    public function executeInner()
    {
        ...
        $forms = $this->_createOutForm($id, $selectedCoursePlanCd, $selectedFromAreaCd);
    ...
    public function _parseGetParameter()
    {
        ...
        if ($paramCount === 2) {
            // コースプラン・出発エリア指定がある場合
            $ids = explode(Sgmov_Service_CoursePlan::ID_DELIMITER, $params[1], 3);
            if (count($ids) != 3) {
                // 不正遷移
                Sgmov_Component_ErrorExit::errorExit(Sgmov_Component_ErrorCode::ERROR_VIEW_INVALID_PAGE_ACCESS, '$_GET[\'param\']が不正です。');
            }
     ...
     * ['errorForm'] エラーフォーム
     */
    public function _createOutForm($id, $selectedCoursePlanCd, $selectedFromAreaCd)
    {
        $outForm = new Sgmov_Form_Asp014Out();
        ...
        if (is_null($spInfo)) {
            ...
            $errorForm->addError('top', '指定された情報は存在しません。');
        ...
        $errorForm = $this->_checkInput($spInfo, $selectedCoursePlanCd, $selectedFromAreaCd);
     ...
     */
    public function _checkInput($spInfo, $selectedCoursePlanCd, $selectedFromAreaCd)
    {
        $errorForm = new Sgmov_Form_Error();
     ...
     * @param Sgmov_Form_Asp014Out $outForm 出力フォーム
     * @param array $spInfo 特価情報
    ...
    public function _setSpInfo($outForm, $spInfo, $masters)
    {
        ...
        if ($maxTime > time()) {
            $closed = FALSE;
            if ($spInfo['draft_flag'] === '1') {
                $outForm->raw_sp_status = '下書き';
            } else {
                $outForm->raw_sp_status = '公開中';
            }
        } else {
            $closed = TRUE;
            $outForm->raw_sp_status = '終了';
        }

        // 特価内容
        $outForm->raw_sp_kind = $spInfo['special_price_division'];
    ...
    public function _setSpDetailInfo($db, $outForm, $spInfo, $masters)
    {
        ...
        // 出発エリアリストの名称
        $outForm->raw_cur_from_area = $this->_getLabelString($outForm->raw_from_area_cd_sel, $masters['fromAreaIds'],
             $masters['fromAreaNames']);
