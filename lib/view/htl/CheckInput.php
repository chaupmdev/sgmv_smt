...
//Sgmov_Component_Log::debug("########################## 801-6");
            Sgmov_Component_Redirect::redirectPublicSsl('/htl/input');
...
//Sgmov_Component_Log::debug("########################## 801-7");
            Sgmov_Component_Redirect::redirectPublicSsl('/htl/confirm');
        }
...
//Sgmov_Component_Log::debug("########################## 801-9");
                ...
                break;
            case '2': // クレジット
                Sgmov_Component_Redirect::redirectPublicSsl('/htl/credit_card');
                break;
            case '3': // 電子マネー
                Sgmov_Component_Redirect::redirectPublicSsl('/htl/confirm');
                break;
...
//                    $errorForm->addError('comiket_detail_outbound_delivery_date', '搬入-お届け日' . $v->getResultMessageTop());
                    if ($formatDateSt < $formatDateEnd) {
                        $errorForm->addError('comiket_detail_outbound_delivery_date', "搬入-お届け日は、{$formatDateSt}～{$formatDateEnd}までの日付を入力してください。");
                    } elseif ($formatDateSt > $formatDateEnd) {
                        throw new Exception("There must be an error in the data in ".__FILE__." at line ".__LINE__.": formatDateSt {$formatDateSt} > formatDateEnd {$formatDateEnd}?!)");
                    }
                }
...
//            }
     ...
     */
    private function checkComiketBoxOutInboundNumAry($inForm, &$errorForm, $targetList, $targetClassName, $errMsgOutInbount) {
        $result = array(
            "errflg" => FALSE,
            "errData" => array(),
        );
