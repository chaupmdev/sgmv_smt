 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Gmm_Cancel extends Sgmov_View_Gmm_Input {
...
Sgmov_Component_Log::debug ( 'CD不一致' );
        ...
        if($comiketDetailInfo['type'] == '1') { // 往路
            ...
            $collectDate2 = date('Y-m-d H:i:s', strtotime("{$collectDate} 12:00:00 -1 day"));
            $toDate = date('Y-m-d H:i:s');
            
            if ($collectDate2 <= $toDate) {
                $collectDate2 = date('Y年m月d日 H:i', strtotime($collectDate2));

                $title = urlencode("催事・イベント配送受付サービスの搬入のキャンセルお申し込みができませんでした");
                $message = urlencode("既に {$collectDate2} を過ぎているため搬入のキャンセルのお申し込みができませんでした。(集荷日前日 12:00までキャンセル可能)");
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error?t={$title}&m={$message}");
            }
        } else { // 復路
            ...
            if ($eventTermEndDate <= $toDate) {
                ...
                $title = urlencode("催事・イベント配送受付サービスの搬出のキャンセルお申し込みができませんでした");
                $message = urlencode("既に {$eventTermEndDate2} を過ぎているため搬出のキャンセルのお申し込みができませんでした。(イベント終了日までキャンセル可能)");
                Sgmov_Component_Redirect::redirectPublicSsl("/gmm/error?t={$title}&m={$message}");
            }
        }
        ///////////////////////////////////////////////////////////////////////////////////////////////////
