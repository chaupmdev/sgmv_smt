 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Unf_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'UNF';
    /**
     * マスタから取得する時、使う
     */
    const FEATURE_ID_FOR_DB = 'UNA';
     ...
     * UNA001の画面ID
     */
    const GAMEN_ID_UNF001 = 'UNA001';

    /**
     * UNA002の画面ID
     */
    const GAMEN_ID_UNF002 = 'UNA002';

    /**
     * UNA003の画面ID
     */
    const GAMEN_ID_UNF003 = 'UNA003';
     ...
     * 集荷の開始期間
     */
    const ADD_DELIVERY_DAYS_START = 1;
    /**
     * 集荷の終了期間
     */
    const ADD_DELIVERY_DAYS_END = 9;
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    protected function createOutFormByInForm($inForm, $outForm = array()) {
        ...
        $eventAll = $this->_EventService->fetchEventListWithinTerm2($db, null, null, self::FEATURE_ID_FOR_DB);
        ...
        if (!isset($inForm['comiket_detail_type_sel2'])) {
            $inForm['comiket_detail_type_sel2'] = '2';//復路
        }
...
//            $dateAdd  = $now->modify('+'.self::ADD_DELIVERY_DAYS_START.' days');
//            $endDate = new DateTime($deliveryDateOutForm);
//            $endDate = $endDate->modify('+'.self::ADD_DELIVERY_DAYS_END.' days');
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                    ...
                    if(isset($valDet["comiket_box_list"])){
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Unf_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Unf_Common::CURRENT_TAX));
                        // // 税込
...
//            $returnList[] = $data;
            ...
            if ($inForm['comiket_detail_type_sel2'] == '2') {//集荷の往復 : 復路
                $data2 = array(
                    "comiket_id" => $id,
                    "type" => "2",
                    "cd" => "ev{$customerCd}2",
    ...
    */
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            $fukuroNum = "復路：". $comiket_detail_list[0]['comiket_box_list'][0]['num']. "個";
            $data['num_area'] = $fukuroNum;
            
            //問合せ番号
            $fukuRoToi = "【復路・問合せ番号】";
            $fukuRoToi .= @$comiket_detail_list[0]["toiawase_no_niugoki"];
            
            //復路集荷日
            if(empty($comiket_detail_list[0]['collect_date'])) {
                $collectDateName = "";
            } else {
                $collectDate = new DateTime($comiket_detail_list[0]['collect_date']);
                $extWeek = date('w',strtotime($comiket_detail_list[0]["collect_date"]));
                $collectDateName = $collectDate->format('Y年m月d日')."(".$week[$extWeek].")";
            }             
            $data['type2_collect_date'] = $collectDateName;  
            
            $data['toiawase_no'] = $fukuRoToi;
