...
Sgmov_Lib::useForms(array('Error', 'BpnSession', 'Bpn003Out'));
/**#@-*/
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Bpn_Confirm extends Sgmov_View_Bpn_Common {
    ...
    public function executeInner() {
        ...
        // 戻る
        $backPath=$this->getBackInputPath($inForm['comiket_id']);
        $dispItemInfo['back_input_path'] = $backPath."/".$inForm["shikibetsushi"]."/".$inForm["bpn_type"]."/".$inForm["shohin_pattern"];

        // チケットを発行
        $ticket = $session->publishTicket(self::FEATURE_ID, self::GAMEN_ID_BPN003);
     ...
     * @return Sgmov_Form_Bpn003Out 出力フォーム
     */
    public function _createOutFormByInForm($inForm, $db) {
        return $this->createOutFormByInForm($inForm, new Sgmov_Form_Bpn003Out());
    }
}