 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Nss_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
    const FEATURE_ID = 'NSS';
    // 2022-03-10 ToanDD3 implement SMT6-69
    const LOGIN_ID = 'NSS_LOGIN';
     ...
     * イベントサブID
     */
    const EVENT_SUB_ID = '90';

    /**
     * NSS001の画面ID
     */
    const GAMEN_ID_NSS001 = 'NSS001';

    /**
     * NSS002の画面ID
     */
    const GAMEN_ID_NSS002 = 'NSS002';

    /**
     * NSS003の画面ID
     */
    const GAMEN_ID_NSS003 = 'NSS003';
...
///////////////////////////////////////////////////////////////////////////////////
     ...
     * @param type $inForm
     * @param Sgmov_Form_Nss001Out $outForm
     * @return type
     */
    ...
    public function setDispEvent(&$dispItemInfo){
        ...
        foreach($dispItemInfo['event_alllist'] as $k=>$v){
            if(isset($dispItemInfo['eventsub_selected_data']['event_id']) && $v['id']==$dispItemInfo['eventsub_selected_data']['event_id']){
                    ...
                    $dispItemInfo['dispEvent']['customName']=$v['event_name'].$v['eventsub_name'] .'係';
                    ...
                    // ロゴ画像ファイル名：例)logo.nss.gif logo_[イベント識別子].gif
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                    ...
                    if(isset($valDet["comiket_box_list"])){
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Nss_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Nss_Common::CURRENT_TAX));
                        // // 税込
    ...
    */
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($eventsubData['building_display'] !== '0') {
                ...
                if (!empty($comiket['building_name'])) {
                    if($comiket['building_name'] == "その他"){
                        ...
                        $building = "東". $comiket['building_name'];
                    }
                }
                
                $data['comiket_building_name'] = PHP_EOL . '【ブースNO】' . $building . $comiket['booth_position'] . " " . substr($comiket['booth_num'], -2);
            }
            ...
            // 2022-03-11 ToanDD3 implement SMT6-59
            // if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) { // 法人
            //     $mailTemplate[] = "/nss_parts{$footerTmpName}_footer_type_1.txt";

            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/nss_parts{$footerTmpName}_footer_type_1.txt";
            // } else { // 個人
            //     $mailTemplate[] = "/nss_parts{$footerTmpName}_footer_type_2.txt";

            //     // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
            //     $mailTemplateSgmv[] = "/nss_parts{$footerTmpName}_footer_type_2.txt";
            // }
            ...
            // $data['edit_url'] = $urlPublicSsl . "/nss/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
...
";
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // 2022-03-24 ToanDD3 implement SMT6-84
            // $data['cancel_url'] = $urlPublicSsl . "/nss/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            // $data['size_change_url'] = $urlPublicSsl . "/nss/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            if (!$isSecurity) {
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        try {
            ...
            $message = "にじそうさく: 業務連携に失敗しました。\n[nss] {$clasName} : paramOrg = {$paramOrg} / param = {$param}\n\nDB comiket.del_flg（論理削除フラグ）は更新しています。\n\n";
