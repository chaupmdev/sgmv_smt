 ...
 * #@-
 */
class Sgmov_View_Evp_Bcm extends Sgmov_Process_BcmSender {
	...
	private function selectComiket($comiket_id) {
		...
		$sql = "SELECT  *
				FROM    alpen_app
				WHERE   id = $1
				AND     payment_method_cd <> 1
				AND
				(
				    (payment_method_cd = 2 AND merchant_result = 1 AND receipted is not null)
				 OR (payment_method_cd = 3)
				 OR (payment_method_cd = 4 AND merchant_result = 1 AND auto_authoriresult = 'OK')
				 OR (payment_method_cd = 5)
				)
				";
	...
	private function bcmOutline(&$comiket) {
		...
		if ($comiket ["batch_status"] == self::STATUS_SEND) {
            ...
            // ステータス更新と管理者へエラーメール送信（ただし実際にメール送信するのはエラー時のみ）
			$this->SendMailManager ( $comiket ); 
		} else {
	...
	private function SendMailManager(&$comiket) {
		if ($comiket ["send_result"] == self::RESULT_FAILURE || $comiket ["send_result"] == self::RESULT_RETRY) {
			...
			Sgmov_Component_Mail::sendTemplateMail ( $comiket, dirname(dirname( dirname( __FILE__ ) )) . '/mail_template/bcm_error_send.txt', $mail_to );
		}
