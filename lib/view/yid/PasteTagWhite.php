 ...
 * #@-
 */
class Sgmov_View_Yid_PasteTag_White {
	...
	private function addSheet(&$pdf, &$comiket, $page) {
		...
		// 会場名（固定文言）
		$pdf->SetXY ( $x=26, $y=25 + $btm );
		$pdf->SetFont ( 'DejaVu', '', 18 );
		$pdf->Cell ( 100, 14, '東京ドームシティ プリズムホール', '1', 0, 'C' );

		// イベント名（変数）
		$pdf->SetXY ( $x=26, $y=39 + $btm );
		$pdf->SetFont ( 'DejaVu', '', 20 );
		$pdf->Cell ( 100, 14, 'よいどれ市' /*$comiket ['event_name']*/, '1', 0, 'C' );

		// 個口数・枠線
		$pdf->SetXY ( $x=133, $y=25 + $btm );
		$pdf->Cell ( 48, 28, '', '1', 0, '' );
		...
		$pdf->SetXY ( $x=26, $y=56 + $btm );
			$this->setColor ( $pdf, $whichDay, 2 );
			$pdf->SetFont ( 'DejaVu', '', 14 );
			$pdf->Cell ( 26, 26, 'ブース番号', 0, 0, 'C', 1 );

		// ブース番号とブース名  入力枠の下地（色ON）
		...
		$pdf->SetXY ( $x=57, $y=60 + $btm );
			...
			$pdf->Cell ( 35, 18, $comiket ['building_name'], 0, 0, 'L', 1 );

		// ブース名（固定文言）
		$pdf->SetXY ( $x=97, $y=67 + $btm );
			$this->setColor ( $pdf, $whichDay, 2 );
			$pdf->SetFont ( 'DejaVu', '', 14 );
			$pdf->Cell ( 16, 5, 'ブース名', 0, 0, 'C', 1 );

		// ブース名（変数）
		$pdf->SetXY ( $x=117, $y=60 + $btm );
			$this->setColor ( $pdf, $whichDay, 1 );	
			$pdf->SetFont ( 'DejaVu', '', 10 );
			$pdf->Cell ( 59, 18, $comiket ['booth_position'], 0, 0, 'L', 1 );
