 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Yid_CheckInput extends Sgmov_View_Yid_Common {
    ...
    public function executeInner() {
        ...
        // チケットの確認と破棄
        $session->checkTicket(self::FEATURE_ID, self::GAMEN_ID_TWF001, $this->_getTicket());
...
//        $inForm->comiket_detail_inbound_note = mb_convert_kana(filter_input(INPUT_POST, 'comiket_detail_inbound_note'), 'RNASKV', 'UTF-8');
        ...
        //if(!empty(filter_input(INPUT_POST, 'comiket_detail_caremark_flg'))) {
        if(!empty($post['comiket_detail_caremark_flg'])) {
            $inForm->comiket_detail_caremark_flg = 1;
        } else {
            $inForm->comiket_detail_caremark_flg = 0;
        }
        $inForm->comiket_detail_caremark_flg_enable = empty($post['comiket_detail_caremark_flg_enable']) ? 0 : 1;
        ...
        $inForm->comiket_detail_caremark_flg_enable_inbound = empty($post['comiket_detail_caremark_flg_enable_inbound']) ? 0 : 1;
...
//            }
            ...
            // ブース位置の未設定はエラーにしない
                ...
                $v = Sgmov_Component_Validator::createSingleValueValidator($buildingCd)->isSelected()->isIn($idList);
                if (!$v->isValid()) {
                    $errorForm->addError('building_name_sel', 'エリア、ブース番号' . $v->getResultMessageTop());
                } else {
                    $buildingBoothPostionInfoAry = $this->_BuildingService->fetchBuildingBoothPostionByBuildingCd($db, $buildingCd, $inForm->eventsub_sel);
                    $posIdList = array();
                    foreach ($buildingBoothPostionInfoAry['ids'] as $key => $val) {
                        $posIdList[] = $val;
                    }
                    $v = Sgmov_Component_Validator::createSingleValueValidator($inForm->building_booth_position_sel)->isSelected()->isIn($posIdList);
                    if (!$v->isValid()) {
                        $errorForm->addError('building_name_sel', 'エリア、ブース番号' . $v->getResultMessageTop());
                    }
                }
...
//            } 
        ...
        if($inForm->comiket_detail_type_sel == "1" || $inForm->comiket_detail_type_sel == "3") { // 搬入の場合
            ...
            //GiapLN add 梱包ガイドライン 2022.08.11
