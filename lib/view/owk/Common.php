...
Sgmov_Lib::useServices(array('CruiseRepeater', 'Prefecture', 'Yubin', 'SocketZipCodeDll'
    ...
    , 'AppCommon', 'HttpsZipCodeDll', 'BoxFare', 'CargoFare', 'Comiket', 'EventsubCmb', 'Time', 'CenterMail', 'ComiketDetail'));
 ...
 * @copyright  2018-2018 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
abstract class Sgmov_View_Owk_Common extends Sgmov_View_Public {
    /**
     * 機能ID
     */
     ...
     * OWK001の画面ID
     */
    const GAMEN_ID_OWK001 = 'OWK001';

    /**
     * OWK002の画面ID
     */
    const GAMEN_ID_OWK002 = 'OWK002';

    /**
     * OWK003の画面ID
     */
    const GAMEN_ID_OWK003 = 'OWK003';
...
///////////////////////////////////////////////////////////////////////////////////
     ...
     * @return 生成されたプルダウン
     */
    public static function _createPulldown($cds, $lbls, $select, $flg = null, $date = null, $execludePIdList = array()) {
        ...
        for ($i = 0; $i < $count; ++$i) {
            ...
            if (in_array($cds[$i], $execludePIdList)) {
                continue;
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // $outForm->raw_comiket_detail_inbound_collect_date_year_sel = "2020";
        // $outForm->raw_comiket_detail_inbound_collect_date_month_sel = "10";
        // $outForm->raw_comiket_detail_inbound_collect_date_day_sel = "17";
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function setYubinDllInfoToInForm(&$inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "2") { // 搬出
            ...
            $resultInboundPrefData["name"] = isset($resultInboundPrefData["name"]) ? $resultInboundPrefData["name"]: NULL;
    ...
    protected function calcEveryKindData($inForm, $comiketId = "", $isAmountDataFromSession = false) {
       ...
       $costTotal = 0;
        ...
        if ($inForm['comiket_detail_type_sel'] == "1" || $inForm['comiket_detail_type_sel'] == "2" || $inForm['comiket_detail_type_sel'] == "3") {
            ...
            foreach ($procList as $keyTree => $valTree) {
                ...
                foreach($valTree["comiket_detail_list"] as $keyDet => $valDet) {
                        ...
                        /////////////////////////////////////////////////////////
                        $valTree["comiket_detail_list"][$keyDet]['fare'] = ceil((string)($fareTotal / Sgmov_View_Owk_Common::CURRENT_TAX));

                        $valTree["comiket_detail_list"][$keyDet]['fare_tax'] = $fareTotal;

                        $valTree["comiket_detail_list"][$keyDet]['cost'] = ceil((string)($costTotal / Sgmov_View_Owk_Common::CURRENT_TAX));
                        // // 税込
        ...
        if ($inForm['comiket_detail_type_sel'] == "2" || $inForm['comiket_detail_type_sel'] == "3") { // 搬出
    ...
    public function _createComiketInsertDataByInForm($inForm, $id, $type="") {
        ...
        $inForm['merchant_result'] = isset($inForm['merchant_result']) ? $inForm['merchant_result']: NULL;
        $inForm['merchant_datetime'] = isset($inForm['merchant_datetime']) ? $inForm['merchant_datetime']: NULL;
        $inForm['receipted'] = isset($inForm['receipted']) ? $inForm['receipted']: NULL;
        $inForm['receipt_cd'] = isset($inForm['receipt_cd']) ? $inForm['receipt_cd']: NULL;

        $data = array(
            "id" => $id,
            "merchant_result" => isset($inForm['merchant_result']) ? $inForm['merchant_result']: NULL,
            "merchant_datetime" => isset($inForm['merchant_datetime']) ? $inForm['merchant_datetime']: NULL,
            "receipted" => isset($inForm['receipted']) ? $inForm['receipted']: NULL,
            "send_result" => "0",
            "sent" => NULL,
            ...
            "receipt_cd" => isset($inForm['receipt_cd']) ? $inForm['receipt_cd']: NULL,
            ...
            "transaction_id" => isset($inForm['sgf_res_transactionId']) ? $inForm['sgf_res_transactionId']: NULL,
            "auto_authoriresult" => isset($inForm['sgf_res_autoAuthoriresult']) ? $inForm['sgf_res_autoAuthoriresult']: NULL,
            "haraikomi_url" => isset($inForm['payment_url']) ? $inForm['payment_url']: NULL,
            "kounyuten_no" => isset($inForm['sgf_res_shopOrderId']) ? $inForm['sgf_res_shopOrderId']: NULL,
            "del_flg" => '0',
    ...
    public function _createComiketDetailInsertDataByInForm($inForm, $id) {
        ...
        if($inForm['comiket_detail_type_sel'] == "2") { // 搬出
            ...
            // TODO VASI
            // お預かり日時は、2020/10/17 17:00 20:00
            $collectStTime = null;
            $collectEdTime = null;
    ...
    public function checkCurrentDateWithInTerm($inForm) {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 往路
            if(!$this->isCurrentDateWithInTerm("departure", $inForm['eventsub_sel'])) {
                Sgmov_Component_Redirect::redirectPublicSsl("/owk/error_term/{$inForm['comiket_detail_type_sel']}");
                exit;
            }
            ...
            if($isDepartureErr && $isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/owk/error_term/3"); // 往復エラー画面
                exit;
            } else if($isDepartureErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/owk/error_term/1"); // 往路エラー画面
                exit;
            } else if($isArrivalErr) {
                Sgmov_Component_Redirect::redirectPublicSsl("/owk/error_term/2"); // 復路エラー画面
                exit;
            }
        }
    }
    ...
    public function sendCompleteMail($comiket, $sendTo2 = '', $sendCc = '', $type = '', $tmplateType = '') {
        ...
        try {
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) {
                //法人用メールテンプレート
                $mailTemplate[] = "/owk/owk_complete_business{$tmplateType}.txt";

                // 2018.09.10 tahira add 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/owk/owk_complete_business_sgmv{$tmplateType}.txt";
                ...
                //個人用メールテンプレート
                $mailTemplate[] = "/owk/owk_complete_individual{$tmplateType}.txt";

                // 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/owk/owk_complete_individual_sgmv{$tmplateType}.txt";
            ...
            foreach ($comiket_detail_list as $k => $comiket_detail) {
                ...
                if ($comiket_detail['type'] == 2) {
                    ...
                    //搬出用メールテンプレート
                    $mailTemplate[] = '/owk/owk_parts_complete_choice_2.txt';

                    // 申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                    $mailTemplateSgmv[] = '/owk/owk_parts_complete_choice_2_sgmv.txt';
            ...
            if ($comiket['div'] == self::COMIKET_DEV_BUSINESS) { // 法人
                $mailTemplate[] = "/owk/owk_parts{$footerTmpName}_footer_type_1.txt";

                //  申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/owk/owk_parts{$footerTmpName}_footer_type_1.txt";
            } else { // 個人
                $mailTemplate[] = "/owk/owk_parts{$footerTmpName}_footer_type_2.txt";

                //  申込者と営業所で使用するメールテンプレートをわけ別々に送信する
                $mailTemplateSgmv[] = "/owk/owk_parts{$footerTmpName}_footer_type_2.txt";
            }
            ...
            $data['edit_url'] = $urlPublicSsl . "/owk/input/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;

            // 説明書URL
//            $data['manual_url'] = $urlPublicSsl . "/owk/pdf/manual/manual_{$comiket["eventsub_id"]}.pdf";
            $data['manual_url'] = '';

            if($eventsubData['manual_display'] == '1') { // コミケとGoOutCampは記載しない
                $data['manual_url'] = "【宅配便WEB申込みのご案内】" . PHP_EOL . $urlPublicSsl . "/owk/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            }
//            $data['manual_url'] = $urlPublicSsl . "/owk/pdf/manual/{$eventData['name']}{$eventsubData['name']}.pdf";
            ...
            if(($comiket['choice'] == "1" || $comiket['choice'] == "3")
                ...
                $data['paste_tag_url'] = "【貼付票URL】" . PHP_EOL . $urlPublicSsl . "/owk/paste_tag/{$pasteTagId}/";
            }
            
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            // サイズ変更/キャンセルURL
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            $data['cancel_url'] = $urlPublicSsl . "/owk/cancel/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            $data['size_change_url'] = $urlPublicSsl . "/owk/size_change/" . sprintf('%010d', $comiket["id"]) . $comiketIdCheckD;
            //-------------------------------------------------
            //メール送信実行
    ...
    protected function execWebApiCancelComiket($wsProtocol, $wsHost, $wsPath, $wsPort, $param, $paramOrg , $dispErrTitle = "", $dispErrMsg = "") {
        ...
        $sendFileName = 'rms_' . date ( 'YmdHis' ) . '.csv';
        $boundary = "-----" . md5 ( uniqid () );
...
//            $eventsubInfo = $this->_EventsubService->fetchEventsubByEventsubId($db, $comiketInfo['eventsub_id']);
            ...
            $mailTemplateList = array(
                "/Owk_cancel_{$divType}_error.txt",
//                "/Owk_parts_cancel_footer_type_{$comiketDetailInfo['type']}.txt",
            );
        ...
        foreach ($comiketDetailList as $key => $comiketDetailInfo) {
            if($comiketDetailInfo['type'] == '2') { // 往路
                ...
                if ($eventTermEndDate <= $toDate) {
                    ...
                    Sgmov_Component_Redirect::redirectPublicSsl("/owk/error?t={$title}&m={$message}");
                }
