...
Sgmov_Lib::useView('tms/Common', 'CommonConst');
Sgmov_Lib::useView('tms/Bcm');
 ...
 */
class Sgmov_View_Tms_Complete extends Sgmov_View_Tms_Common {
    ...
    public function __construct() {
        ...
        $this->_BcmView             = new Sgmov_View_Tms_Bcm();
    ...
    public function executeInner() {
        ...
        if($inForm['comiket_detail_type_sel'] == "1") { // 搬入
            ...
            if ($dateColl <= $dateNow) {
                ...
                Sgmov_Component_Redirect::redirectPublicSsl("/tms/error/?t=受付時間が終了しました&m=お預かり日：{$yearColl}/{$monthColl}/{$dayColl}の受付時間が終了しました");
                exit;
            }
        }
        /////////////////////////////////////////////////////////////////////////////////////////////
...
//                    $comiketDataInfo = $this->_cmbTableDataFromInform($inForm, $comiketId);
                    ...
                    if (@empty($calcDataInfo['amount_tax_free'])) {
                        // 支払い金額が \0 の場合 ベリトランス連携はしない
                        $inForm['merchant_result'] = '1'; // メール処理や以降の処理でエラーにならないよう "1" を設定しておく（DB売掛登録の時は "0" にして登録している）
                                                          // この場合は、無料のみの登録の場合となる
                        $inForm['receipt_cd'] = NULL;
                        break;
                    }

                    $inFormForCredit = $calcDataInfo['amount_tax_free'];
...
//            $inForm['merchant_result'] = '1'; // 決済データ送信結←tahira change 0を入れると申込完了メールが送信されない為1を固定セットにする
        ...
        // 無料box情報
        $freeBoxInfo = $this->getFreeBoxInfoInInForm($inForm);
        ...
        try {
            ...
            if (@!empty($freeBoxInfo)) {
                ...
                // 無料売掛データ作成
                ////////////////////////////////////////////////////////////
                $comiketDataInfoFlatForFreeBox = $comiketDataInfoFlat;
                
                //////////////////////////
                // コミケ申込データ 
                //////////////////////////
                $comiketDataInfoFlatForFreeBox["comiketData"]['merchant_result'] = 0; // 売掛なので "0：未送信（送信失敗）" を設定
                $comiketDataInfoFlatForFreeBox["comiketData"]['merchant_datetime'] = null;
                $comiketDataInfoFlatForFreeBox["comiketData"]['receipted'] = null;
                $comiketDataInfoFlatForFreeBox["comiketData"]['authorization_cd'] = null;
                $comiketDataInfoFlatForFreeBox["comiketData"]['send_result'] = 0;
                $comiketDataInfoFlatForFreeBox["comiketData"]['sent'] = null;
                $comiketDataInfoFlatForFreeBox["comiketData"]['batch_status'] = 1; // 1:登録済
                $comiketDataInfoFlatForFreeBox["comiketData"]['payment_method_cd'] = 5; // 売掛
                $comiketDataInfoFlatForFreeBox["comiketData"]['payment_order_id'] = null;
                $comiketDataInfoFlatForFreeBox["comiketData"]['customer_cd'] = $comiketDataInfoFlatForFreeBox["comiketData"]['customer_cd']; // TODO customer_cd確認

                // 金額（税抜き）
                $comiketAmount = $comiketDataInfoFlat["comiketData"]['amount'];
                $comiketAmountFree = $comiketDataInfoFlat["comiketData"]['amount_free'];
                $calcRes = $comiketAmount - $comiketAmountFree;
                $comiketDataInfoFlatForFreeBox["comiketData"]['amount'] = @empty($calcRes) ? $comiketAmountFree : $calcRes;
                
                // 金額（税込み）
                $comiketAmountTax = $comiketDataInfoFlat["comiketData"]['amount_tax'];
                $comiketAmountTaxFree = $comiketDataInfoFlat["comiketData"]['amount_tax_free'];
                $calcResTax = $comiketAmountTax - $comiketAmountTaxFree;
                $comiketDataInfoFlatForFreeBox["comiketData"]['amount_tax'] = @empty($calcResTax) ? $comiketAmountTaxFree : $calcResTax;
                
                $comiketDataInfoFlatForFreeBox["comiketData"]['id_sub'] = $comiketIdSub;
                
                if (@empty($calcResTax)) {
                    $isUnsetOrgDbInsert = true;
                }

                //////////////////////////
                // コミケ申込明細データ 
                //////////////////////////
                $comiketDataInfoFlatForFreeBox["comiketDetailDataList"][0]["cd"] .= $comiketDataInfoFlatForFreeBox["comiketData"]['id_sub'];
                
                // 金額（税抜）
                $comiketDetFare = $comiketDataInfoFlat["comiketDetailDataList"][0]["fare"];
                $comiketDetFareFree = $comiketDataInfoFlat["comiketDetailDataList"][0]["fare_free"];
                $comiketDetCalcRes = $comiketDetFare - $comiketDetFareFree;
                $comiketDataInfoFlatForFreeBox["comiketDetailDataList"][0]["fare"] = @empty($comiketDetCalcRes) ? $comiketDetFareFree: $comiketDetCalcRes;
                
                // 金額（税込）
                $comiketDetFareTax = $comiketDataInfoFlat["comiketDetailDataList"][0]["fare_tax"];
                $comiketDetFareTaxFree = $comiketDataInfoFlat["comiketDetailDataList"][0]["fare_tax_free"];
                $comiketDetTaxFreeCalcRes2 = $comiketDetFareTax - $comiketDetFareTaxFree;
                $comiketDataInfoFlatForFreeBox["comiketDetailDataList"][0]["fare_tax"] = @empty($comiketDetTaxFreeCalcRes2) ? $comiketDetFareTaxFree : $comiketDetTaxFreeCalcRes2;
                
                $comiketDataInfoFlatForFreeBox["comiketDetailDataList"][0]["comiket_id_sub"] = $comiketIdSub;
                
                //////////////////////////
                // コミケ申込宅配データ 
                //////////////////////////
                $unsetBoxData = null;
                $comiketDataInfoFlatForFreeBox["comiketBoxDataList"] = array();
                foreach ($comiketDataInfoFlat["comiketBoxDataList"] as $key => $val) {
                    if ($freeBoxInfo['boxId'] == $val['box_id']) {
                        $boxInfo = $this->_BoxService->fetchBoxById($db, $freeBoxInfo['boxId']);
                        $boxFareData = $this->_BoxFareService->fetchBoxFareByJis2AndBoxId(
                                $db, $inForm["inbound_hatsu_jis2code"], $inForm["inbound_chaku_jis2code"], $boxInfo["cd"],  $comiketDataInfoFlatForFreeBox["comiketData"]["eventsub_id"]);

                        $comiketDataInfoFlatForFreeBox["comiketBoxDataList"][0] = $val;
                        $comiketDataInfoFlatForFreeBox["comiketBoxDataList"][0]["num"] = $freeBoxInfo['freeNum'];
                        $comiketDataInfoFlatForFreeBox["comiketBoxDataList"][0]["fare_price"] = $val['fare_price'];
                        
                        // 金額（税抜）
                        $comiketBoxFareCalcRes = $val['fare_price'] - $val['fare_price_free'];
                        $comiketDataInfoFlatForFreeBox["comiketBoxDataList"][0]["fare_amount"] = @empty($comiketBoxFareCalcRes) ? $val['fare_price_free'] : $comiketBoxFareCalcRes;
                        $comiketDataInfoFlatForFreeBox["comiketBoxDataList"][0]["fare_price_tax"] = $val['fare_price_tax'];
                        
                        $comiketDataInfoFlatForFreeBox["comiketBoxDataList"][0]["comiket_id_sub"] = $comiketIdSub;
                        
                        if ($val['num'] == $freeBoxInfo['freeNum']) {
                            $unsetBoxData = $key;
                            break;
                        }
                        
                        $comiketDataInfoFlat["comiketBoxDataList"][$key]['num'] -= $freeBoxInfo['freeNum'];
                        $comiketDataInfoFlat["comiketBoxDataList"][$key]["fare_amount"] = $val['fare_price_free'];
                ...
                $this->_Comiket->insert($db, $comiketDataInfoFlatForFreeBox["comiketData"]);
                
                foreach ($comiketDataInfoFlatForFreeBox["comiketDetailDataList"] as $key => $val) {
                    $this->_ComiketDetail->insert($db, $val);
                }
                
                foreach ($comiketDataInfoFlatForFreeBox["comiketBoxDataList"] as $key => $val) {
            ...
            if (@!empty($calcDataInfo['amount_tax_free'])) {
                // 支払い金額が \0 でない場合
                $comiketDataInfoFlat["comiketData"]["id_sub"] = $comiketIdSub;
                $this->_Comiket->insert($db, $comiketDataInfoFlat["comiketData"]);
Sgmov_Component_Log::debug("################# 805-2");
                foreach ($comiketDataInfoFlat["comiketDetailDataList"] as $key => $val) {
                    $val['comiket_id_sub'] = $comiketIdSub;
                    $val['cd'] .= $comiketIdSub;
                    $this->_ComiketDetail->insert($db, $val);
                }
Sgmov_Component_Log::debug("################# 805-3");
                foreach ($comiketDataInfoFlat["comiketBoxDataList"] as $key => $val) {
                    $val['comiket_id_sub'] = $comiketIdSub;
                    $this->_ComiketBox->insert($db, $val);
                }
Sgmov_Component_Log::debug("################# 805-4");
                foreach ($comiketDataInfoFlat["comiketCargoDataList"] as $key => $val) {
                    $this->_ComiketCargo->insert($db, $val);
                }
Sgmov_Component_Log::debug("################# 805-5");
            ...
            Sgmov_Component_Log::debug("リダイレクト /tms/{$backInputPaht}/");
            Sgmov_Component_Log::err('データベースの登録・更新に失敗しました。');
            Sgmov_Component_Log::err($e);
            Sgmov_Component_Redirect::redirectPublicSsl("/tms/{$backInputPaht}/");
            exit;
        }
        $db->commit();
...
//                }
                ...
                $param = array('res_sgf_transactionId' => @$resultData["transactionInfo"]["transactionId"]);
                $mail_to = Sgmov_Component_Config::getLogMailTo();
                Sgmov_Component_Mail::sendTemplateMail($param, dirname(__FILE__) . '/../../mail_template/dsn_error_for_sgfinancial_shipment_report.txt', $mail_to);
                Sgmov_Component_Log::info('SgFinancial-ShipmentReport-NG');
            }
        }
        
        // $li 取得時は既にデータがあるため、再設定の必要あり
        $li["treeData"]['amount_free'] = $calcDataInfo['amount_free'];
        $li["treeData"]['amount_tax_free'] = $calcDataInfo['amount_tax_free'];

//        $li["treeData"]['amount'] = $calcDataInfo['amount_free'];
//        $li["treeData"]['amount_tax'] = $calcDataInfo['amount_tax_free'];
        ...
        if ((!empty($li["treeData"]['mail']) && !empty($li["treeData"]['merchant_result']))
            ...
            $this->sendCompleteMail($li["treeData"], $li["treeData"]['mail'], $ccMail, $type, '', $freeBoxInfo);
        }
