...
Sgmov_Lib::useForms(array('Error', 'PcsSession', 'Pcs001Out'));
/**#@-*/
 ...
 * @copyright  2009-2010 SAGAWA COMPUTERSYSTEM CO,.LTD. All rights reserved.
 */
class Sgmov_View_Pcs_Input extends Sgmov_View_Pcs_Common
{
    ...
    public function executeInner()
    {
        ...
        if (isset($sessionForm)) {
            ...
            // 出力情報を設定
            $outForm = new Sgmov_Form_Pcs001Out();
        ...
        // チケット発行
        $ticket = $session->publishTicket(self::FEATURE_ID, self::GAMEN_ID_PCS001);
 ...
 //       $outForm->raw_pref_lbls = $prefs['names'];
     ...
     * @return Sgmov_Form_Pcs001Out 出力フォーム
     */
     ...
     * @param Sgmov_Form_Pcs001Out $outForm 出力フォーム
     * @return Sgmov_Form_Pcs001Out 出力フォーム
     */
...
//        $prefs = $service->fetchPrefectures($db);
    ...
    public function _parseGetParameter() {
        ...
        if (!isset($_GET['param'])) {
            ...
            if (!preg_match("/^[0-9]+$/", $params[0]) || !(1 <= $params[0] && $params[0] <= 6)) {
                // 半角数字、または、1～6以内でない場合、NULLをセット
                return NULL;
            }
