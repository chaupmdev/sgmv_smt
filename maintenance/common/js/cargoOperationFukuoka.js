...
function clickTanshinCargoPlan(clickId) {
	...
	if (clickId == "1_1") {
		...
		if (document.getElementById("1_2").checked) {
			...
			//福岡県以外の出発エリア・到着エリア非活性化処理
			for (i = 0; i < (allFromAreas.length - 1); i++) {
				if (allFromAreas[i] != "area_from_center_0_3") {
			...
			//福岡県/東京23区/大阪府/福岡県以外の出発エリア・到着エリア非活性化処理
			for (i = 0; i < (allFromAreas.length - 1); i++) {
 ...
 * centerId（1:福岡県、2:東京23区、3:福岡）がクリックされた場合
 *
 ***************************************************************************/
function clickArea(centerId) {

	// 福岡県のチェック有無
	sapporo = document.getElementById("area_from_center_0_3").checked;
	// 単身カーゴプランのチェック有無
	cargo = document.getElementById("1_1").checked;
	// 単身エアカーゴのチェック有無
	aircargo = document.getElementById("1_2").checked;
	...
	if (aircargo) {
		...
		if (centerId == "area_from_center_0_3") {
			if (document.getElementById("area_from_center_0_3").checked) {
				// 福岡県にチェックを入れた場合
				// 北海道（札幌）、東京23区を非活性、チェックを外す
				for (i = 0; i < (allToAreas.length - 1); i++) {
					if (allToAreas[i] != "area_to_center_0_0" && allToAreas[i] != "area_to_center_2_0") {
						document.getElementById(allToAreas[i]).disabled = true;
						document.getElementById(allToAreas[i]).checked = false;
			...
			if (sapporo) {
				// 札幌on
				clickArea("area_from_center_0_3");
			}
		}
	} else {
